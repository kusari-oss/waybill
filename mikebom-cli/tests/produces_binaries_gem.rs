//! Integration tests for the milestone 116 PR-B gem slice (FR-008).
...
#[test]
fn gemspec_executables_array_extracts_all_names() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let out = tmp.path().join("sbom.cdx.json");
    let output = run_scan(&fixture("with-executables"), &out);
    assert!(
    ...
    let produced = produces_binaries_for_purl(&sbom, "pkg:gem/fixture-baz@1.0.0")
        .expect("gem main-module component should be present");
    assert_eq!(
        produced,
        vec!["baz".to_string(), "baz-server".to_string()]
    );
}

#[test]
fn library_only_gemspec_omits_produces_binaries_property() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let out = tmp.path().join("sbom.cdx.json");
    ...
    let sbom = read_sbom(&out);
    let produced = produces_binaries_for_purl(&sbom, "pkg:gem/fixture-libonly@1.0.0");
    match produced {
        None => {} // component not emitted
        Some(v) => assert!(
            v.is_empty(),
            "library-only gem must NOT carry produces-binaries; got {v:?}"
        ),
    }
}
