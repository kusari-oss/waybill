...
fn bazel_module_emits_pkg_bazel_purls_with_native_scope() {
    ...
    // emits as `mikebom:lifecycle-scope = "development"` per the
    // existing milestone-052 mapping.
    let scope_prop = component_property(googletest[0], "mikebom:lifecycle-scope");
    assert_eq!(
        scope_prop,
        Some("development"),
...
fn bazel_workspace_http_archive_emits_url_and_sha() {
    ...
    assert_eq!(
        component_property(rules_python[0], "mikebom:download-url"),
        Some("https://github.com/bazelbuild/rules_python/archive/0.30.0.tar.gz")
    );
    assert_eq!(
        component_property(rules_python[0], "mikebom:bazel-archive-name"),
        Some("rules_python")
    );
...
fn bazel_workspace_git_repository_short_sha() {
    ...
    assert_eq!(
        component_property(rules_foo[0], "mikebom:download-url"),
        Some("https://github.com/foo/rules_foo.git")
    );
}

/// FR-010: every Bazel component carries `mikebom:source-files`.
#[test]
fn bazel_components_carry_source_files() {
    ...
    for c in &bazel_components {
        let source = component_property(c, "mikebom:source-files")
            .expect("every bazel component must carry mikebom:source-files (FR-010)");
        assert!(
