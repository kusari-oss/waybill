...
//! `mikebom:source-read-set` (C130) carries the synthetic `<stdin>`
...
//! in `mikebom-cli/src/trace/compiler_pipeline.rs`) has a documented
...
//     // 1. spawn `mikebom trace run -- gcc -x c - < <(cat main.c)`
...
//     // 4. assert its `mikebom:source-read-set` payload's read_set
