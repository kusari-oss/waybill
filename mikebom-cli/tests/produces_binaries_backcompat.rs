...
use mikebom::binding::{AliasSource, BindingStrength, SourceDocumentBinding, SourceSbomContext};
use serde_json::json;
...
fn pre_feature_envelope_serializes_round_trip_byte_identical() {
    ...
    let baseline = SourceDocumentBinding::unknown(
        mikebom::binding::SourceDocumentId {
            sha256: "deadbeef".repeat(8),
            iri: None,
        },
...
fn post_feature_envelope_carries_alias_source_when_set() {
    ...
    let binding = SourceDocumentBinding {
        source_doc_id: mikebom::binding::SourceDocumentId {
        ...
        reason: None,
        algo: mikebom::binding::BINDING_HASH_ALGO_V1.to_string(),
        alias_from: mikebom_common::types::purl::Purl::new("pkg:generic/baz").ok(),
        alias_to: mikebom_common::types::purl::Purl::new("pkg:cargo/baz@1.0.0").ok(),
