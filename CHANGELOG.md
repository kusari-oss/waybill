...
All notable changes to waybill are recorded here. The format follows
...
Milestone 061 introduced the C45 `waybill:orphan-reason` per-component
...
emitter at `waybill-cli/src/scan_fs/package_db/golang/legacy.rs:2091,2118`
UNCHANGED. New emit-time classifier at
`waybill-cli/src/generate/orphan_reason.rs` runs after
...
`waybill:orphan-reason=unresolved-indirect-require` annotation on
...
```bash
jq '[.components[] |
    select(.properties[]?.name == "waybill:orphan-reason") |
    {purl: .purl,
     reason: (.properties[] |
              select(.name == "waybill:orphan-reason") |
              .value)}] |
    group_by(.reason) |
...
was a real bug discovered in waybill's SPDX 3 emission on live upstream Go
...
**Root cause**: waybill's SPDX 3 annotation merge point at
`waybill-cli/src/generate/spdx/v3_document.rs:754-820` combined 4 annotation
...
`hash(subject_iri | field)` — e.g., `waybill:graph-completeness` emitted on
...
`waybill:graph-completeness=partial` annotation on the SpdxDocument subject
...
**Consumer jq recipe** to verify the uniqueness invariant on any waybill SPDX
3 SBOM:
...
waybill sbom scan --path <monorepo> 2>&1 | grep 'pnpm-lock parsed'
...
measurement showed waybill's npm workspace-peer readers emitted **159
...
- **Unresolved** (neither source produced a hit) → dep-name accumulated
  into the peer's `waybill:unresolved-declared-dep` (C115) annotation.
...
- `waybill:unresolved-declared-dep` (C115, per-component) — names dep(s)
  declared in a workspace-peer's `package.json` (in `dependencies:` or
  `devDependencies:`) that waybill couldn't cross-resolve against the
...
- Q1 (unresolvable dep declaration) + Q2 (range mismatch): **SUPPRESS
  the edge from `dependsOn` + emit `waybill:unresolved-declared-dep`
...
jq '.components[]
    | select((.properties // [])[] | .name == "waybill:unresolved-declared-dep")
    | {name, purl,
       unresolved: (.properties[] | select(.name == "waybill:unresolved-declared-dep") | .value)}' \
   sbom.cdx.json
...
showed waybill silently dropped 1 of 250 declared Gemfile.lock edges
...
is not in GEM/specs, waybill emits a synthetic `pkg:gem/<name>`
component with **versionless PURL** (no `@version` segment per Q1 +
FR-003) + two per-component annotations.

**New annotations**:

- `waybill:synthetic-built-in` (C113, per-component) — bare string
...
- `waybill:built-in-requirement` (C114, per-component) — preserves
...
jq '.components[]
    | select((.properties // [])[] | .name == "waybill:synthetic-built-in")
    | {name, purl,
       req: (.properties[] | select(.name == "waybill:built-in-requirement") | .value // null)}' \
   sbom.cdx.json

# Detect real (non-synthetic) gems only
jq '.components[]
    | select(.purl // "" | startswith("pkg:gem/"))
    | select((.properties // []) | all(.name != "waybill:synthetic-built-in"))
    | .purl' sbom.cdx.json
```
...
`waybill-cli/tests/ruby_built_in_gems.rs` exercising the release
...
v1.x with `go.work` + 40+ nested modules) showed waybill's Go
...
- `waybill:go-workspace-mode` (C112) — three-value document-scope
  string with grammar `detected: <N> use-modules` | `absent` |
  `malformed: <reason>`. Distinct from milestone-158's
  `waybill:graph-completeness` (C104) and milestone-160's
  `waybill:go-transitive-coverage` (C110) per research.md R1: C104
...
jq -r '.metadata.properties[] | select(.name == "waybill:go-workspace-mode") | .value' sbom.cdx.json

# Extract use-module count
jq -r '.metadata.properties[]
       | select(.name == "waybill:go-workspace-mode")
       | .value
...
- T053 (test-kubernetes fixture landing in `kusari-oss/waybill-fixtures`).
...
`waybill-cli/tests/go_workspace_edges.rs` (synthetic 3-module go.work
fixture end-to-end).
...
measurement on `kusari-sandbox/test-podman` showed waybill's Go
...
- `waybill:go-transitive-source` (C108) — kebab-case string naming
...
- `waybill:go-transitive-unresolved-reason` (C109) — kebab-case
...
- `waybill:go-transitive-coverage` (C110) — three-value string
  `complete` | `partial` | `unknown`. Distinct from milestone-158's
  `waybill:graph-completeness` (C104) per research.md R1: C104
...
- `waybill:go-transitive-coverage-reason` (C111) — grammar
...
- Q2: `waybill:go-transitive-source` — universal on every Go
...
jq -r '.metadata.properties[] | select(.name == "waybill:go-transitive-coverage") | .value' sbom.cdx.json

# Count Go modules per resolution source
jq '[.components[]
     | select(.purl // "" | startswith("pkg:golang/"))
     | .properties // [] | .[]
     | select(.name == "waybill:go-transitive-source") | .value]
...
`waybill-cli/tests/go_transitive_coverage.rs` exercising the
...
aliases to a different real package. Pre-159 waybill emitted
...
1. New `waybill-cli/src/scan_fs/package_db/npm/alias_mapping.rs`
   submodule with:
...
2. Both `pnpm_lock.rs::parse_pnpm_lock` and
   ...
   - Rewrite other entries' `depends` lists to reference the aliased
     canonical name.
   - Attach `waybill:pnpm-alias` / `waybill:yarn-alias` component-
...
case), waybill emits the annotation with a `Value::Array` of
...
jq '.components[] | select((.properties // [])[] | .name | test("^waybill:(pnpm|yarn)-alias$")) | {purl, alias: (.properties[] | select(.name | test("-alias$")) | .value)}' sbom.cdx.json

# Count alias-affected components per ecosystem
jq '[.components[] | .properties // [] | .[] | .name | select(test("^waybill:(pnpm|yarn)-alias$"))] | group_by(.) | map({(.[0]): length}) | add' sbom.cdx.json
```

**Parity catalog**: 2 new rows C106 (`waybill:pnpm-alias`) + C107
(`waybill:yarn-alias`) with `Directionality::SymmetricEqual`.
Milestone-071 parity check enforces symmetric emission across CDX +
SPDX 2.3 + SPDX 3.

Zero new Cargo dependencies. `waybill-ebpf` untouched.
...
`kusari-sandbox/test-*` repos). For workspace monorepos, waybill's
...
2. **`waybill:graph-completeness` + `waybill:graph-completeness-reason`
...
- The pre-implementation ≥99% target was miscalibrated: the fixture
  ...
  edge-resolution issue in waybill's npm workspace-peer parsers,
...
- 25 new unit tests in `waybill-cli/src/generate/graph_completeness/`
...
- SC-002 dual-side byte-identity guard: 27 of 33 milestone-090 goldens
  changed by exactly the added `waybill:graph-completeness = complete`
...
completeness=$(jq -r '.metadata.properties[] | select(.name == "waybill:graph-completeness") | .value' sbom.cdx.json)
case "$completeness" in
    complete)
        echo "Graph is fully connected — safe to consume"
        ;;
    partial)
        reason=$(jq -r '.metadata.properties[] | select(.name == "waybill:graph-completeness-reason") | .value' sbom.cdx.json)
...
- Principle V (Standards-native precedence): FR-010 acknowledges the
  `waybill:*` deviation is required because no CDX 1.6 / SPDX 2.3 /
...
Empirical reproduction 2026-07-03 confirmed the bug: waybill's pnpm
parser at `waybill-cli/src/scan_fs/package_db/npm/pnpm_lock.rs:83-91`
...
section — waybill's parser never learned to read it. Result on
...
- 9 new unit tests in `pnpm_lock.rs` (SC-007 floor ≥8; 12 total pnpm
  tests after impl).
- 4 integration tests in `waybill-cli/tests/npm_pnpm_v9_dep_graph.rs`:
...
emitter code. Zero new `waybill:*` annotation keys. Zero new catalog
...
`WAYBILL_CMAKE_THIRD_PARTY_RECURSIVE=1`) extends recursion to
`third_party/` too when set.

**Implementation**: reuses milestone-054's `safe_walk` helper
(`waybill-cli/src/scan_fs/walk.rs`) for the recursive descent —
...
extended recursive descent (per FR-018 waybill does NOT auto-exclude
...
jq '.components[] | select(.properties[]?
  | select(.name == "waybill:source-mechanism" and .value == "cmake-find-package"))
  | select(.properties[]?
    | select(.name == "waybill:source-files" and (.value | contains("cmake/modules/"))))
  | .purl' sbom.cdx.json
```

Wire-format-agnostic: no changes to CycloneDX 1.6 / SPDX 2.3 / SPDX
3.0.1 emitters. Zero new `waybill:*` annotation keys. Zero new
...
- `find_package(<Name> [<Version>])` → `pkg:generic/<lowercased-name>[@<highest-declared-version>]`
  with `waybill:source-mechanism = "cmake-find-package"`. Multi-file
...
- `pkg_check_modules(<TARGET> [REQUIRED] [IMPORTED_TARGET] <modules>)`
  ...
  keywords filtered) with `waybill:source-mechanism = "cmake-pkg-check-modules"`.
- **Case preservation**: `find_package(OpenSSL 1.1.0)` emits
  `pkg:generic/openssl@1.1.0` (PURL lowercased per spec convention) and
  a new `waybill:cmake-find-package-name = "OpenSSL"` annotation
...
```bash
jq '.components[] | select(.properties[]?
  | select(.name == "waybill:source-mechanism"
...
`cmake/modules/Find*.cmake` and are NOT reached by waybill's current
...
`waybill-cli/tests/fixtures/cmake-find-package/kamailio-shape/`
...
`waybill:also-detected-via` list into production emission.
...
`waybill:*`-prefixed annotation key (`waybill:cmake-find-package-name`)
...
waybill now sweeps every emitted `simplelicensing_LicenseExpression`
...
```json
{
  ...
  "name": "{idstring}",
  "simplelicensing_licenseText": "License text not extracted by waybill. Consult the original package (e.g., /usr/share/licenses/<name>/ on Debian/RPM, or upstream project source) for the full text."
}
```
...
new `waybill:*` annotation introduced. **Principle IX + X**: the
placeholder text explicitly discloses that waybill did not extract the
...
waybill now sweeps every emitted SPDX 2.3 document for inline
...
The `<name>` token is a LITERAL — waybill does not substitute the
...
| select(.extractedText | startswith("License text not extracted by waybill."))
```
...
is defined in the referenced OTHER document, not this one. waybill
...
waybill now wraps each unrecognized operand as a SPDX 2.3-spec-blessed
...
a `WITH` clause is unrecognized, waybill wraps it as
`LicenseRef-<sanitized>` and preserves the exception. When the
EXCEPTION (right side) is unrecognized, waybill conservatively collapses
...
identifiers — no new `waybill:*` annotation key introduced.

### Homebrew (brew + Linuxbrew) package detection (closes #432)

waybill gains a sixth OS package-DB reader alongside dpkg, apk, rpm,
...
**Why waybill-specific** (Constitution Principle V audit): the
`pkg:brew/` PURL type is not yet registered in the purl-spec
PURL-TYPES.rst — waybill emits per the de-facto convention shared
...
`waybill:*` annotations introduced; zero new parity-catalog C-rows;
...
waybill gains a fifth OS package-DB reader alongside the existing
...
component-identity field. Zero new `waybill:*` annotations
...
**Per-component property** `waybill:duplicate-purl-divergent` (C99)
...
**Document-scope summary** `waybill:purl-collisions-detected` (C100)
...
`CollisionsSummary` types in `waybill-common::divergence` are
...
(`waybill-cli/tests/fixtures/golden/{cyclonedx,spdx-2.3,spdx-3}/*.json`)
stay byte-identical pre/post this milestone (SC-002 invariant).

**Why waybill-specific (Constitution Principle V audit)**: no CDX 1.6
...
`waybill_common::types::purl::Purl::new`; non-spec input fails fast
...
(`waybill-cli/tests/root_purl_flag.rs`) covering verbatim emission
...
`waybill:license-concluded-source = "operator-asserted"` annotation
...
**Comparison data** (waybill v0.1.0-alpha.47 vs syft v1.42.3 on
KubeLB v1.4.2 per the issue body):

| Field                       | waybill default | waybill + `--conclude-licenses` | syft   |
|-----------------------------|-----------------|----------------------------------|--------|
| `licenseDeclared` coverage  | 99.2 %          | 99.2 %                           | 61.3 % |
| `licenseConcluded` coverage | 0 %             | 99.2 %                           | 97.5 % |

waybill now matches syft's `licenseConcluded` coverage when the
operator opts in, while preserving the SPDX-correct provenance
distinction via the `waybill:license-concluded-source` annotation
(C98 catalog row).

**New parity catalog row C98** `waybill:license-concluded-source`
...
(`waybill-cli/tests/conclude_licenses.rs`) — default-mode preserves
...
- **Strict Boundary §5 added** — "No file-tier duplicates in default
  ...
  level `waybill:file-inventory-mode = "full"` annotation so
...
**`waybill:file-inventory-mode` annotation (C97)**:
...
- "Why waybill rejected the alternative designs" section
...
|---|---|---|---|
| C93 | `waybill:file-inventory-skipped-oversize` | document | Count of files skipped because their size exceeded `--file-inventory-size-limit`. |
| C94 | `waybill:file-inventory-skipped-special-files` | document | Count of special files (devices, sockets, FIFOs) skipped. |
| C95 | `waybill:file-inventory-unreadable` | document | Count of files waybill could not open / read (permissions, missing, mid-flight delete). |
| C96 | `waybill:file-paths-truncated` | per-component | `"true"` when a file-tier component's `waybill:file-paths` array hit the 100-entry FR-007 cap. |
...
- **CDX 1.6** (already correct from US1.B): `type = "file"`, no `purl`, name = basename, SHA-256 in `hashes[]`, `waybill:component-tier = "file"` + `waybill:file-paths` in `properties[]`.
- **SPDX 2.3** (new in US1.C): Package with `filesAnalyzed: false`, no `externalRef[purl]`, SHA-256 in `checksums[]`, `waybill:component-tier` + `waybill:file-paths` as `Package.annotations[]` envelope entries.
...
- **C91** `waybill:component-tier` — per-component annotation. CDX side rides through `properties[]`; SPDX 2.3 / SPDX 3 sides ride through the standard annotation envelope. Extractors wired symmetrically; `holistic_parity::all_extractors_symmetric_or_directional` catches drift.
- **C92** `waybill:file-paths` — per-component annotation carrying a JSON-encoded sorted-ascending path array. Same emission shape across formats.
...
libraries with no package metadata / embedded archives that waybill
...
- 6 new C-rows + parity extractors (`waybill:component-tier`,
  `waybill:file-paths`, `waybill:file-paths-truncated`,
...
forcing downstream tools to scrape the `waybill:source-files` annotation
...
(`waybill-cli/src/generate/cyclonedx/evidence.rs:113`); SPDX 2.3 +
...
`waybill:*` namespace addition; no new C-row.

### `waybill:layer-digest` per-component property for image scans (milestone 133 US2.2)

Adds a new C88 catalog row + `waybill:layer-digest` per-component property
...
feature-parity gap: pre-133 waybill emitted no layer-digest information,
making image-diff / forensic / "which layer added this content" queries
impossible to answer from a waybill SBOM alone.
...
- `waybill-cli/src/scan_fs/docker_image.rs`:
...
- `waybill-cli/src/scan_fs/mod.rs`:
  - New `tag_components_with_layer_digest(components, layer_path_map)` helper
    ...
    map, and stamps `waybill:layer-digest` into the component's
    `extra_annotations` bag when found. No-op when `layer_path_map` is
    `None` (non-image scans).

- `waybill-cli/src/cli/scan_cmd.rs` calls the tagger after component
...
**New C-row**: C88 `waybill:layer-digest` in `docs/reference/sbom-format-
...
### Fix: `waybill:source-files` defects on CycloneDX emission (milestone 133 US2.1)

Three defects discovered during milestone-133 US2 implementation prep on the
existing `waybill:source-files` property (pre-dates milestone 133):

- **Defect A — rootfs/tempdir prefix leak**: image scans emitted values like
  `/private/var/folders/dz/.../waybill-image-XXXXXX/rootfs/usr/bin/curl`,
...
`waybill:component-path` annotation. Ground-truth read during implementation
revealed `waybill:source-files` already exists and serves the same semantic.
...
| Overall weighted score (waybill vs syft) | 2.8 vs 2.3 = +0.5 | **3.1 vs 2.3 = +0.8** |
...
matcher (at `waybill-cli/src/scan_fs/package_db/nuget/pe_clr.rs:973`) with six new
...
`waybill sbom scan --image <registry>@sha256:<digest>` failed with
...
have a `+sha` build-metadata suffix in waybill's version that syft strips. waybill
...
waybill against syft can key on either representation — **without changing what's
...
the existing `waybill:assembly-version-informational` annotation now ALSO carries a
companion `waybill:assembly-version-informational-stripped` annotation when:
...
canonical representation" alongside a primary version. The parity-bridging `waybill:*`
...
existing `waybill:assembly-version-informational` emission site in `read()`.
...
`pkg/compare/packages.go::normVersion`). It does NOT read waybill annotations.
...
`waybill:assembly-version-informational` annotation. Projected impact based on the
...
the projected ceiling for SC-002 movement via waybill-side changes alone is from 389
...
`normVersion` to read `waybill:assembly-version-informational-stripped`. Both
...
|---|---|---|---|
| SC-001 weighted (milestone-131 was syft + 0.1) | ≥ syft + 0.4 | waybill 2.8 − syft 2.3 = **+0.5** | MET |
| SC-002 VERSION_MISMATCH | < 50 | 389 | NOT MET — US2 deferred |
| SC-003 License Coverage | ≥ 3★ | 2★ (waybill 37.9 % vs syft 3.1 %) | NOT MET — US3 deferred |
| SC-004 Supplier Attribution | ≥ 3★ | **5★** (waybill 99.9 % vs syft 2.6 %) | MET |
...
Per-dimension scorecard (waybill vs syft): Completeness 1★ vs 5★ (structural — syft's
file inventory; see §Out of Scope item 2 in milestone-132 spec), Version Accuracy 3★ vs
3★ (tied; US2 needed for >3), **License Coverage 2★ vs 1★** (waybill leads but below
...
**Bug surfaced during SC verification** (not fixed in this PR): `waybill sbom scan
...
without a tag — `docker save` emits `"RepoTags": null`, which waybill's manifest
...
**Scope deferred to follow-up PRs**: US2 (`waybill:assembly-version-informational-stripped`
...
- The full set of 3 version annotations (`waybill:assembly-version-{informational,file,runtime}`)
  emits on the component when each version was extracted. The 4-tuple `runtime` annotation always
  emits (FR-010).

**Audit-image impact**:

- **630 of 635** PE/CLR managed-assembly components now carry `waybill:assembly-version-informational`.
...
- Overall weighted scorecard unchanged at 2.6 (waybill) vs 2.5 (syft).
...
- The milestone-130 nested walker at `extract_nested_meta` reads both `dependencies` AND `licenses` from each nested entry's pom.xml. When `licenses` is non-empty, the raw `<name>` strings serialize as a JSON array under the `waybill:nested-licenses` plumbing annotation on the emitted `EmbeddedMavenMeta`.
- `jar_pom_to_entry` consumes the plumbing annotation: each name is run through `SpdxExpression::try_canonical`; successful canonicalizations populate `PackageDbEntry.licenses`. The plumbing annotation is STRIPPED from the output map (it's not a wire-format primary — the canonical CDX `licenses[].license.id` / SPDX `Package.licenseDeclared` are). Successful extraction also adds `waybill:license-source = "pom-xml"` per FR-012.
...
- `nested_jar_pom_xml_licenses_flow_through_to_nested_meta_annotations` — Spring Boot uber JAR shape with nested pom.xml carrying `<licenses><license><name>Apache-2.0</name></license></licenses>` → emitted `EmbeddedMavenMeta` carries `waybill:nested-licenses` JSON array annotation.
...
**US2a — PE/CLR LICENSE.txt fingerprint-matching** (`waybill-cli/src/scan_fs/package_db/nuget/pe_clr.rs`):
...
- `AssemblyAccumulator::flatten` emits per FR-013 / FR-015 / C97:
  - `LicenseProbeResult::Identified { spdx_id }` → `PackageDbEntry.licenses` populated via `SpdxExpression::try_canonical(spdx_id)` + `waybill:license-source = "package-dir"` annotation.
  - `LicenseProbeResult::Unrecognized { sha256_hex }` → empty `licenses[]` + `waybill:license-source = "package-dir-unrecognized"` + `waybill:license-text-sha256 = <hex>` (C97 — hex-encoded SHA-256 of the 4 KB window so downstream tools can cross-reference the same license body across packages).
  - `LicenseProbeResult::NotFound` → empty `licenses[]` + `waybill:license-source = "package-dir-no-license"` (FR-015).

**US2c — cargo-auditable registry-required signal** (`waybill-cli/src/scan_fs/binary/entry.rs::cargo_auditable_packages_to_entries`):

- For each `packages[]` entry whose `source` matches `"crates.io"`, `"crates-io"`, `"registry"`, or `"registry+https://..."`, emit `waybill:license-source = "registry-required"`. Per Constitution Principle XII this is a signal for downstream deps.dev-style enrichment — the annotation does NOT consult external sources itself; it just tells downstream tools where to look. Per FR-014.
...
**Overall sbom-comparison weighted score**: post-130 2.4 → post-131-US3 2.6 → **post-131-US2 2.6** (waybill leads syft 2.5 on weighted score). License-Coverage component of the weighted average lifted.

**New annotation keys catalogued** (C96 + C97):
- `waybill:license-source` (US2a + US2c) — values: `package-dir`, `package-dir-no-license`, `package-dir-unrecognized`, `registry-required`. Catalogued in `specs/131-quality-metadata-backfill/contracts/annotation-schema.md` with full Principle V audit.
- `waybill:license-text-sha256` (US2a only, unrecognized-license branch) — parity-bridging extension for cross-package license-body identity matching.
...
showed waybill's supplier-attribution score dropping from 4/5 to 2/5 because the three new
...
**`waybill-cli/src/scan_fs/mod.rs::external_refs_from_purl` extensions**:
...
- **maven-nested**: `pkg:maven/<g>/<a>@<v>` components carrying `waybill:source-mechanism = "maven-jar-nested"` (from milestone 130 US2) → `externalReferences[].url = "https://search.maven.org/artifact/<g>/<a>/<v>/jar"` with `type = "website"` (FR-019). Top-level JARs unchanged so the existing milestone-009 sidecar-derived URLs aren't clobbered.
- **cargo with `git+https://...` source field**: cargo-auditable's `source` field is parsed for `git+`-prefixed URLs at `binary/entry.rs::cargo_auditable_packages_to_entries`. When matched, the cleaned URL (sans `git+` prefix, sans trailing `.git`, sans `#<rev>` fragment) is stashed under the new C98 `waybill:cargo-vcs-source-url` plumbing annotation. The downstream `external_refs_from_purl` helper reads this and emits an additional `type = "vcs"` ExternalReference (FR-020). Provenance is preserved: the VCS URL came from the build-time cargo-auditable declaration, not from PURL-heuristic guessing.

**New annotation key catalogued (C98)**: `waybill:cargo-vcs-source-url` — see `specs/131-quality-metadata-backfill/contracts/annotation-schema.md` for the full Principle V audit narrative. The annotation is in-process plumbing; the native CDX `externalReferences[]` entry is the wire-format primary.
...
**New module** `waybill-cli/src/scan_fs/package_db/nuget/pe_clr.rs` (~580 LOC):
...
- `waybill:source-mechanism = "dotnet-assembly-metadata"`
- `waybill:assembly-version-runtime` carries the 4-tuple
- `waybill:assembly-cultures` carries the comma-joined sorted set of non-"neutral" cultures
  (omitted entirely when the collapsed component has only the "neutral" culture)
- `waybill:sbom-tier = "image"`
...
components; waybill now emits 819 unique (184 from `.deps.json` + 635 from PE/CLR), exceeding the
syft superset.
...
- `waybill:source-mechanism = "maven-jar-nested"` distinguishes them from top-level entries
  (which emit unchanged with no source-mechanism annotation, preserving alpha.48 byte-identity).
- `waybill:source-files` uses the JAR-URL `!`-separator convention:
...
Wolfi-based dev-tooling container image (`remediation-planner:latest`): pre-130, waybill emitted only
...
The existing milestone-029 reader at `waybill-cli/src/scan_fs/binary/cargo_auditable.rs` is correct
and functional — it parses the ZLIB-decompressed JSON payload faithfully. The bug was at the call
site in `waybill-cli/src/scan_fs/binary/mod.rs:700`, where the emission block was gated by
...
`(name, version)` cargo PURLs: 58 → 582. waybill now exceeds syft's 493 unique cargo count by 89
components (waybill's superset comes from the Cargo.lock source-tier reader catching the Rust
...
Fills the largest single ecosystem gap surfaced by a side-by-side audit against syft on a polyglot Wolfi-based dev-tooling container image (`767397973649.dkr.ecr.us-east-1.amazonaws.com/remediation-planner:latest`): pre-milestone-129, waybill emitted **zero** `pkg:nuget` PURLs on .NET-bearing images because the existing milestone-106 NuGet reader is source-tier (`.csproj`/`Directory.Packages.props`/`packages.lock.json`) and production container images ship only the compiled output. This milestone adds a `.deps.json` sidecar reader to `waybill-cli/src/scan_fs/package_db/nuget/deps_json.rs` that walks the rootfs for `*.deps.json` files (emitted by `dotnet publish` and shipped throughout the .NET SDK + runtime store layouts) and emits one `pkg:nuget/<name>@<version>` per `libraries[]` entry with `type: "package"`. On the audit image, this lifts the unique NuGet count from 0 to **184** (vs syft's 635 unique; the residual ~451 packages live in `.dll` PE/CLR metadata which a follow-up milestone will address).

**Output shape**:

- Per-component `waybill:sbom-tier = "image"` annotation (distinguishes from source-tier NuGet hits from milestone 106).
- Per-component `waybill:source-mechanism = "dotnet-deps-json"` annotation.
- Per-document `waybill:dotnet-runtime-target` annotation carrying the `.NETCoreApp,Version=v8.0`-style runtime target name when `runtimeTarget.name` is set in the `.deps.json`.
- Per-component `waybill:image-presence = "declared-not-installed"` annotation when the `.deps.json` entry declares a `path` field pointing at an assembly file that isn't present in the rootfs (best-effort probe checks the `.deps.json`'s parent dir, grandparent dir, and the `/usr/share/dotnet/` runtime-store root).
...
- Existing milestone-105 dedup pipeline collapses cross-mechanism duplicates; collisions between source-tier (.csproj) and image-tier (.deps.json) emit ONE component with both source-mechanism strings in `waybill:also-detected-via`.
...
- US2 cargo-auditable debugging — waybill's existing milestone-029 `cargo_auditable.rs` reader emits 0 components on the audit image's `/usr/bin/uv` and `/usr/bin/uvx`; the 58 `pkg:cargo` components currently emitted come from a `Cargo.lock` source-tier hit, not the `.dep-v0` ELF section. Bug investigation tracked for a follow-up.
...
Whenever any tiebreaker fires AND the auto-pick falls through past at least one detected main-module, the emitted SBOM gains a document-scope `waybill:root-selection-heuristic` annotation carrying `{"heuristic": <name>, "confidence": <float>}` (C-row C69 in `docs/reference/sbom-format-mapping.md` — full Principle V native-field audit attached). The same condition surfaces a `tracing::warn!` log at scan-end naming the picked subject AND every loser main-module's PURL, recommending the operator pass `--root-name`/`--root-purl-type` for deterministic control.
...
**Internal-only annotation kept off the wire:** the new `waybill:is-workspace-root` boolean drives the tiebreakers but is filtered out at every per-format `extra_annotations` iteration site (`generate/cyclonedx/builder.rs`, `generate/spdx/annotations.rs`, `generate/spdx/v3_annotations.rs`) so it never reaches serialized SBOM output. This is the byte-identity preservation lever — without it the 33 alpha.48 goldens would churn at every emission.
...
- Cargo / npm / pip / gem / maven / Go main-module components now carry the new `waybill:produces-binaries` (C64) annotation listing the canonical binary names the ecosystem manifest declares (milestone 116). The cross-tier `--bind-to-source` binder uses this to auto-alias image-tier `pkg:generic/<name>` components to their source-tier ecosystem PURL — operators using `waybill verify-binding` get more binding coverage out of the box.
- `waybill sbom scan --supplement-cdx <PATH>` now accepts a hand-authored CDX 1.6 supplement file declaring ground truth the scanner cannot observe (SaaS deps, vendored libraries without manifests, license / supplier / copyright metadata). When the flag is in effect, the emitted SBOM carries `waybill:source-tier = "declared"` on solo entries + a document-scope `waybill:supplement-cdx = "<path>@sha256:<hex>"` provenance annotation + per-component `waybill:assertion-conflict` annotations on collisions. Three new annotation keys (C65 / C66 / C67) with full Principle V audit narratives in `docs/reference/sbom-format-mapping.md`.
- Two new ecosystem readers: Swift Package Manager (`Package.resolved` lockfiles, `pkg:swift/<host>/<ns>/<name>@<version>` PURLs) and Kotlin DSL Gradle (`build.gradle.kts` + `libs.versions.toml` + `settings.gradle.kts`, `pkg:maven/...` PURLs). KMP multi-target source-set provenance rides the new `waybill:kmp-source-set` (C68) annotation. Off by default in the sense that scans of non-Swift / non-Kotlin trees produce identical output, but Swift / Android / KMP scans previously produced empty SBOMs and now produce real ones.
...
- Every disagreement records a `waybill:assertion-conflict` annotation as a JSON-encoded array of conflict records per the C1-committed `BTreeMap<String, serde_json::Value::Array>` storage convention.
- Document-scope `waybill:supplement-cdx = "<path>@sha256:<hex>"` provenance.
- Parse / I/O / schema failures fail closed before any walker begins per FR-002.

Phase-2 (#352) extended this with SPDX 2.3 + SPDX 3 service projection (CDX `services[]` entries surface as `packages[]` with `waybill:component-role = "saas-service"` annotation in SPDX), C68/C67 parity-catalog rows, and the document-scope `waybill:supplement-cdx` annotation on SPDX outputs. Follow-up (#353) propagates supplement-declared `licenses[]` overrides onto Cargo's `metadata.component` main-module path via a typed `Vec<SpdxExpression>` projection so the operator's declared license appears in every emission format uniformly.

Three new annotation keys with full Principle V audit narratives: C65 (`waybill:source-tier = "declared"` value extension), C66 (`waybill:supplement-cdx` envelope-level provenance), C67 (`waybill:assertion-conflict` per-component JSON-array). Hand-rolled structural validator (no `jsonschema` runtime dep).
...
Two new opt-in flags on `waybill sbom scan` that give operators producer-side control of the root component's PURL across all three output formats:

- **`--root-purl-type <TYPE>`** — overrides the type segment of the auto-derived root PURL. Today, when `--root-name` is supplied, waybill hardcodes `pkg:generic/<name>@<version>`. The new flag replaces that hardcoded `generic` with an operator-supplied type token (e.g., `--root-purl-type=golang --root-name=github.com/example/svc` produces `pkg:golang/github.com%2Fexample%2Fsvc@<version>`). Validated at parse time against the purl-spec type charset `^[a-z][a-z0-9.+-]*$`. REQUIRES `--root-name`. Mutually exclusive with `--no-root-purl`.
...
- `waybill-cli/tests/fixtures/golden/`: 33 byte-identity goldens regenerated (11 CDX + 11 SPDX 2.3 + 11 SPDX 3). Deltas are version-bump-only — the waybill-self-component `version` field bumps from alpha.47 → alpha.48 across CDX + SPDX 2.3 + SPDX 3, and the SHA-derived SPDX 3 document IDs shift accordingly per milestone 011's deterministic-ID scheme.
...
- Go components discovered only via `go.sum` fallback now carry `waybill:build-inclusion: unknown` always-on; when a `go` toolchain is on PATH, `go mod why -m -vendor` runs by default to classify those modules into `not-needed` (CDX `scope: "excluded"`) / `test-only` (lifecycle scope test) / `needed`. Opt-out via `--no-go-mod-why` or `WAYBILL_NO_GO_MOD_WHY=1` (milestone 112).
- Go test scope now propagates correctly through the test-only module closure — modules transitively reachable only from `_test.go` imports now carry `waybill:lifecycle-scope: test` plus `waybill:lifecycle-scope-derivation: test-only-closure`.
...
Milestone 110 Phase 4 surface complete: the pluggable fingerprint corpus v2 capability lands end-to-end. waybill now ships a multi-indicator corpus record schema (symbols + version strings + Build-IDs + ABI markers + ecosystem-alias PURLs + CPE candidates) AND the matcher + loader + production wiring that consumes it. Third-party corpus authors can target `docs/reference/corpus-record-v2.schema.json` today and have waybill load, fuse, and emit versioned PURLs against scanned binaries.
...
Lays the type system + public JSON Schema for v2 corpus records. New types in `waybill-cli/src/scan_fs/binary/fingerprints/`:
...
User-visible behaviour change for opt-in scans: every fingerprint-derived component now carries a new `waybill:fingerprint-confidence` annotation whose value is the numeric fused-confidence score (formatted `"X.XX"` — e.g., `"0.70"` / `"0.85"` / `"0.99"`). Distinct from the existing C16 `waybill:confidence` enum-string carrier (value=`"heuristic"`) so no collision. Spec FR-017 revised during implementation to emit numeric (lossless, matches CDX-native `evidence.identity.confidence` semantics) rather than the originally-planned bucket-name.

Public artifacts:
- `docs/reference/corpus-record-v2.schema.json` — JSON Schema Draft 2020-12 contract for third-party corpus authors.
- `waybill-cli/contracts/corpus-record-v2.schema.json` — test-local copy.
...
### Parity row C59 for `waybill:fingerprint-confidence` (#314)
...
The two loader entry points (`load_corpus_from_cache` for v1, `load_v2_records_from_cache` for v2) are independent: a caller may invoke one or both. Forward-compat for archives that adopt v2 ahead of waybill's matcher wiring.
...
- `entry::v2_match_to_entry` — `MatchResult` → `PackageDbEntry`. Uses the matcher's numeric confidence score for `waybill:fingerprint-confidence` rather than v1's hardcoded `"0.70"` baseline.
...
Three integration tests in `waybill-cli/tests/fingerprints_v2_e2e.rs` prove the v2 pipeline emits versioned PURLs in production SBOMs:
...
The dedup pipeline (`resolve::deduplicator::deduplicate`) now folds LOSER-side `extra_annotations` into the WINNER on PURL collision so the merged component carries BOTH source-tier (`waybill:source-mechanism = cmake-fetchcontent-git`) AND binary-tier (`waybill:fingerprint-corpus-sha`, `waybill:fingerprint-symbols-matched`) annotations.
...
- **US3 regression tests** prove non-opt-in scans + single-binary scans preserve milestone-108 behavior (SC-003 + SC-004): no `waybill:fingerprint-corpus-sha` annotations appear without the opt-in; the milestone-108 generic-PURL fallback fires correctly for single-binary scans.
...
### Companion demo: `kusari-sandbox/waybill-cmake-demo`
...
External symbol-fingerprint corpus (milestone 108) — the first milestone since 091 to add a NEW network call to waybill, deliberately and bounded. waybill can now identify statically-linked C libraries beyond the bundled 7 (openssl, zlib, libcurl, sqlite, pcre, pcre2, gnutls) by consulting an external corpus pinned at a SHA in the sibling repo [`kusari-sandbox/waybill-fingerprints`](https://github.com/kusari-sandbox/waybill-fingerprints). Every component identified via the external path carries a `waybill:fingerprint-corpus-sha` provenance annotation that consumers can resolve back to the exact fingerprint record. Plus a Mach-O extension to the symbol matcher that closes the macOS gap on the fingerprint path.

**Default behavior unchanged.** Operators who don't opt into the external corpus (no `--fingerprints-corpus` flag, no `WAYBILL_FINGERPRINTS_CORPUS=1` env) see zero behavioral change — bundled `FINGERPRINTS` const is still the matcher's source, no new annotations stamped, all 33 byte-identity goldens pass byte-identically. SC-003 byte-identity contract is the milestone's primary no-regression guarantor.

### Sibling repo seeded: `kusari-sandbox/waybill-fingerprints`
...
`waybill-cli/src/scan_fs/binary/fingerprints/` lands as a new sub-module:
...
- `cache.rs` — `cache_root()` resolution (`WAYBILL_FINGERPRINTS_CACHE_DIR` env > XDG > HOME), `cache_dir_for_sha`, `cache_hit`, `cache_clear(KeepRev)` per FR-009.
...
Build-time SHA pin lives at `tests/fingerprints.rev` (one line); `build.rs::emit_fingerprints_corpus_sha()` reads it, validates 40-char lowercase hex, emits `cargo:rustc-env=WAYBILL_FINGERPRINTS_CORPUS_SHA=<sha>`. No network at build time.
...
`--fingerprints-corpus` flag (also `WAYBILL_FINGERPRINTS_CORPUS=1`) opts the binary scanner into the external corpus. `fingerprints/fetch.rs` ships the GitHub-archive fetch path: 30s timeout, 5-redirect cap, retry 3x on 5xx with 1/2/4s exponential backoff, `Retry-After`-on-429 (60s cap), 404 → typed `NotFound`. Atomic-write protocol stages to `.tmp-<uuid>/corpus/` then renames to `<full-sha>/`; concurrent-writer race handled. Blocking HTTP wrapped in `std::thread::scope` to escape waybill's tokio runtime — same posture as `golang::graph_resolver`'s blocking workers.

`load_corpus(opts)` implements the FR-004 decision tree (`!external_enabled` → bundled; cache hit → `Cached`; cache miss + `!offline` → fetch → `Fetched`; cache miss + offline → bundled with warn). `symbol_fingerprint::scan_with_corpus` stamps `waybill:fingerprint-corpus-sha` (12-hex of the corpus revision OR literal `bundled` sentinel) on every emitted match. FR-013 multi-record collision: when matched-symbol sets overlap (e.g., LibreSSL + OpenSSL share `SSL_*` symbols), each match emits + carries `waybill:also-detected-via` listing the other; independent co-resident libraries (openssl + zlib, disjoint matched sets) don't trigger.
...
New reader at `waybill-cli/src/scan_fs/package_db/yocto/manifest.rs`. Walks `build/tmp/deploy/images/<machine>/*.manifest` and emits one `pkg:opkg/<name>@<version>?arch=<arch>` component per `<name> <arch> <version>` line — same PURL ecosystem as the installed-DB reader, so cross-source emissions of the same coord collapse via the milestone-105 dedup pipeline. FR-010 precedence: `OpkgInstalled` > `YoctoImageManifest`, so when both fire on the same scan the installed-DB wins and the manifest's source-mechanism appears in `waybill:also-detected-via`.

### BitBake recipe walker (#296, US4)

New reader at `waybill-cli/src/scan_fs/package_db/yocto/recipe.rs`. Walks the scan tree (max_depth=8) for `.bb` recipe files in `meta-<vendor>/recipes-*/<name>/<name>_<version>.bb` and emits one component per recipe. **Filename-only** — recipe body is NOT parsed (FR-007 explicit scope boundary). PURL: `pkg:bitbake/<name>@<version>?layer=<layer-name>` — distinct ecosystem from `pkg:opkg/` because recipes are layer declarations, not installed packages.

Per FR-008: filenames containing unexpanded `${...}` (typically shared-base recipes like `${PN}_${PV}.bb`) are silently skipped with `tracing::warn!` — no placeholder component, no `unresolved` sentinel. Filenames with no `_<version>` segment emit with `version: "unknown"` + `waybill:version-status: "missing"` annotation.
...
- `tests/cross_reader_dedup_ecosystem_107.rs` (SC-007) puts the same canonical PURL into BOTH `/var/lib/opkg/status` and a `<image>.manifest`. Asserts the emitted SBOM's surviving component carries `waybill:source-mechanism: "opkg-installed"` — proving the FR-010 precedence ladder + the `SourceMechanism` enum declaration order are wired correctly.

### Validation across the milestone

- Workspace-wide unit + integration tests: 1730+ tests pass on `./scripts/pre-pr.sh`.
- All fixture package names use synthetic `waybill-fixture-*` prefixes (lesson from milestone 106 — no CVE-advisory collisions).
...
Ecosystem coverage expansion (milestone 106). Five new lockfile readers landed across six PRs, covering every modern JS / Python / Java / .NET package manager waybill didn't previously see on source-tree scans. Filesystem-only, zero new Cargo dependencies, zero new network calls (FR-012 build-time audited).

### uv (#284, closes #276)

New reader at `waybill-cli/src/scan_fs/package_db/pip/uv_lock.rs`. Modern Python projects using `uv.lock` (TOML) now emit `pkg:pypi/<name>@<version>` components with `[[package.dependencies]]`-derived dep-graph edges. Workspace support: `[tool.uv.workspace]` member detection emits a synthetic `pkg:generic/<workspace-name>` root + `waybill:component-role: "main-module"` member components with intra-workspace edges preserved.

### Bun (#285, closes #278)

New reader at `waybill-cli/src/scan_fs/package_db/npm/bun_lock.rs`. Parses Bun's JSONC `bun.lock` format via the shared `npm/jsonc.rs` comment-stripper (also new). Emits `pkg:npm/<name>@<version>` PURLs with URL-encoded `@` on scoped names; workspace support mirrors uv's shape; `overrides` map applied at registry-emission time. The legacy binary `bun.lockb` format is out of scope.

### Gradle (#286, closes #277)

New reader at `waybill-cli/src/scan_fs/package_db/gradle/`. Handles both `gradle.lockfile` (runtime classpath) and `buildscript-gradle.lockfile` (build-script / plugin classpath) via one line-oriented parser. Emits `pkg:maven/<group>/<name>@<version>` PURLs so existing deps.dev enrichment applies without changes. Filename selects lifecycle scope: `buildscript-gradle.lockfile` → `LifecycleScope::Build` → CDX `scope: "excluded"` + SPDX `BUILD_DEPENDENCY_OF` via the existing milestone-052 emission path.

### NuGet (#287, closes #275)

New reader at `waybill-cli/src/scan_fs/package_db/nuget/` (5 files: `mod.rs` orchestration, `csproj.rs` XML parser, `directory_packages_props.rs` CPM lookup + walk-up, `private_assets.rs` LifecycleScope classifier, `packages_lock.rs` JSON lockfile parser). Walks `.csproj`/`.vbproj`/`.fsproj` files and applies a four-step version-resolution ladder: `packages.lock.json` → inline `Version=` → CPM (`Directory.Packages.props` walked up bounded by `scan_root`) → `unresolved` sentinel. `PrivateAssets="All"` / `IncludeAssets`-without-`runtime` / `ExcludeAssets=runtime` map to `LifecycleScope::Build`. Lockfile transitives that don't appear in any `.csproj` emit as `waybill:source-type: "transitive"`. Dep-graph edges from the lockfile populate `PackageDbEntry.depends`.

### Yarn (#289, closes #274)

New reader at `waybill-cli/src/scan_fs/package_db/npm/yarn_lock.rs`. Handles **both** Yarn lockfile formats — v1 (Classic, line-oriented text) and Berry (Yarn 2+, YAML-shaped) — with content-sniffed auto-detection via the `__metadata:` block sentinel. Both formats emit the same `pkg:npm/<name>@<version>` PURL shape (scoped names URL-encoded) and populate dep-graph edges from each entry's `dependencies:` map.
...
- All five fixture sets use synthetic `waybill-fixture-*` / `MikebomFixture.*` package names (lesson from PR #285's lodash flagging) — the fixtures never collide with real-world CVE advisories.
...
alpha.40's #267 walked all four standard npm dep sections — `dependencies`, `devDependencies`, `peerDependencies`, `optionalDependencies` — when building `entry.depends`. User flagged that `peerDependencies` are semantically **declarative** ("the consumer should have X installed") not **install-relational** ("this package depends on X"). The SBOM `dependsOn` / `DEPENDS_ON` slot encodes the latter; trivy and syft also skip peer-edges. waybill now matches.
...
Adds a wall-clock time limit for the entire `waybill` invocation. When exceeded, waybill emits a `tracing::error` to stderr and exits with status `124` (POSIX [`timeout(1)`](https://www.gnu.org/software/coreutils/manual/html_node/timeout-invocation.html) convention). Disabled when omitted or set to `0`.
...
Coexists with the existing `waybill trace run --timeout` (which caps the SUBPROCESS being traced, not waybill itself). Whichever fires first wins. Per-fetch internal timeouts (OCI registry pulls, deps.dev HTTP, `go mod graph` subprocess) are unaffected.
...
- **Annotation**: each merged dep's component gets a `waybill:source-manifest: <relative-path>` annotation so the manifest provenance survives the topology flattening. Graph-walking SBOM consumers see the dep is reachable from root; provenance-walking consumers can still trace it to its declaring manifest.
...
`parse_go_mod` previously fell into its "unknown directive — skip" branch for `tool` lines, leaving Go 1.24+ tool deps as orphans tagged `waybill:orphan-reason: unresolved-indirect-require`. This release adds:
...
- **Annotation**: matched components get `waybill:component-role: build-tool` (a new closed-enum value on the existing milestone-061 annotation slot — `main-module` was the only prior value). SBOM consumers can now distinguish Go build-time tool deps from regular runtime/library deps.
- **Standards-first follow-ups deferred**: CDX `Component.scope: optional`, SPDX 2.3 `BUILD_TOOL_OF` relationship type, and SPDX 3.0.1 `usesTool` relationship type would be deeper emitter changes; the waybill-namespaced annotation carries the diagnostic signal across all three formats today.
...
Real-world Go workspaces (e.g. `guacsec/guac@ebb808e`) saw 70 `// indirect` requires emitted as orphans on alpha.36 even though each was reachable per `go mod why -m` through a module waybill DID include in the SBOM. With 161 of 660 components (24%) unreachable from the document root, graph-walking SBOM consumers were missing a quarter of the dep tree. This release adds:
...
- **Annotation**: backfilled components get `waybill:orphan-reason: flat-attached-fallback` (a new closed-enum value on the milestone-061 annotation slot). The annotation's meaning widens slightly: from strictly "no incoming edge" to "incoming edge attribution unknown / synthesized." Existing `unresolved-indirect-require` continues to mean "no incoming edge AND backfill couldn't pick it up" (rare).
...
- **#248** — `mv waybill-* staging` globbed both the tarball file AND the extracted directory, requiring `staging/` to pre-exist. Trailing slash on the glob (`waybill-*/`) restricts to directories only.
...
- **Three tags per release**: `ghcr.io/kusari-sandbox/waybill:v0.1.0-alpha.X` (full git tag), `:0.1.0-alpha.X` (version without `v`), and `:latest` (moves with every alpha release until 1.0).
- **Cosign keyless signing**: every published image is signed against the GitHub OIDC issuer; consumers verify with `cosign verify --certificate-identity-regexp 'https://github.com/kusari-sandbox/waybill/.+' --certificate-oidc-issuer https://token.actions.githubusercontent.com <image>`.
...
Extends the OCI registry credential resolution at `scan_fs/oci_pull/auth.rs` so waybill can pull from private registries when running in environments without a Docker config file at the conventional `~/.docker/config.json` path — e.g. inside a Kubernetes CronJob pod where credentials arrive via `imagePullSecrets`-derived volume mounts or environment variables.

#### Added

- **`--registry-credentials-dir <PATH>` CLI flag** on `waybill sbom scan`. Probes K8s secret-mount filenames in order: `config.json` (plain Docker), `.dockerconfigjson` (K8s `kubernetes.io/dockerconfigjson` secret type), `.dockercfg` (legacy K8s `kubernetes.io/dockercfg` secret type). First readable + parseable file wins; standard Docker config shape applies.
- **Env-var credential sources**:
  - **Per-registry**: `WAYBILL_REGISTRY_<HOST>_USERNAME` + `WAYBILL_REGISTRY_<HOST>_PASSWORD`, where `<HOST>` is the registry hostname uppercased with `[^A-Z0-9]` replaced by `_` (e.g. `ghcr.io` → `WAYBILL_REGISTRY_GHCR_IO_USERNAME`; `my-ecr.amazonaws.com` → `WAYBILL_REGISTRY_MY_ECR_AMAZONAWS_COM_USERNAME`).
  - **Generic**: `WAYBILL_REGISTRY_USERNAME` + `WAYBILL_REGISTRY_PASSWORD` — applies to every registry as a catch-all (useful when a cluster scan only ever hits one registry).
...
- No behavior change for existing users. When neither `--registry-credentials-dir` nor any `WAYBILL_REGISTRY_*` env var is set, the resolver falls through to the default Docker config path with the existing precedence (`credHelpers` > `credsStore` > `auths.<reg>.auth` > `auths.<reg>.identitytoken`).
...
Every binary discovered by the file-level binary reader (`waybill-cli/src/scan_fs/binary/`) was historically emitted with CycloneDX `type: "library"` regardless of whether the file was an executable or a shared library. SBOM consumers reading the `type` field on `/bin/ls` saw `library` — the inverse of reality: `ls` is an application, not something other components link into. This milestone classifies every binary-reader-discovered component into one of four roles (`Application`, `SharedLibrary`, `Object`, `Other`) by reading the file's format header and maps the role to the format-native component-type field in each of CycloneDX, SPDX 2.3, and SPDX 3.
...
Per Constitution Principle V, all three target formats have purpose-built native fields for component typing — the role rides exclusively through those standards-native slots. **No `waybill:binary-role` annotation is introduced**; the existing `waybill:binary-class` annotation (carrying format `elf`/`macho`/`pe`) is preserved as a distinct signal.

#### Added

- **`BinaryRole` enum** in `waybill_common::resolution` with four variants (`Application` / `SharedLibrary` / `Object` / `Other`). Serde-renames as `snake_case` for stable wire-format serialization.
...
- **`waybill-cli/src/scan_fs/binary/role.rs` module** containing the four-way classifier, the ELF `PT_INTERP`-based PIE disambiguation helper, and 9 unit tests covering each role variant per format.
...
- **`transitive_parity_pip_plain` baseline bumped 0 → 13** (the synthetic 13-pkg requirements.txt fixture). FR-008's "all 3 tools agree on zero" invariant moves to a soft warning — waybill's CDX has been emitting these edges since milestone 084, and post-fix SPDX agrees with waybill's own CDX. The `cross_tool_parity_check` continues to surface the divergence-from-Trivy/Syft as informational output.
...
- **`--spdx2-relationship-compat <PROFILE>`** CLI flag on `waybill sbom scan`. Accepts `full` (default — current behavior, spec-native typed variants) or `basic` (every dep as natural-direction `DEPENDS_ON`, for downstream tooling that only implements the basic SPDX 2.3 relationship vocabulary). Only affects the `spdx-2.3-json` format; CDX and SPDX 3 emission are unaffected.
- **`waybill:lifecycle-scope` Package annotation in SPDX 2.3 output**, emitted for every non-runtime scoped Package regardless of compat mode. The annotation field name and `development` / `build` / `test` value set match the existing CDX `waybill:lifecycle-scope` property (parity-catalog row C42).
...
- **Dedicated identifier flags** on `waybill sbom scan` (path + image modes) and `waybill trace run` (FR-002):
...
- **Parity catalog row C47** registered for `waybill:identifiers` with `Directionality::SymmetricEqual`. The cross-format-parity test suite passes against the new row across CDX / SPDX 2.3 / SPDX 3 — user-defined identifiers byte-equivalent across formats after canonicalization.
- **`docs/reference/identifiers.md`** (FR-010) — published external-consumer guide. Covers wire format, CLI surface (with a migration note from the pre-073 `--with-source` draft), the 4 built-in schemes' per-format carriers, the user-defined-passthrough rule + `waybill:identifiers` envelope shape, the auto-detection algorithm (3-step git fallback + `image:` synthesis), the determinism contract, runnable `jq` decode recipes per format, a Python reference extractor, the V1 stability commitment, and a forward pointer to milestone 074. SC-006 deliverable: external auditor writes a working extractor from this doc alone.
...
- The `SpdxExternalRef` struct in `waybill-cli/src/generate/spdx/packages.rs` adds an optional `comment: Option<String>` field. Existing constructions get `comment: None` and emit identical bytes to alpha.15.
...
- **Identifier-keyed `--bind-to-source` resolution** — passing `--bind-to-source repo:git@...` and having waybill find the matching source SBOM file path via a local lookup directory. Milestone 074's scope. This milestone (073) emits identifiers in a shape 074's resolution layer can consume; no emission-side work needed at 074 land time.

## [0.1.0-alpha.15] — 2026-05-05

The **milestone 072 closure release.** Three sequential PRs (#140, #141, #142) shipped end-to-end cross-tier SBOM binding. With this release, an operator can: (a) emit image-tier SBOMs with verifiable cross-tier binding metadata; (b) verify the binary in the image matches the source SBOM via `waybill sbom verify-binding`; (c) propagate VEX statements safely from source to image, with binding-strength-aware caveats by default; (d) triage which source SBOM (if any) describes a build that produced an image-tier component via `waybill sbom trace-binding`; (e) hand external auditors `docs/reference/cross-tier-binding.md` and the binding-fixtures/ reference set so they can write their own verifier without waybill source-code access.
...
Second of three sequential PRs implementing milestone 072. PR-A delivered the foundation + verification (`waybill sbom verify-binding`). PR-B delivers **US2**: cross-tier VEX propagation that respects the binding strength PR-A established. PR-C will add operator triage (`waybill sbom trace-binding`) + the published verifier guide.

User-visible scope this PR:

- **`waybill sbom enrich --vex-propagation-mode {permissive,caveated,strict}`** (FR-007) — new flag with default `caveated`. Wires real VEX propagation (replacing the pre-072 no-op `--vex-overrides` stub). Pre-072 callers needing exact prior behavior pass `--vex-propagation-mode permissive`. The flag's help text + the `waybill sbom enrich --help` output document the breaking-change opt-out.
- **`propagate_vex_with_binding`** engine in `waybill-cli/src/sbom/mutator.rs` — handles all three modes, applies per-`Directionality` matching (one-to-one when source statement carries `Product.identifiers.cyclonedx-bom-ref` / `spdx-spdxid`; one-to-many broadcast when source is pre-072 PURL-only). Emits per-instance `affects[]` entries on the target CDX `vulnerabilities[]` array.
...
- **`waybill:vex-binding-status: unverified` caveat** (FR-009) — when `caveated` mode propagates onto a non-`verified` instance, every `affects[]` entry on the target CDX `vulnerabilities[]` row carries this sibling annotation per `contracts/openvex-instance-identifiers.md` C-5. Operators reading the SBOM see exactly which propagated statements lack verified bindings.
- **Refusal-rationale annotations** — `strict` mode refuses to propagate onto non-`verified` instances. The refused (vulnerability, instance) pairs are recorded under a new `waybill:vex-propagation-refusals` document-level property carrying a structured per-refusal record (`vulnerability`, `purl`, `bom_ref`, `binding_strength`, `reason`). The command exits non-zero per VR-006 so CI pipelines can gate on strict-mode outcomes; the SBOM is still written so operators can audit the refusal rationale.
- **`waybill-cli/tests/binding_drift.rs`** — 2 tests: strict-mode refusal on weak binding (exit non-zero, no `vulnerabilities[]` entry, refusal annotation present); strict-mode acceptance on verified binding.
- **`waybill-cli/tests/vex_per_instance.rs`** — 1 test, the canonical worked-example regression (US2 AS#4 / SC-003): two instances of `pkg:golang/golang.org/x/net@v0.28.0` (verified-bound + unbound), `caveated`-mode propagation correctly produces clean propagation on the bound instance and caveated propagation on the unbound instance.

### Changed — VEX propagation default flips from no-op to `caveated`

- Pre-072: `waybill sbom enrich --vex-overrides <path>` was a documented no-op — the legacy flag did nothing. Post-072: same flag combination triggers real propagation in `caveated` mode by default. Callers depending on the no-op behavior pass `--vex-propagation-mode permissive` to disable binding-strength-aware caveats. Strictly speaking, this is a behavior change rather than a "breaking change in output that previously existed" — pre-072 there was no propagation output at all. Documented in spec SC-008 + the `--vex-propagation-mode` flag's help text.
...
- `waybill sbom trace-binding` operator-triage subcommand (FR-006 / US3) — PR-C.
...
This is the first of three sequential PRs implementing milestone 072. PR-A delivers the **foundational binding contract + US1** (verify image's foo == source's foo). PR-B will add US2 (`waybill sbom enrich --vex-propagation-mode` + per-instance OpenVEX). PR-C will add US3 (`waybill sbom trace-binding`). User-visible scope this PR:

- **`waybill-cli/src/binding/`** — new module owning the layered binding-hash algorithm (FR-002), per-component `SourceDocumentBinding` annotation shape (FR-001), per-ecosystem source-input extraction (cargo / npm / pip / gem / maven / golang per research.md §1), and consumer-side verification logic (FR-005). Public re-exports include `compute_binding_hash`, `extract_source_inputs_for_component`, `verify_binding`, plus the data types (`BindingHashInputs`, `BindingHash`, `BindingStrength`, `SourceDocumentId`, `SourceDocumentBinding`, `VexPropagationMode`).
- **`waybill sbom scan --bind-to-source <path>`** flag (FR-011) — image-tier scans loaded with this option resolve the named source-tier SBOM and emit per-component `waybill:source-document-binding` annotations carrying the layered-hash + `BindingStrength` (verified / weak / unknown) labels. Non-`verified` components carry a structured `reason` per FR-003. `--path` scans (source-tier) warn-and-skip emission to preserve alpha.14 source-tier byte-identity.
- **`waybill sbom verify-binding`** subcommand (FR-005) — given an image-tier SBOM and a source-tier SBOM, recomputes per-component binding hashes from the source-tier inputs and reports verification pass/fail. `--format {table,json}` (default `table`); exits non-zero on any verification failure per VR-005.
- **Standards-native cross-document references** per Constitution Principle V (FR-004) — CDX `metadata.component.externalReferences[type:bom]`, SPDX 2.3 `externalDocumentRefs[]` + `BUILT_FROM` relationship, SPDX 3 `import[] ExternalMap` + `Relationship[built_from]` graph element. Every cross-tier reference rides through standards-native fields; only the per-component hash + strength label live in the `waybill:source-document-binding` annotation.
- **`OpenVexProduct.identifiers: BTreeMap<String, String>`** field added at `waybill-cli/src/generate/openvex/statements.rs:71` per contracts/openvex-instance-identifiers.md C-1. The field is `skip_serializing_if_empty` — pre-072 wire shape preserved for callers that don't populate it. PR-B will populate `cyclonedx-bom-ref` / `spdx-spdxid` keys at propagation time when both source-VEX and target-SBOM are paired.
- **Parity catalog row C46** registered for `waybill:source-document-binding` with `Directionality::SymmetricEqual` per milestone-071's invariant. The cross-format-parity test suite passes against the new row across all 9 ecosystem fixtures × 3 formats — image-tier emission is symmetric across CDX / SPDX 2.3 / SPDX 3.
...
- `waybill sbom trace-binding` operator-triage subcommand (FR-006 / US3) is **PR-C**.
...
- **Harness implication**: external conformance harnesses that shell out to `waybill sbom parity-check` to validate cross-format parity were missing real value-drift bugs pre-071. Upgrade to alpha.14 or later for the rigorous check.
...
- No new `waybill:*` annotation keys; no removed keys; no directionality changes in the parity catalog. Milestone 071 is purely an internal-verification + documentation milestone.
...
across every ecosystem waybill supports.
...
- **Multi-module reactor support (FR-002).** When the top-level
  `pom.xml` declares `<modules>`, waybill emits one main-module
...
- **Pure-Rust regex parsing.** Waybill never executes the
...
- **Editable-install merge precedence (FR-011).** When a venv
  ...
  (`waybill:sbom-tier: deployed`, hashes from METADATA) while
...
- **`dynamic = ["version"]` → `0.0.0-unknown` placeholder.**
  ...
  setuptools-scm or similar, waybill emits the literal
...
- **Cargo SBOMs now identify the project itself** via a synthetic
  ...
  application`. Carries `waybill:component-role: main-module` (C40)
...
- **Generator-side hooks generalized.** The milestone-053 CDX
  ...
  `waybill:component-role: main-module`) instead of Go-PURL-prefix-
...
- The C40 supplementary annotation continues to emit unchanged on
  Go and now extends to cargo. The `waybill:component-role:
...
- **Go transitive dependency edges work in offline + empty-cache
  ...
  waybill is the only static SBOM tool that produces the full Go
...
- **Graph-completeness transparency signal (Principle X).** The
  ...
  consumers couldn't tell "dead dep" from "waybill couldn't
  resolve." Post-061, the SBOM signals the limitation natively:
  a document-level `waybill:graph-completeness` annotation
  (`complete` / `partial`) + a `waybill:graph-completeness-reason`
  free-text summary, plus per-component `waybill:orphan-reason`
...
- **Two new CDX `metadata.properties[]` entries appear on every
  Go-touching scan**: `waybill:graph-completeness` and (when
  partial) `waybill:graph-completeness-reason`. Equivalents
...
- **New per-component property `waybill:orphan-reason`** appears
...
- **Filesystem-walker symlink-loop hang fixed.** Pre-054
  `waybill sbom scan --path <project>` hung at 100% CPU
  ...
  depth backstop to every walker in `waybill-cli/src/scan_fs/`.
...
- **Default scan now emits ALL lifecycle scopes, not just
  Runtime.** Pre-052, the default `waybill sbom scan` silently
...
- **Lifecycle-scope dependency tagging via standards-native
  fields.** The legacy `waybill:dev-dependency` annotation is
...
- **`waybill:dev-dependency` annotation** is gone everywhere —
...
- **051**: polyglot dev/test tagging — cargo + gem + maven
  ...
  `waybill:dev-dependency` (legacy, removed in 052/part-2;
  PR #96).
- **050**: `waybill:not-linked` annotation on Go source-tier
...
where `waybill sbom scan --path` on a Go project emitted only
...
- **Go source-tree scans now emit the full go.sum closure by
  ...
  deps carry the existing `waybill:dev-dependency = true`
...
`waybill:component-role` annotation surfacing
...
- **Build-tool and language-runtime components are now
  ...
  `waybill:component-role = "build-tool"` or
  `waybill:component-role = "language-runtime"`. Downstream
...
- **`waybill:component-role` annotation** (048). Components
  whose `evidence.occurrences[]` paths match a curated
  filesystem heuristic now carry a `waybill:component-role`
  ...
  application-deps reporting without waybill dropping any
...
- **SPDX-side document-level scope hint** (047). SPDX 2.3 +
  ...
  Closes the user-reported "is waybill undercounting?"
...
- **CI test-suite flake hardening** (045 + 044-followon).
  ...
  `WAYBILL_FIXED_TIMESTAMP` in subprocess-spawning helpers).
...
- **SPDX-side document-level scope hint** (047). SPDX 2.3
  ...
  `waybill:sbom-tier` annotation for finer-grained detail. SPDX
...
- **SPDX byte-identity test flake** (045). Three byte-identity
  ...
  branches. Pinned `WAYBILL_FIXED_TIMESTAMP` in the two
...
A small, focused release: makes `waybill sbom scan --image <ref>`
...
- **Docker daemon as a default image source.** When `--image
  <ref>` is an OCI reference, waybill now checks the local docker
...
already cached locally and `docker login`'d, but waybill errored
...
- **`Basic` auth challenge support for the OCI registry pull** (044
  ...
  `WWW-Authenticate` challenges. For `Basic`, waybill applies the
  ...
  on `waybill sbom scan --image <ecr-ref> --image-src remote`. The
...
- **Local docker daemon as a default image source** (044 commit 1).
  `waybill sbom scan --image <ref>` now consults the local docker
...
- **Direct OCI registry image scanning** (034 → 036):
  `waybill sbom scan --image alpine:3.19` now pulls from
...
- **Maven sidecar Debian layout** (042): in addition to
  Fedora's `/usr/share/maven-poms/`, waybill now reads
...
- **Milestone 041 — Rpm FILEDIGESTS cross-reference.** Closes
  ...
  waybill-observed `sha256` for the same file — the
  integrity-check arrow goes both ways. New
  `rpm_file_digest: Option<String>` field on
  `waybill_common::resolution::FileOccurrence` (additive,
...
- **Milestone 040 — Package-DB follow-ons (trifecta).** Three
  ...
  - **US2 (apk SHA-1 cross-ref)**: extends milestone 039's apk
    ...
    waybill-computed `sha256`. Mirrors deb's `md5` cross-ref
...
- **Milestone 038 — Per-file evidence for distroless /
  ...
  images — waybill's `file_hashes.rs` is dpkg-only. Filed as
  follow-on issue
  [#75](https://github.com/kusari-sandbox/waybill/issues/75)
...
- **Milestone 037 — distroless / chainguard / Bazel minimal-image
  dpkg coverage.** waybill now reads per-package metadata from
  `/var/lib/dpkg/status.d/<pkgname>` files in addition to the
  legacy single-file `/var/lib/dpkg/status`. Closes the
  milestone-031-surfaced gap where waybill reported 0 deb
...
- **Milestone 036 (031.z) — On-disk cache for pulled OCI image blobs.**
  ...
  The cache lives at `$WAYBILL_OCI_CACHE_DIR` →
  `$XDG_CACHE_HOME/waybill/oci-layers` →
  `$HOME/Library/Caches/waybill/oci-layers` (macOS) →
  `$HOME/.cache/waybill/oci-layers` (fallback). Default size cap
  10 GB with mtime-based LRU eviction; configurable via
  `--oci-cache-size <bytes>` or `WAYBILL_OCI_CACHE_SIZE=<bytes>`.
  Disable with `--no-oci-cache` or `WAYBILL_OCI_CACHE=0`. Every
...
- **Milestone 035 (031.y) — `--image-platform` CLI flag for cross-arch
  image scans.** New `waybill sbom scan --image <ref>
  ...
  reject with an explanation that waybill's package-DB readers are
...
- **Milestone 034 (031.x) — Authenticated OCI registry pulls.**
  `waybill sbom scan --image <ref>` now supports private registries
...
- **Milestone 030 — Mach-O codesign metadata.** Every Mach-O scan
  ...
  `waybill:macho-codesign-identifier` (e.g. `com.apple.ls` —
  universal across Apple-signed binaries),
  `waybill:macho-codesign-flags` (JSON array decoded from
...
- **Milestone 029 — cargo-auditable extraction.** Extracts the
  ...
  itself gains a `waybill:detected-cargo-auditable = true` cross-link
  annotation (Rust analog of milestone 005's `waybill:detected-go =
  ...
  `waybill:cargo-auditable-source` (non-registry sources) and
  `waybill:cargo-auditable-kind` (non-runtime kinds) preserve
...
- **Milestone 026 — curated version-string scanner expansion (easy-4
  ...
  **11 self-identifying native libraries**. Four new detectors with
  ...
  `waybill:evidence-kind = "embedded-version-string"` and
  `waybill:confidence = "heuristic"`, flowing through the existing
...
- **Milestone 028 — PE binary identity.** Every Windows-binary scan
  ...
  accessors: `waybill:pe-pdb-id` (the `<guid-hex-lowercase>:<age>`
  ...
  Linux's NT_GNU_BUILD_ID and macOS's LC_UUID), `waybill:pe-machine`
  (lowercase `IMAGE_FILE_HEADER.Machine` — `amd64` / `i386` /
  `arm64` / `armnt` / `ia64` / `riscv32` / `riscv64` / `unknown`),
  and `waybill:pe-subsystem` (lowercase
...
- **Milestone 024 — Mach-O binary identity.** Every macOS-binary
  ...
  load-command parsing: `waybill:macho-uuid` (16-byte LC_UUID
  ...
  `waybill:macho-rpath` (LC_RPATH paths in declaration order, dedup'd
  — `@executable_path` / `@loader_path` / `@rpath` recorded raw,
  runtime-context-dependent expansion deferred to consumers), and
  `waybill:macho-min-os` (`<platform>:<version>` shape — e.g.
  ...
  `waybill:macho-uuid` and a non-empty `<platform>:<version>`
  `waybill:macho-min-os` — both universal on every supported macOS
...
- **Milestone 025 — Go BuildInfo VCS metadata.** Every Go-binary scan
  ...
  `waybill:go-vcs-revision` (commit SHA from `vcs.revision`),
  `waybill:go-vcs-time` (RFC 3339 commit timestamp from `vcs.time`),
  `waybill:go-vcs-modified` (dirty-tree boolean from `vcs.modified`,
...
- **Milestone 023 — ELF binary identity + per-component generic
  ...
  on the file-level binary component: `waybill:elf-build-id`,
  `waybill:elf-runpath`, `waybill:elf-debuglink`. SC-002 is satisfied
...
- **Milestone 010 — SPDX 2.3 output + OpenVEX sidecar + SPDX 3.0.1
  ...
  all 9 supported ecosystems. A single `waybill sbom scan` invocation
  ...
  spec has one, `annotations[]` entry with a `waybill-annotation/v1`
...
- **`oci-registry` Cargo feature is now on by default.** Direct
  registry pulls (`waybill sbom scan --image alpine:3.19`) work
  out of the box on a stock `cargo install waybill` — matches
  syft / trivy UX without requiring `--features oci-registry`.
  The post-milestone-032 substrate (`oci-spec` types-only +
  workspace `reqwest 0.12` + waybill-owned thin HTTP client) is
  small enough + durable enough that the milestone-031 default-off
  framing no longer pays for itself. Users embedding waybill in a
  context that needs a minimal-deps build can opt out via
  `cargo install waybill --no-default-features`; the local
...
### Removed
- **`waybill sbom compare` subcommand** and the `demos/` directory.
...
- **Feature 009 US1 — shade-relocation ancestor emission.** When a JAR
  contains `META-INF/DEPENDENCIES`, waybill emits one nested
  `pkg:maven/...` component per declared ancestor, nested under the
  enclosing JAR's primary coord and tagged with
  `waybill:shade-relocation = true`. Ancestor licenses are parsed from
...
- **Feature 006 US5 — SBOM enrichment (`waybill sbom enrich`).**
  RFC 6902 JSON Patch applier with per-patch provenance recorded as
  `waybill:enrichment-patch[N]` properties on the BOM metadata. Replaces
  a previously stubbed bail.
- **Feature 006 US4 — in-toto policy layouts (`waybill policy init`
  and `waybill sbom verify --layout`).** Single-step functionary-keyed
...
- **Feature 006 US2 — DSSE signing + verification** via `sigstore-rs`
  ...
  Principle I). `waybill sbom verify` replaces the never-shipped `sbom
  validate` stub; exit contract: 0 pass / 1 crypto / 2 envelope /
  3 layout.
- **Feature 006 foundation — DSSE verify MVP + witness-v0.1 emission.**
  `waybill trace run` emits in-toto statements compatible with
...
- **`metadata.component` carries synthetic `purl` + `cpe`** for sbomqs
  ...
  `cpe:2.3:a:waybill:<name>:<version>:...`).
...
### Changed
- **`waybill trace` reclassified as experimental.** Primary SBOM
  surface is now `waybill sbom scan`. Trace-mode output format
...
- **Dual-identity Maven coords.** JARs at `/usr/share/java/*` owned by
  ...
  Maven coord is tagged `waybill:co-owned-by = rpm` (or equivalent);
...
- **CycloneDX 1.6 conformance pass.** `evidence.identity` is now an
  ...
  payload wasn't `tools` by the spec's definition); `waybill:
  source-connection-ids` + `waybill:deps-dev-match` now land on the
...
- **Primary-dependency fallback.** When the scanned project's root
  ...
  waybill now synthesizes edges from the primary metadata.component to
...
(`b0f31c1 feat: bootstrap waybill + milestones 001-005`) and ship the
