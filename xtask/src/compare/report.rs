...
/// Write the human-readable summary alongside the JSON.
///
/// This exists so that nobody ever has a reason to hand-write one. A report
/// composed by a person lands wherever that person's habits put it — and in
/// this repository the habit is `specs/<feature>/`, which is published. The
/// generated file can only go where the generator puts it, so the location
/// constraint holds regardless of what the content happens to contain.
pub fn write_summary(
    ...
    out.push_str("Generated by `xtask compare`. Lives under `target/`, which is\n");
    out.push_str("gitignored — this file is not publishable and must not be copied\n");
    out.push_str("into `specs/` or `docs/`.\n\n");
    out.push_str(&format!("{context}\n\n"));
    out.push_str("| tool | target | mode | median | spread | distinct | raw | identityless | accuracy |\n");
    out.push_str("|---|---|---|---|---|---|---|---|---|\n");
    for m in measurements {
        let acc = match &m.scored {
            Scored::Yes(a) => format!(
                "found {} / missed {} / extra {}{}",
                a.found, a.missed, a.extra,
                if a.truth_is_superset { " (SUPERSET)" } else { "" }
            ),
            Scored::No(n) => n.reason(),
        };
        out.push_str(&format!(
            "| {} | {} | {} | {} | {:.2} | {} | {} | {} | {} |\n",
            m.tool_id, m.target,
            if m.enriched { "enriched" } else { "offline" },
            m.median_secs.map(|v| format!("{v:.2}s")).unwrap_or_else(|| "-".into()),
            m.spread, m.distinct_packages, m.raw_components, m.identityless, acc,
        ));
    }
    out.push('\n');
    out.push_str(&render_timing_ratios(measurements));
    out.push_str("\n## Verdict\n\n");
    match verdict {
        Verdict::Comparable => out.push_str("Comparable — all preconditions met.\n"),
        Verdict::Withheld { reasons } => {
            out.push_str("**Withheld.** Figures above are measurements, not a comparison.\n\n");
            for r in reasons {
                out.push_str(&format!("- {}\n", r.describe()));
            }
        }
...
mod tests {
    ...
    /// The summary is generated, never hand-written, and the generator can
    /// only put it in the gitignored directory. This is a location
    /// constraint rather than a content check on purpose: a content check
    /// is a denylist and loses to any name nobody thought of.
    #[test]
    fn summary_can_only_be_written_under_target() {
        let tmp = std::env::temp_dir().join(format!("m780-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        let path = write_summary(&tmp, &[], &Verdict::Comparable, "ctx").expect("write");
        assert!(path.starts_with(tmp.join("target/compare")), "{path:?}");
        assert_eq!(path.file_name().and_then(|n| n.to_str()), Some("summary.md"));
        let body = std::fs::read_to_string(&path).expect("read");
        assert!(body.contains("gitignored"), "the file must say what it is");
        let _ = std::fs::remove_dir_all(&tmp);
    }
