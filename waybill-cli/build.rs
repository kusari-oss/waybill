...
fn write_version_file(override_value: Option<&str>) {
    ...
    // TEMPORARY MUTATION (#979 step 10) -- proves the corpus lane can still
    // fail. Reverted in the next commit. The version string lands in CDX
    // `metadata.tools`, SPDX 2.3 `annotations[].annotator` and SPDX 3
    // `createdBy` -- three different structures, so a format whose
    // comparison silently no-ops cannot hide behind the other two.
    let value = format!(
        "{}-CORPUS-GATE-PROBE",
        override_value
            .map(String::from)
