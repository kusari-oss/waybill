...
- **`conanfile.txt`** — declares `abseil/20240722.0` under `[requires]`. The conan reader (active since alpha.41) emits `pkg:conan/abseil@20240722.0` with `mikebom:source-mechanism: "conan-recipe"`.
- **`.gitmodules`** — declares `third_party/abseil` pointing at `https://github.com/abseil/abseil-cpp.git`. The git-submodule reader (introduced by milestone 105 US6) will emit `pkg:github/abseil/abseil-cpp@<sha>` with `mikebom:source-mechanism: "git-submodule"` and a `mikebom:build-reference` annotation.
...
- The Conan recipe wins; the git-submodule signal goes into `mikebom:also-detected-via: ["git-submodule"]`.
