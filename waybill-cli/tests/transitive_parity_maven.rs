...
//!
//! Pre-092, waybill's pom.xml parser captured the project-level
...
//!
//! Even post-092, with `$M2_REPO` empty (the CI baseline), waybill
...
/// `$M2_REPO` is empty. Waybill's maven reader needs cached parent
...
const EXPECTED_WAYBILL_EDGE_COUNT: usize = 0;
