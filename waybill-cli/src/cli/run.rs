...
pub async fn execute(args: RunArgs) -> anyhow::Result<()> {
    ...
    let auto_detected_ids =
        waybill::binding::identifiers::auto_detect::auto_detect_build_tier_identifiers(
            &invocation_cwd,
    ...
    match crate::attestation::serializer::read_attestation(&args.attestation_output) {
        Ok(statement) => {
            let subject_ids =
                waybill::binding::identifiers::auto_detect::subject_identifiers_from_attestation_subjects(
                    &statement.subject,
                );
    ...
    let mut manual_ids: Vec<waybill::binding::identifiers::Identifier> = Vec::new();
    ...
    let trace_user_metadata = waybill::binding::user_metadata::merge_file_and_flags(
        metadata_file,
        args.creator.clone(),
