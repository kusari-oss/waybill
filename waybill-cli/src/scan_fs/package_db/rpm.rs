...
fn assemble_entry(
    ...
    let encoded_name = waybill_common::types::purl::encode_purl_segment(&name);
    let encoded_version = waybill_common::types::purl::encode_purl_segment(&version);
    let encoded_release = waybill_common::types::purl::encode_purl_segment(&release);
    let purl_str = format!(
    ...
    let licenses: Vec<waybill_common::types::license::SpdxExpression> = if license_str.is_empty() {
        Vec::new()
    } else {
        waybill_common::types::license::SpdxExpression::try_canonical(&license_str)
            .ok()
            .into_iter()
            .collect()
    };
