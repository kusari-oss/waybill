...
impl MavenRepoCache {
    ...
    pub(crate) fn read_artifact_hash(
        ...
        artifact: &str,
        version: &str,
    ) -> Vec<waybill_common::types::hash::ContentHash> {
        if self.total_roots() == 0 {
            return Vec::new();
        }
        let group_path = group.replace('.', "/");
        let base = format!("{group_path}/{artifact}/{version}/{artifact}-{version}.jar");
        let mut out: Vec<waybill_common::types::hash::ContentHash> = Vec::new();
...
pub(crate) struct EmbeddedMavenMeta {
    ...
    /// [`jar_pom_to_entry`].
    pub sidecar_hash: Option<waybill_common::types::hash::ContentHash>,
    ...
    pub archive_sha256: Option<waybill_common::types::hash::ContentHash>,
...
fn compute_archive_sha256(
    ...
    waybill_common::types::hash::ContentHash::sha256(&digest).ok()
}
...
fn jar_pom_to_entry(
    ...
    depends: Vec<String>,
    source_path: &str,
    sidecar_hash: Option<waybill_common::types::hash::ContentHash>,
    archive_sha256: Option<waybill_common::types::hash::ContentHash>,
    ...
    let mut hashes: Vec<waybill_common::types::hash::ContentHash> = Vec::new();
    ...
    for entry in nested_licenses_raw {
        ...
        match waybill_common::types::license::SpdxExpression::try_canonical(raw) {
  ...
  <dependencies>
    ...
    fn read_with_claims_emits_claimed_jars_with_co_ownership_tag() {
        ...
        let has_sha256 = with_claim[0].hashes.iter().any(|h| {
            matches!(
                h.algorithm,
                waybill_common::types::hash::HashAlgorithm::Sha256
            )
        });
        assert!(
            !has_sha256,
