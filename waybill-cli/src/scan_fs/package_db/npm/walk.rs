...
fn walk_node_modules(
    ...
    for child in children {
        ...
        let license = parsed
            ...
            .and_then(|v| v.as_str())
            .and_then(|s| {
                waybill_common::types::license::SpdxExpression::try_canonical(s.trim()).ok()
            })
            .into_iter()
            .collect();
    ...
    for (section, is_dev) in [("dependencies", false), ("devDependencies", true)] {
        ...
        for name in names {
            ...
            out.push(PackageDbEntry {
                ...
                maintainer: None,
                licenses: Vec::new(),
                lifecycle_scope: if is_dev { Some(waybill_common::resolution::LifecycleScope::Development) } else { Some(waybill_common::resolution::LifecycleScope::Runtime) },
...
mod tests {
    ...
    fn root_pkgjson_fallback_include_dev_adds_devdeps() {
        ...
        assert_eq!(jest.lifecycle_scope, Some(waybill_common::resolution::LifecycleScope::Development));
    }
    ...
    fn make_purl(purl_str: &str) -> waybill_common::types::purl::Purl {
        waybill_common::types::purl::Purl::new(purl_str).unwrap()
    }
