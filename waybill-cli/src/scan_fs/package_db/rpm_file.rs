...
fn parse_rpm_file(
    ...
    let purl_str = if vendor_slug.is_empty() {
        format!(
            "pkg:rpm/{}@{}?arch={}{}{}",
            waybill_common::types::purl::encode_purl_segment(&name),
            waybill_common::types::purl::encode_purl_segment(&version_tok),
...
/// change on the rpm.rs call site.
/// Milestone 202 (FR-002, closes #579): extracted to `waybill_common::types::license::sanitize_license_operand_to_ref`
/// for CDX/SPDX 2.3 emitter parity. This local alias preserves the
/// pre-m202 call-site ergonomics via a thin re-export.
pub(crate) use waybill_common::types::license::sanitize_license_operand_to_ref as sanitize_to_license_ref_idstring;
