    ...
    if read_package_db {
        ...
        for entry in &db_entries {
            ...
            components.push(ResolvedComponent {
                ...
                requirement_ranges: entry.requirement_ranges.clone(),
...
) -> Vec<waybill_common::resolution::FileOccurrence> {
...
/// through.
fn apply_go_cache_zip_filter(components: &mut Vec<waybill_common::resolution::ResolvedComponent>) {
...
fn apply_lifecycle_scope_to_edges(
    ...
    use waybill_common::resolution::{BuildInclusion, LifecycleScope, RelationshipType};
...
fn supplier_from_purl(purl: &waybill_common::types::purl::Purl) -> Option<String> {
...
) -> Vec<waybill_common::resolution::ExternalReference> {
...
",
    ...
    fn make_component(source_path: &str) -> waybill_common::resolution::ResolvedComponent {
        use waybill_common::resolution::{
            ResolutionEvidence, ResolutionTechnique, ResolvedComponent,
        };
        let purl = waybill_common::types::purl::Purl::new("pkg:cargo/test@1.0.0").unwrap();
        ResolvedComponent {
            build_inclusion: None,
