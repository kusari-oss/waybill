...
//!
//! Per `data-model.md` Element Catalog §`Annotation`: any waybill
...
//! `waybill:<foo>` annotation for a given component, SPDX 3 emits
...
/// Build the `Annotation` elements for document-level waybill
...
/// two Annotations: `waybill:component-role = "saas-service"` (C40
/// fallback per research Decision 4) and `waybill:source-tier =
...
fn push_component_fields(
    ...
    if let Some(ref v) = c.source_type {
        push(out, "waybill:source-type", json!(v));
    }
    ...
    if let Some(ref v) = c.evidence_kind {
        push(out, "waybill:evidence-kind", json!(v));
    }
    // C5 sbom-tier
    ...
    if let Some(inclusion) = c.build_inclusion {
        push(out, "waybill:build-inclusion", json!(inclusion.as_str()));
    }
    // C7 co-owned-by
    if let Some(ref v) = c.co_owned_by {
        push(out, "waybill:co-owned-by", json!(v));
    }
    // C8 shade-relocation
    if c.shade_relocation == Some(true) {
        push(out, "waybill:shade-relocation", json!("true"));
    }
    // C9 npm-role
    if let Some(ref v) = c.npm_role {
        push(out, "waybill:npm-role", json!(v));
    }
    // C10 binary-class
    if let Some(ref v) = c.binary_class {
        push(out, "waybill:binary-class", json!(v));
    }
    // C11 binary-stripped
    ...
    if let Some(ref v) = c.linkage_kind {
        push(out, "waybill:linkage-kind", json!(v));
    }
    // C13 buildinfo-status
    if let Some(ref v) = c.buildinfo_status {
        push(out, "waybill:buildinfo-status", json!(v));
    }
    // C14 detected-go
    if c.detected_go == Some(true) {
        push(out, "waybill:detected-go", json!("true"));
    }
    // C15 binary-packed
    if let Some(ref v) = c.binary_packed {
        push(out, "waybill:binary-packed", json!(v));
    }
    // C16 confidence
    if let Some(ref v) = c.confidence {
        push(out, "waybill:confidence", json!(v));
    }
    // C17 raw-version
    if let Some(ref v) = c.raw_version {
        push(out, "waybill:raw-version", json!(v));
    }
    // C42 waybill:lifecycle-scope — DELIBERATELY OMITTED in SPDX 3.
    ...
    // primary signal and waybill MUST NOT add a redundant annotation.
    ...
    if c.cpes.len() > 1 {
        push(out, "waybill:cpe-candidates", json!(c.cpes));
    }
    // C20 requirement-ranges (milestone 199 — always-array shape).
    if !c.requirement_ranges.is_empty() {
        push(out, "waybill:requirement-ranges", json!(c.requirement_ranges));
    }
    ...
    if let Some(pipeline) = compiler_pipeline {
        ...
        if let Some(payload) = mapping.payload {
            push(out, "waybill:source-read-set", payload);
        }
        push(
            out,
...
fn push_document_fields(
    ...
    if let Some(cw) = scan.go_cache_warming {
        if !cw.failures.is_empty() {
            ...
            push(out, "waybill:go-cache-warming-failed", json!(value));
        }
    }
    ...
    // annotation invariant by construction.
    {
        ...
        if !workspaces.is_empty() {
            ...
            push(out, "waybill:workspaces-detected", json!(value));
        }
    }
    ...
    if let Some(mode) = scan.go_workspace_mode {
        ...
        if !matches!(mode, WorkspaceMode::Absent) {
            push(out, "waybill:go-workspace-mode", json!(mode.as_wire_str()));
        }
    }
    ...
    if matches!(
        ...
        push(out, "waybill:image-source", json!("podman"));
    }
    ...
    if let Some(pipeline) = scan.compiler_pipeline {
        if let Ok(value) = serde_json::to_value(&pipeline.completeness) {
            push(out, "waybill:compiler-pipeline-completeness", value);
        }
    ...
    // `waybill:purl-collisions-detected` summary. Omitted when no
    ...
    if let Some(summary) = scan.collisions_summary {
        if let Ok(value) = serde_json::to_value(summary) {
            push(out, "waybill:purl-collisions-detected", value);
        }
    }
...
mod tests {
    ...
    /// field-derived `waybill:source-files` (from
    /// `c.evidence.source_file_paths`) AND a legacy stamping in
    /// `extra_annotations["waybill:source-files"]` (the pre-145 Maven
    /// nested-JAR pattern), the SPDX 3 emitter MUST emit EXACTLY ONE
    /// `waybill:source-files` annotation — the field-derived one.
    ...
    fn spdx3_source_files_dedup_no_double_emission_md145() {
        ...
        // waybill:source-files-nested-url per Option 2b); we replicate
        ...
        assert_eq!(
            envs.len(),
            1,
            "FR-009 violation: SPDX 3 emitted {} waybill:source-files entries; \
