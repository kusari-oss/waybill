...
fn component_to_package(
    ...
    if is_main_module {
        for id in identifiers {
            if let waybill::binding::identifiers::IdentifierKind::Builtin(_) = id.kind {
                external_refs.push(SpdxExternalRef {
    ...
    let primary_package_purpose = match c.binary_role {
        Some(waybill_common::resolution::BinaryRole::Application) => {
            Some(SpdxPrimaryPackagePurpose::Application)
        }
        Some(waybill_common::resolution::BinaryRole::SharedLibrary) => {
            Some(SpdxPrimaryPackagePurpose::Library)
        }
        Some(waybill_common::resolution::BinaryRole::Object) => {
...
mod tests {
    ...
    fn mk_artifacts<'a>(
        ...
        rels: &'a [waybill_common::resolution::Relationship],
