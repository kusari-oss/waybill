...
pub fn annotate_document(
    ...
    if let Some(v) = artifacts.nixpkgs_haskell_resolution {
        push(&mut out,
    ...
    if let Some(identity) = artifacts.resolve_identity.as_ref() {
        push(&mut out, "waybill:document-resolve", json!(identity));
    }
    ...
    if let Some(summary) = artifacts.haskell_parse_summary {
        push(
            &mut out,
