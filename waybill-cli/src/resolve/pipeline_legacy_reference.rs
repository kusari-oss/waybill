//! Milestone 209 (#601) — PRE-REFACTOR REFERENCE ORACLE.
//!
//! This is a byte-for-byte snapshot of `pipeline.rs::resolve` from
//! before the m209 refactor. It serves as the SC-001 byte-identity
//! oracle for the `resolver_chain_byte_identity.rs` integration
//! test: the post-refactor chain output MUST match this legacy
//! oracle's output exactly.
//!
//! Gated `#[cfg(test)]` — never compiled into the shipped binary.
//! Scheduled for deletion in a follow-up cleanup milestone (target:
//! 2 releases post-m209 merge, once byte-identity durability has
//! been observed across multiple releases). Per research R6.
//!
//! DO NOT edit this file to fix bugs. If the post-refactor pipeline
//! diverges from this oracle, the fix belongs in the resolvers or
//! chain code — this file is frozen at the pre-refactor commit.
//!
//! Original doc-comment preserved verbatim below:
//!
...
fn collect_connection_hashes(
    conn: &waybill_common::attestation::network::Connection,
) -> Vec<ContentHash> {
    conn.response
        .as_ref()
        .and_then(|r| r.content_hash.as_ref())
...
mod tests {
    ...
    async fn empty_attestation_returns_empty() {
        ...
        use waybill_common::attestation::statement::{InTotoStatement, ResourceDescriptor};
        ...
        let attestation = InTotoStatement {
            ...
            predicate: waybill_common::attestation::statement::BuildTracePredicate {
