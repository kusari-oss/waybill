...
**Inside GitHub Actions** — no helper action, no secret, no token
handling. The `id-token: write` permission is the whole configuration:
...
The recorded identity here is the **workflow**, not a person — the
certificate subject is a reference like
`repo:org/repo:ref:refs/heads/main`, and `--certificate-identity` must
match that. Verification recipes written for an email identity will not
match a workflow-signed artifact.

Rather than composing the verify command yourself, use the one waybill
prints on success: every value in it is read off the issued certificate,
so it cannot disagree with what actually verifies.
...
INFO waybill::attestation::signer: SBOM signed via Sigstore keyless
  ...
  fulcio_cert_identity_shape=<email|workload|unrecognized>
  fulcio_cert_oidc_issuer=<the issuer the CERTIFICATE attests>
  oidc_provider=<explicit-env|github-actions-ambient>
```
