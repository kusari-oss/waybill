# Reading a waybill SBOM

**Audience**: anyone reading a waybill-emitted CycloneDX 1.6 / SPDX 2.3 / SPDX 3.0.1 document — compliance engineers, vulnerability-scanner authors, SBOM-diff tool maintainers, auditors walking output for the first time.

**What this doc is**: a consumer-facing tour of the signals waybill makes available beyond the CDX/SPDX spec baseline — what they mean, where to find them per format, what to do with them, plus a complete index of every `waybill:*` annotation key.
...
4. [The `waybill-annotation/v1` envelope](#4-the-waybill-annotationv1-envelope)
...
## 1. Positioning — how waybill uses spec-native fields

waybill strictly conforms to **CycloneDX 1.6**, **SPDX 2.3**, and **SPDX 3.0.1**. Most data in a waybill-emitted SBOM lives in spec-native fields — PURL, name, version, supplier, licenses, hashes, CPEs, dependency edges. If you've parsed CDX or SPDX before, the bulk of a waybill SBOM looks familiar.
...
- Parsing the annotation envelope as a tool author → [§4](#4-the-waybill-annotationv1-envelope) + [§7](#7-for-tool-authors)
- Looking up an unfamiliar `waybill:*` key seen in an SBOM → [Appendix A](#appendix-a--annotation-key-index)
- Mapping a waybill binary version to which signals are available → [Appendix B](#appendix-b--milestone-citation-map)
...
**C1 — Drives a consumer policy decision.** YES if a documented consumer workflow (CVE filtering, license auditing, build-provenance verification, completeness audit, supplement-conflict resolution) explicitly reads the signal to decide whether to alert, suppress, gate, or escalate. NO if the signal is purely informational (e.g., binary forensics data like Mach-O load commands) or is consumed only by waybill's own internal pipeline (e.g., dedup co-ownership evidence). Audit method: search this doc for an explicit "use it to do X" or "as a filter for Y" clause referencing the signal — if no such clause is plausibly authorable, C1 is NO.

**C2 — Cross-ecosystem reach OR ecosystem-essential.** YES if either (a) the signal is emitted by ≥2 ecosystem readers, OR (b) the signal is emitted by exactly one ecosystem AND is essential to the default consumer workflow for that ecosystem (not just an opt-in / advanced-feature flag). The "essential" exception covers cases like [`waybill:not-linked`](#waybill-not-linked) (Go-only; essential for Go CVE matching because Go's `runtime/debug.BuildInfo` is the only mechanism for proving linker DCE) and [`waybill:peer-edge-targets`](#waybill-peer-edge-targets) (npm-only; essential for npm SCA closure because peer-dep semantics are npm-unique). It does NOT cover advanced-feature / opt-in signals like `waybill:kmp-source-set` (Kotlin Multiplatform; Android-only Kotlin projects don't use it) or `waybill:shade-relocation` (Maven shade plugin; opt-in build-tool feature).

**C3 — Audit-significant.** YES if the signal affects the consumer's trust in the SBOM itself — it answers "should I trust this component's identification?" or "did waybill miss anything?" or "did the operator override scanner-derived facts?". Drives auditor / reviewer workflows. NO if the signal is runtime-decision-oriented only (e.g., `waybill:lifecycle-scope = "test"` filters CVE alerts but doesn't change the auditor's view of the SBOM's trustworthiness — it's a consumer-policy signal under C1, not an audit-trust signal under C3). A signal CAN satisfy both C1 and C3.

**C4 — Composes with another signal.** YES if the signal forms a meaningful tuple / trio with related signals consumers query together. Examples: the trust trio (`waybill:source-type` + `waybill:evidence-kind` + `waybill:confidence`); the completeness pair (`waybill:graph-completeness` + `waybill:graph-completeness-reason`); the collision pair (`waybill:duplicate-purl-divergent` + `waybill:purl-collisions-detected`); the unresolved-deps pair (`waybill:depends-unresolved` + `waybill:rdepends-unresolved`). NO if the signal is standalone — consumers query it on its own, not in combination with another `waybill:*` key.
...
1. **Score** the new `waybill:KEY` against each of C1–C5 (5 yes/no answers).
2. **Sum** the YES count.
3. **Verdict**:
...
|--------|---------|:--:|:--:|:--:|:--:|:--:|:---:|:-------:|
| `waybill:lifecycle-scope` | 3.1 | Y | Y | Y | N | Y | 4 | DEPTH ✓ |
| `waybill:layer-digest` | 3.1 | Y | Y | Y | N | Y | 4 | DEPTH ✓ |
| `waybill:duplicate-purl-divergent` + `…-purl-collisions-detected` | 3.1 | Y | Y | Y | Y (paired) | Y | 5 | DEPTH ✓ |
| `waybill:linkage-kind` (new in 151) | 3.1 | Y | Y | Y | N | Y | 4 | DEPTH ✓ |
| `waybill:not-linked` (new in 151) | 3.1 | Y | Y (Go-essential) | Y | N | Y | 4 | DEPTH ✓ |
| `waybill:license-concluded-source` | 3.2 | Y | Y | Y | N | Y | 4 | DEPTH ✓ |
| `waybill:component-tier` (file value) | 3.2 | Y | Y | N | N | Y | 3 | DEPTH ✓ |
| `waybill:demoted-from-main-module` | 3.2 | Y | Y | Y | N | Y | 4 | DEPTH ✓ |
| `waybill:source-type` | 3.3 | Y | Y | Y | Y (trust trio) | Y | 5 | DEPTH ✓ |
| `waybill:evidence-kind` (new in 151) | 3.3 | Y | Y | Y | Y (trust trio) | Y | 5 | DEPTH ✓ |
| `waybill:confidence` (new in 151) | 3.3 | Y | Y | Y | Y (trust trio) | N | 4 | DEPTH ✓ |
| `waybill:generation-context` | 3.3 | Y | Y | Y | N | Y | 4 | DEPTH ✓ |
| `waybill:source-document-binding` | 3.3 | Y | Y | Y | Y | Y | 5 | DEPTH ✓ |
| `waybill:file-inventory-mode` | 3.4 | Y | Y | Y | N | Y | 4 | DEPTH ✓ |
| `waybill:graph-completeness` + `…-reason` | 3.4 | Y | Y | Y | Y (paired) | Y | 5 | DEPTH ✓ |
| `waybill:go-transitive-fallback-count` (new in 172) | 3.4 | Y | Y (Go-essential; degraded-scan signal) | Y | Y (companion to C110) | Y | 5 | DEPTH ✓ |
| `waybill:go-cache-warming-mode` + `…-failed` (new in 173) | 3.4 | Y | Y (Go-essential; monorepo ergonomics) | Y | Y (paired) | Y | 5 | DEPTH ✓ |
| `waybill:peer-edge-targets` | 3.4 | Y | Y (npm-essential) | Y | N | Y | 4 | DEPTH ✓ |
| `waybill:depends-unresolved` + `…-rdepends-unresolved` (new in 151) | 3.4 | Y | Y (Yocto-essential; reserved key) | Y | Y (paired) | Y | 5 | DEPTH ✓ |
| `waybill:assertion-conflict` (new in 151) | 3.4 | Y | Y | Y | N | Y | 4 | DEPTH ✓ |
...
|--------|:--:|:--:|:--:|:--:|:--:|:---:|:-------:|
| `waybill:macho-build-tools` (Mach-O load-command details) | N | N | N | N | N | 0 | APPENDIX ✓ |
| `waybill:pe-machine` (PE architecture enum) | N | N | N | N | N | 0 | APPENDIX ✓ |
| `waybill:elf-build-id` (opaque hex) | N | N | N | N | N | 0 | APPENDIX ✓ |
| `waybill:yocto-layer-version-missing` (Yocto-specific transparency) | N | N | Y | N | N | 1 | APPENDIX ✓ |
| `waybill:shade-relocation` (Maven opt-in build-time feature) | N | N | N | N | Y | 1 | APPENDIX ✓ |
| `waybill:co-owned-by` (dedup-pipeline internal evidence) | N | N | N | N | N | 0 | APPENDIX ✓ |
| `waybill:also-detected-via` (dedup-pipeline alternative-source list) | N | N | N | N | Y | 1 | APPENDIX ✓ |
...
## 3. Signals waybill makes available — by use case
...
jq -r '.packages[]
       | select(.annotations[]?
                | .comment | fromjson?
                | select(.field == "waybill:lifecycle-scope" and .value == "development"))
...
#### `waybill:workspace-member` + `waybill:workspaces-detected`
...
>
> **The advisory log** — when the scan detects N > 1 workspaces AND produced ≥1 component, waybill emits exactly one INFO-level log line on stderr:
>
> ```
> monorepo shape detected: 3 workspaces (docs, src/frontend, src/lfx). Downstream consumers can filter per-workspace via `waybill:workspace-member`; see docs/reference/monorepos.md for jq recipes.
> ```
>
> The message is grep-stable: `grep -F 'monorepo shape detected: '` matches whether waybill's log formatter is plain-text or JSON. Suppressed on single-project scans (N ≤ 1) so non-monorepo scans stay quiet. NOT gated on `--offline` — the remediation (per-workspace jq slicing) is entirely consumer-side.
>
> **Where they live**:
...
jq --arg layer "sha256:abc123..." '
  .components[]
  | select(.properties[]?
           | .name == "waybill:layer-digest" and .value == $layer)
  | {purl, name, version}
' your-image.cdx.json
```

#### `waybill:duplicate-purl-divergent` + `waybill:purl-collisions-detected`

> **What they are**: when two Cargo `(name, version)` coords collide in a scan but their declared dep sets or hashes diverge, waybill flags both the per-component (C99) and document-scope (C100) signals. Without these, a vulnerability scanner could silently merge two distinct binaries under one CVE assessment — masking that the actual code differs. The per-component annotation carries a `DivergenceRecord` envelope `{v, purl, reason, paths[], dep_sets_by_path?, hashes_by_path?}` where `reason` is `deps-differ` / `hashes-differ` / `both`. The document-scope summary aggregates every detected collision in one `jq`-queryable surface.
> **Where it lives**:
...
jq '.metadata.properties[]?
    | select(.name == "waybill:purl-collisions-detected")
    | .value
    | fromjson
    | .collisions[]' your.cdx.json
```

#### `waybill:linkage-kind`
...
jq '.components[]
    | select(.properties[]?
             | .name == "waybill:linkage-kind" and .value == "static")
    | {name, version, purl}
' your.cdx.json
```
...
> **What it is**: marker on Go source-tier components (from `go.sum`) signaling that waybill's binary-vs-source comparison proved the Go linker dead-code-eliminated the module from the produced binary's `runtime/debug.BuildInfo`. Emitted only when (a) a Go binary is present in the rootfs AND (b) the binary's BuildInfo does NOT confirm the component as linked. Boolean literal `true` when present. Composes with `waybill:linkage-kind` for the full binary-tier filtering view.
>
> **Scope**: Go-only emission (milestone 050). This signal is NOT emitted on non-Go components, and a non-Go component lacking this annotation should NOT be interpreted as either present or absent in any binary. The signal applies to Go's specific build-time DCE behavior; equivalents for other ecosystems do not currently exist in waybill.
>
> **Two-state interpretation rule** (per catalog C41):
> - **Present + `true`** → waybill PROVED the module is not linked (Go BuildInfo authoritative).
> - **Absent** → either (a) the component IS linked (confirmed via BuildInfo), OR (b) no Go binary was present in the scan to compare against. Consumers MUST disambiguate by checking whether any binary-tier components exist in the SBOM — a SBOM with no `waybill:component-tier = "binary"` components is in case (b).
>
> **Where it lives**:
> - **CDX 1.6**: `components[].properties[]` entry `{name: "waybill:not-linked", value: "true"}`.
...
#### `waybill:license-concluded-source`
...
> **Where it lives**:
> - **CDX 1.6**: `components[].properties[]` entry `{name: "waybill:license-concluded-source", value: "operator-asserted"}`.
> - **SPDX 2.3**: annotation envelope on the Package.
...
jq '.components[]
    | select(.properties[]?
             | .name == "waybill:license-concluded-source")
    | {purl, licenseConcluded: (.licenses[]? | .license.id // .expression)}
' your.cdx.json
```

#### `waybill:component-tier` (specifically `"file"`)
...
> **Where it lives**:
> - **CDX 1.6**: `components[].properties[]` entry `{name: "waybill:component-tier", value: "file"}`. The component itself uses `type: "file"`, omits `purl`, and carries paths via `waybill:file-paths` (C92).
...
> **What to do with it**: when a compliance auditor walks the dep tree for license attribution, file-tier components represent content with no manifest-declared license. Treat them as "needs human review" or "license: unknown" rather than skipping. Pair with `waybill:file-paths` to see where each piece of unattributed content was found.
...
jq '.components[]
    | select(.properties[]?
             | .name == "waybill:component-tier" and .value == "file")
    | {
        name,
        sha256: (.hashes[]? | select(.alg == "SHA-256") | .content),
        paths: (.properties[]? | select(.name == "waybill:file-paths") | .value | fromjson)
      }
' your.cdx.json
```

#### `waybill:demoted-from-main-module`
...
> **Where it lives**:
> - **CDX 1.6**: `components[].properties[]` entry `{name: "waybill:demoted-from-main-module", value: "true"}`.
> - **SPDX 2.3**: annotation envelope on the Package.
...
jq '.components[]
    | select(.properties[]?
             | .name == "waybill:demoted-from-main-module" and .value == "true")
    | {purl, name, version}
' your.cdx.json
```
...
**Trust trio composition.** Three signals in this cluster compose to support threshold-based vulnerability-scanner policies: `waybill:source-type` (where the evidence came from), `waybill:evidence-kind` (how it was derived), and `waybill:confidence` (how strongly waybill backs the claim). Consumers building risk-weighting filters should consider all three together — `source-type` answers "where", `evidence-kind` answers "how", and `confidence` answers "how strongly". A worked composing jq recipe appears under [`waybill:source-type`](#waybill-source-type) below.

#### `waybill:source-type`

> **What it is**: tags each component with its discovery provenance. Common values include `"trace-observed"` (eBPF-observed during a live build trace), `"declared-not-cached"` (declared in a lockfile but waybill couldn't verify its presence on disk), `"transitive"` (added via transitive lockfile resolution from an observed component), `"package-database"` (read from a system package DB like dpkg/rpm/apk). Strong-vs-weak provenance markers.
> **Where it lives**:
...
> **Composes with**: [`waybill:evidence-kind`](#waybill-evidence-kind) (how it was derived), [`waybill:confidence`](#waybill-confidence) (how strongly).

```jq
# CDX — list components grouped by source-type provenance:
jq '[.components[]
     | {purl, source_type: (.properties[]? | select(.name == "waybill:source-type") | .value)}]
...
jq '.components[]
    | {
        ...
        evidence_kind: (.properties[]? | select(.name == "waybill:evidence-kind") | .value),
        confidence:    (.properties[]? | select(.name == "waybill:confidence")    | .value)
      }
...
> **What it is**: classification of how waybill derived this component's identity. Closed enum: `direct-observation` (waybill's eBPF / filesystem scan directly observed the evidence — e.g., a file with a matching SHA-256, a package-DB record, a `runtime/debug.BuildInfo` entry), `inference` (waybill derived the identity from a secondary signal — e.g., a lockfile entry parsed without disk-confirming the artifact, a transitive resolution that walks a dep graph), `enrichment` (waybill looked up the identity from an external source — e.g., deps.dev, PurlDB). Pairs with `waybill:source-type` and `waybill:confidence` as the trust trio.
> **Where it lives**:
> - **CDX 1.6**: `components[].properties[]` entry `{name: "waybill:evidence-kind", value: "<enum>"}`.
> - **SPDX 2.3**: annotation envelope on the Package — `{schema: "waybill-annotation/v1", field: "waybill:evidence-kind", value: "<enum>"}`.
> - **SPDX 3**: annotation envelope on the `software_Package` element.
...
> **Composes with**: [`waybill:source-type`](#waybill-source-type) (where), [`waybill:confidence`](#waybill-confidence) (how strongly).

```jq
# CDX — list components whose identity came from direct observation:
jq '.components[]
    | select(.properties[]?
             | .name == "waybill:evidence-kind" and .value == "direct-observation")
    | .purl
' your.cdx.json
...
jq -r '.packages[]
       | select(.annotations[]?
                | .comment | fromjson?
                | select(.field == "waybill:evidence-kind" and .value == "direct-observation"))
       | .name + " " + .versionInfo' your.spdx.json
...
> **What it is**: qualitative confidence label for components identified via fuzzy or heuristic matching. Closed enum — currently only the value `"heuristic"` is emitted (set on components resolved through any heuristic path; absent on components with deterministic identity such as direct package-DB reads or content-hash matches). The third member of the trust trio (with `waybill:source-type` and `waybill:evidence-kind`).
>
> **Distinct from `waybill:fingerprint-confidence`** (catalog C59, appendix-only, milestone 110): that separate annotation key carries a numeric quantitative score (`"0.70"` / `"0.85"` / `"0.99"`) specifically for milestone-108 / milestone-110 symbol-fingerprint matches on binary-tier components. Do NOT conflate the two keys — they have different value spaces, different emission gating, and live on different components. This guide depth-covers C16 (`waybill:confidence`) only; C59 stays in Appendix A pending operator demand.
>
> **Where it lives**:
> - **CDX 1.6**: `components[].properties[]` entry `{name: "waybill:confidence", value: "heuristic"}`.
> - **SPDX 2.3**: annotation envelope on the Package.
> - **SPDX 3**: annotation envelope on the `software_Package` element.
> **What to do with it**: pair with `waybill:evidence-kind` to identify components where the identity is both heuristic-derived AND from inference / enrichment — the lowest-trust bucket. A policy might alert on these only when the underlying CVE itself has a high CVSS score, or surface them for operator review before downstream automation acts.
> **Milestone**: 002 — added (foundational).
> **Catalog**: [C16](sbom-format-mapping.md)
> **Composes with**: [`waybill:source-type`](#waybill-source-type) (where), [`waybill:evidence-kind`](#waybill-evidence-kind) (how). For numeric quantitative confidence on fingerprint-matched components, see the separate [`waybill:fingerprint-confidence`](#appendix-a--annotation-key-index) annotation (catalog C59).

```jq
# CDX — list every component flagged with heuristic confidence:
jq '.components[]
    | select(.properties[]?
             | .name == "waybill:confidence" and .value == "heuristic")
    | {purl, evidence_kind: (.properties[]? | select(.name == "waybill:evidence-kind") | .value)}
' your.cdx.json
```
...
jq '.metadata.properties[]?
    | select(.name == "waybill:generation-context")
    | .value' your.cdx.json
```
...
Consumers evaluating SBOM completeness need explicit signals when waybill couldn't fully resolve the dep graph or when a non-default emission mode is in effect. waybill is "fail loud" per Constitution Principle X (Transparency) — gaps are surfaced as annotations rather than silently omitted.

#### `waybill:file-inventory-mode`
...
> - **SPDX 3**: document-scope annotation on the `SpdxDocument` element.
> **What to do with it**: when present, your dep-graph walks will see duplicated entries (the file-tier component AND the package-tier component covering the same file). Either deduplicate by content hash on your side, or filter out file-tier entries (`waybill:component-tier = "file"`) before counting components.
...
jq '.metadata.properties[]?
    | select(.name == "waybill:file-inventory-mode")
    | .value' your.cdx.json
...
jq '.metadata.properties[]?
    | select(.name == "waybill:graph-completeness" or .name == "waybill:graph-completeness-reason")
...
jq '
    .metadata.properties[]?
    | select(.name == "waybill:graph-completeness-reason")
    | .value
...
#### `waybill:go-transitive-coverage` + `waybill:go-transitive-coverage-reason`
...
jq '.metadata.properties[]?
    | select(.name == "waybill:go-transitive-coverage" or .name == "waybill:go-transitive-coverage-reason")
    | {name, value}' your.cdx.json
```

#### `waybill:go-transitive-fallback-count`

> **What it is**: document-scope non-negative integer counting Go modules whose FINAL resolution step was the `go.sum` flat fallback — the last rung of the Go transitive-resolution ladder. When this count is > 0, the graph shape for that many modules is a *flat* root→transitive edge rather than the true parent-child topology the resolver would have produced from steps 1–3. Companion to [`waybill:go-transitive-coverage`](#mikebomgo-transitive-coverage-mikebomgo-transitive-coverage-reason) (C110): C110 gives you the aggregate verdict (`complete` / `partial` / `unknown`); C117 tells you HOW MANY modules degraded — the difference between "unknown with 3 fallbacks" and "unknown with 300 fallbacks" is the difference between a minor cache miss and a fully offline scan.
>
...
> 5. **`go.sum` flat fallback** — when steps 1–3 all failed for a module (offline mode, empty cache, `GOPROXY=off`, `go mod graph` degraded), attach modules from `go.sum` as flat root→transitive edges. **Every module attached via step 5 loses its parent-child topology**: waybill knows the module *exists* and hashes to a specific version, but not who required it or who it required in turn.
> 6. **Unresolved** — the module appears somewhere in the scan but no resolution step landed a result. Zero edges emitted; the per-component `waybill:go-transitive-unresolved-reason` annotation names the class.
>
...
> - **SPDX 3**: document-scope Annotation element targeting the `SpdxDocument` root IRI.
>
> **What to do with it**: use as a CI signal to detect degraded Go scans. In a well-maintained CI environment (populated `$GOMODCACHE`, working `$GOPROXY`), the count should be `"0"` for every scan. A non-zero count means EITHER (a) the CI runner lost network / lost cache / degraded proxy access, OR (b) the target repo pins modules whose upstream is no longer resolvable. Both are actionable operator signals. Cross-reference with [`waybill:go-transitive-source`](#mikebomgo-transitive-source-mikebomgo-transitive-unresolved-reason) (C108) on individual components to enumerate WHICH modules degraded.
>
> **SC-005 invariant**: the doc-scope value equals the count of components tagged `waybill:go-transitive-source == "go-sum-fallback"`. Both derive from the same underlying `ResolutionStep::GoSumFallback` classifier. If they diverge in an SBOM, that's an emission bug — please file an issue.
>
> **Milestone**: 172 — added.
...
jq '.metadata.properties[]?
    | select(.name == "waybill:go-transitive-fallback-count")
...
jq '{
  doc_count:  (.metadata.properties[]? | select(.name == "waybill:go-transitive-fallback-count") | .value | tonumber),
  per_component_count: [.components[]?.properties[]? | select(.name == "waybill:go-transitive-source" and .value == "go-sum-fallback")] | length
} | .match = (.doc_count == .per_component_count)' your.cdx.json
```

#### `waybill:go-cache-warming-mode` + `waybill:go-cache-warming-failed`

> **What they are**: paired document-scope signals introduced in milestone 173 to close the "why did my Go graph degrade?" diagnostic loop that milestone 172's `waybill:go-transitive-fallback-count` (C117) opened. The pair captures the operator's chosen cache-warming mode + any per-workspace failures encountered while warming.
>
> **The problem being solved**: pre-m173, an operator seeing `waybill:go-transitive-fallback-count: "73"` in their SBOM knew *that* something degraded but not *what to do about it*. The m055/m091 5-step ladder needs a warm module cache OR reachable `$GOPROXY` to succeed at steps 1–3; without both, it falls through to step-5's flat go.sum fallback (loses parent-child topology). The operator-side fix is to prime the cache before scanning — via `go mod download` per workspace, or by shipping waybill with a flag that does it for them.
>
> **The m173 flag pair**:
> - `--warm-go-cache=<off|per-workspace>` — default `off`. When set to `per-workspace`, waybill invokes `go mod download` in every discovered Go workspace BEFORE the transitive resolver runs. Step 1 (`go mod graph`) then succeeds against a hot cache and emits true parent-child topology.
...
>
> **The advisory log** — when your env is degraded AND you haven't set `--warm-go-cache` explicitly, waybill emits exactly one INFO-level log line on stderr:
>
> ```
...
> ```
>
> The message is grep-stable: `grep -F 'Prime the cache with --warm-go-cache=per-workspace'` matches whether waybill's log formatter is plain-text or JSON. Suppressed when: (1) scan has no Go components, (2) `--offline` is set, (3) operator explicitly set `--warm-go-cache=<any>` (including `off`), or (4) C117 is `"0"`.
>
> **Where they live**:
...
> - **SPDX 3**: document-scope Annotation elements targeting the `SpdxDocument` root IRI.
>
> **What to do with them**: in CI, wire `waybill:go-cache-warming-failed` to your monitoring — its presence is an actionable signal that specific workspaces couldn't warm. Common causes and fixes:
...
> - `timeout` — the workspace's module closure is too large for the 60-second per-workspace budget; either use a warmer proxy, run `go mod download` outside waybill, or (future milestone) raise the timeout via a knob.
...
jq '.metadata.properties[]?
    | select(.name == "waybill:go-cache-warming-mode")
    | .value' your.cdx.json
```
...
jq '.metadata.properties[]?
    | select(.name == "waybill:go-cache-warming-failed")
    | .value
    | fromjson
...
jq '{
  coverage:         (.metadata.properties[]? | select(.name == "waybill:go-transitive-coverage") | .value // "no-go-signal"),
  fallback_count:   (.metadata.properties[]? | select(.name == "waybill:go-transitive-fallback-count") | .value // "not-emitted"),
  warming_mode:     (.metadata.properties[]? | select(.name == "waybill:go-cache-warming-mode") | .value // "not-emitted"),
  warming_failures: ([.metadata.properties[]? | select(.name == "waybill:go-cache-warming-failed") | .value | fromjson | .[]?] // [])
}' your.cdx.json
```
...
>
> **The traceability ladder** (waybill's `sbom_tier` closed enum, ascending in evidence strength):
...
>
> - **CDX 1.6**: `component.version = ""` + `evidence.identity[].confidence < 1.0` + technique `"manifest-analysis"` + `properties[]` contains `{name: "waybill:sbom-tier", value: "design"}`. Doc-scope: `metadata.lifecycles[]` contains `{"phase": "design"}` when the SBOM has ≥1 design-tier component (m047/m081 native aggregate).
> - **SPDX 2.3**: `Package.versionInfo = ""` + `annotations[]` includes the `MikebomAnnotationCommentV1` envelope with `{field: "waybill:sbom-tier", value: "design"}`. Doc-scope: `SpdxDocument.annotations[]` may carry parity-bridge annotations depending on config.
> - **SPDX 3.0.1**: `software_Package.packageVersion = ""` + typed `Annotation` element with `waybill:sbom-tier = "design"` targeting the Package IRI.
>
> **The advisory log** — when the scan detects ≥1 design-tier component AND the scan produced at least one component AND `WAYBILL_NO_DESIGN_TIER_ADVISORY` is unset, waybill emits exactly one INFO-level log line on stderr containing the stable substring `"design-tier components detected: "`. Fires under `--offline` (remediation is offline-capable). Suppressible via the env var for CI pipelines that intentionally scan constraint-only projects.
>
...
> |---|---|---|
> | pip | `uv lock` / `poetry lock` / `pip-compile requirements.in` — produces `uv.lock` / `poetry.lock` / `requirements.txt` (with pins) | `python -m venv .venv && .venv/bin/pip install -r requirements.txt` — then rescan; waybill's pip reader detects the installed venv |
...
jq '[.components[]?.properties[]? | select(.name == "waybill:sbom-tier") | .value]
    | group_by(.)
...
if [ "$DESIGN" -gt 0 ]; then
  echo "::warning::waybill found $DESIGN design-tier components; consider generating a lockfile"
fi
```

**Suppressing the advisory log in CI**: set `WAYBILL_NO_DESIGN_TIER_ADVISORY=1` (or `true`, case-insensitive) in the environment before running waybill. Matches the milestone-110 `WAYBILL_NO_DEPRECATION_NOTICE=1` env-var convention. The suppression is diagnostic-only; the emitted SBOM bytes are unchanged.

#### `waybill:peer-edge-targets`

> **What it is**: alphabetically-sorted array of PURL strings naming the peer-driven `dependsOn` edges from a given npm component. npm `peerDependencies` are install-time conventional (npm 7+ auto-installs them) but semantically declarative — different from regular `dependencies`. waybill emits peer-edges as standard `dependsOn` (matching the npm install reality) AND tags the source component with this annotation so consumers can distinguish install-driven edges from functional-dep edges. Emitted only on npm components with ≥1 resolved peer-driven edge.
> **Where it lives**:
...
> **What to do with it**: vulnerability scanners that want the install-only edge view (matching pre-milestone-147 waybill behavior) can subtract this set from each component's `dependsOn`. License auditors who care about the functional-dep distinction can flag peer-edges separately.
>
...
jq --arg purl "pkg:npm/@react-native-async-storage/async-storage@1.24.0" '
  ...
  | select(.purl == $purl)
  | .properties[]?
  | select(.name == "waybill:peer-edge-targets")
  | .value
  | fromjson
' your.cdx.json
```
...
>
> When both a `poetry.lock` (or `uv.lock`) AND a `pyproject.toml` are present in the same project root, the lockfile classification takes precedence for any component appearing in both (m183 Decision 3 lockfile-precedence). `setup.py`'s `extras_require` and `requirements.txt` are OUT OF SCOPE — no first-class optional-deps syntax exists in `requirements.txt`, and waybill does not shell out to a Python interpreter to evaluate `setup.py`.
...
> - `gradle-compile-only` — Gradle `compileOnly` deps detected via lockfile shape inference: entries appearing on any `*compileClasspath` configuration (main, `testCompileClasspath`, custom source sets like `debugCompileClasspath` for Android or `<name>CompileClasspath` for Kotlin/user-declared sets) AND absent from any `*runtimeClasspath` configuration. `buildscript-gradle.lockfile` entries with the same shape stay classified as `Build` per Decision 2 buildscript-wins. The pre-existing `waybill:gradle-configurations` annotation is PRESERVED alongside — operators can audit the raw configs list that produced the classification. — m184
...
#### `waybill:depends-unresolved` + `waybill:rdepends-unresolved`

> **What they are**: paired closure-gap markers naming components that waybill KNOWS were declared as dependencies but couldn't pin to concrete components in the scan output. `waybill:depends-unresolved` covers build-time / runtime DEPENDS-style declarations; `waybill:rdepends-unresolved` covers the runtime-only RDEPENDS variant. The VALUE is a JSON-encoded array of unresolved dep names. Per Constitution Principle X (Transparency), the SBOM emits these markers rather than silently dropping the declarations — auditors can see exactly which deps waybill failed to resolve.
>
...
jq '.components[]
    | select(.properties[]?
             | .name == "waybill:depends-unresolved")
    | {
        purl,
        depends_unresolved:  (.properties[]? | select(.name == "waybill:depends-unresolved")  | .value | fromjson),
        rdepends_unresolved: (.properties[]? | select(.name == "waybill:rdepends-unresolved") | .value | fromjson? // [])
      }
' your.cdx.json
```

#### `waybill:assertion-conflict`

> **What it is**: structured audit signal flagging components where the operator's supplement file declared a value (via `--supplement-cdx <path>`) that contradicted what waybill's scanner observed. The VALUE is a JSON-encoded array of conflict records, each shaped `{field, scanner_value, supplement_value, winner, justification}`. The `winner` is a closed enum: `scanner` (waybill kept its observation; supplement was rejected) or `supplement` (operator's declaration won; waybill's observation was overridden). The `justification` is a closed enum naming the conflict-resolution rule that fired: `bytes-evident-detection-preserved` (waybill's observation was derived from binary evidence and trumps operator-asserted metadata) or `developer-metadata-override` (the field is metadata-only and the operator's declaration wins by policy).
>
> **Where it lives**:
...
jq '.components[]
    ...
    | {
        ...
        conflicts: (.properties[]
                    | select(.name == "waybill:assertion-conflict")
                    | .value | fromjson)
      }
    | .conflicts[]
...
## 4. The `waybill-annotation/v1` envelope

SPDX 2.3 and SPDX 3 don't have a native flat-string property carrier the way CycloneDX does (CDX's `properties[].{name, value}` accepts arbitrary string-typed values directly). Instead, waybill uses a JSON envelope wrapped inside the format's native `Annotation` element. The envelope shape:

```json
{
  "schema": "waybill-annotation/v1",
  "field": "<waybill:* annotation key>",
  "value": <payload — string OR JSON value, depending on the key>
}
```

**Per-format examples** for a single signal (`waybill:lifecycle-scope` = `"development"`):

| Format | Carrier | Wire shape |
|---|---|---|
| CDX 1.6 | `components[i].properties[]` flat string | `{ "name": "waybill:lifecycle-scope", "value": "development" }` |
| SPDX 2.3 | `packages[i].annotations[].comment` envelope-as-string | `"comment": "{\"schema\":\"waybill-annotation/v1\",\"field\":\"waybill:lifecycle-scope\",\"value\":\"development\"}"` |
| SPDX 3 | `Annotation.statement` envelope-as-string | `"statement": "{\"schema\":\"waybill-annotation/v1\",\"field\":\"waybill:lifecycle-scope\",\"value\":\"development\"}"` |

The envelope schema string `waybill-annotation/v1` is the stability anchor — future envelope evolutions would bump the version (`waybill-annotation/v2`) and provide a migration window. Consumers can safely match on `schema == "waybill-annotation/v1"` to identify waybill-emitted annotations.

**Canonical Rust source references**:
- Encoder: [`waybill-cli/src/generate/spdx/annotations.rs`](../../waybill-cli/src/generate/spdx/annotations.rs) — defines `ENVELOPE_SCHEMA_V1` constant + `MikebomAnnotationCommentV1` struct (lines ~31–67).
- Decoder: [`waybill-cli/src/parity/extractors/common.rs`](../../waybill-cli/src/parity/extractors/common.rs) — verifies envelope `schema` + extracts `field` + `value` payload (line ~185).

For SPDX 3 annotations: the `statement` field carries the envelope JSON-as-string. The `subject` field points at the SPDX 3 IRI of the element the annotation applies to — for most signals this is the relevant `software_Package` element; for document-scope signals it's the `SpdxDocument` element. **SPDX 3 subject-routing quirk**: a few annotations (notably `waybill:demoted-from-main-module`) route to the synth-root IRI rather than the demoted entry's own IRI due to `package_iri_by_purl` aliasing serving milestone-084 relationship re-anchoring. The annotation VALUE remains byte-identical across formats; only the SPDX 3 SUBJECT differs. Consumers querying by annotation `field` key find the annotation regardless. See [C102](sbom-format-mapping.md) for the documented divergence.

---

## 5. Cross-format reading patterns

The same `waybill:*` signal lives in different per-format carriers. Here's a representative cross-format lookup table for 4 depth-covered signals:
...
**Common pattern**: when a signal has a spec-native field in one or more formats AND only some formats need the parity-bridge, waybill emits the native field where available + the annotation only where the native field is absent. This is Constitution Principle V in action — native first, annotation as a parity-bridge. SPDX 3's `LifecycleScopedRelationship.scope` is a representative example (SPDX 3 omits the parity-bridge annotation because the native field is sufficient there).

---

## 6. Stability

**Catalog row numbers are durable identifiers**. Every `waybill:*` annotation has a row in [`sbom-format-mapping.md`](sbom-format-mapping.md) numbered `C<N>`. The row number is the stable identifier — annotation key names and wire-shapes may evolve through documented migrations, but the row number identifies the signal's continuity across versions.

**The `waybill-annotation/v1` envelope shape is stable**. Future envelope evolutions would bump the version string (`waybill-annotation/v2`) and provide a parallel-emission migration window so consumers can support both versions during the transition.

**Opt-in / experimental flags affect which signals are emitted**:

| Flag | Effect on emission |
|---|---|
| `--exclude-scope <dev,build,test>` | Default waybill scans INCLUDE dev/build/test-scoped components (and emit `waybill:lifecycle-scope` annotations for non-runtime entries). This flag DROPS the listed scopes from `components[]` — operator-side opt-out for production-only views. |
| `--file-inventory=full` | Bypasses milestone-133 hybrid dedup; emits `waybill:file-inventory-mode = "full"` as a transparency marker. Default `orphan` mode does not emit this marker. |
| `--file-inventory=off` | Disables file-tier emission entirely. `waybill:component-tier = "file"` components do not appear. |
| `--preserve-manifest-main-module` | Enables `waybill:demoted-from-main-module` annotations when used with `--root-name` / `--root-version` / `--root-purl` overrides. |
| `--include-declared-deps` | Affects whether `declared-not-cached` `waybill:source-type` components appear. |
| `--no-deep-hash` | Skips per-file SHA-256 emission for binary-tier components. Affects whether content-hash signals are present. |
| `--conclude-licenses` | Emits `waybill:license-concluded-source = "operator-asserted"` on affected components. |

**Versioning**: waybill releases follow the `v*-alpha.*` tag sequence (currently in alpha; on `v0.1.0-alpha.52` as of milestone 149). Map binary version → signal availability via the milestone citations in [Appendix B](#appendix-b--milestone-citation-map).

**Consumer-side compatibility**: consumers parsing a waybill SBOM SHOULD treat unknown `waybill:*` annotation keys as ignorable rather than as errors. The catalog is the canonical source-of-truth for which keys exist at any given waybill version; consumers encountering an unknown key in real SBOM data should consult [Appendix A](#appendix-a--annotation-key-index) (this guide's snapshot) or the catalog directly.
...
- Match directly on the envelope `field` field (e.g., `"waybill:lifecycle-scope"`) to filter for a specific signal.

CDX 1.6 has no envelope — the value is a flat string on `properties[].value`. Same `field` semantic; different carrier.

**2. Use the catalog row numbers as the stability anchor**. Per [§6](#6-stability), `C<N>` row numbers in [`sbom-format-mapping.md`](sbom-format-mapping.md) are durable. If you depend on a specific annotation's semantic, reference its C-row number in your tool's docs so future waybill version migrations remain trackable.

**3. Suggested integration patterns**:

| Goal | Pattern |
|---|---|
| Suppress dev-only deps from production CVE alerting | Filter components by `waybill:lifecycle-scope` (CDX `properties[]` / SPDX 2.3+3 envelope) OR walk SPDX 2.3 typed `DEV_DEPENDENCY_OF` relationships OR walk SPDX 3 `LifecycleScopedRelationship.scope`. |
| Attribute findings to OCI layers | Walk `waybill:layer-digest` on every component; pair with the image manifest's `Layers[]` index to find the introducing layer. |
| Flag divergent same-PURL collisions | Walk document-scope `waybill:purl-collisions-detected` once; per-collision details on each affected component's `waybill:duplicate-purl-divergent`. |
| Recover orphan / unattributed content | Walk components with `waybill:component-tier == "file"`; treat as identity-by-SHA-256 (no PURL); pair with `waybill:file-paths` for path coverage. |
| Detect full-mode duplicate file-tier coverage | Check document-scope `waybill:file-inventory-mode == "full"`; if present, deduplicate file-tier vs package-tier components by content hash. |
| Filter npm peer-edges separately | Walk `waybill:peer-edge-targets` to extract install-driven edges; subtract from `dependsOn` for the functional-dep view. |
| Verify build-time vs lockfile-derived provenance | Walk `waybill:source-type`; weight `trace-observed` higher than `declared-not-cached` for CVE risk scoring. |
| Cross-tier source ↔ build correlation | Follow `waybill:source-document-binding` from build SBOM back to source SBOM; verify with `waybill sbom verify-binding`. |

**4. Report unexpected behavior**: if you encounter a signal that's not in the catalog, an annotation that doesn't match the documented envelope shape, or a behavior that contradicts this guide — file an issue at <https://github.com/kusari-oss/waybill/issues> with the waybill binary version + a minimal repro SBOM.
...
| [Identifiers](identifiers.md) | The four-layer identity model (`repo:` / `git:` / `image:` / `attestation:` / user-defined identifiers) — covers `waybill:identifiers`, the `--repo` / `--git-ref` / `--image` / `--attestation` / `--id` CLI surface, and per-flag identity behavior. |
...
| [Cross-tier binding](cross-tier-binding.md) | The `--bind-to-source` flag, `waybill sbom verify-binding` CLI, the binding-hash-v1 algorithm, and source ↔ build ↔ deploy correlation patterns. |
...
| [Ecosystems](../ecosystems.md) | Per-ecosystem coverage matrix for all supported ecosystems — what waybill emits for each ecosystem, source-format detection rules, transitive-dep resolution patterns. |
| [Changelog](../../CHANGELOG.md) | Milestone-by-milestone release history. Each released waybill version maps to a milestone range; cross-reference with [Appendix B](#appendix-b--milestone-citation-map) to determine signal availability per binary version. |

---

## Appendix A — Annotation key index

Snapshot at milestone-150 publication time (98 unique keys across the catalog). Future annotations land in [`sbom-format-mapping.md`](sbom-format-mapping.md) only; this index is best-effort current. When you encounter an unfamiliar `waybill:*` key not listed here, search the catalog directly — its row contains the full wire-shape contract.
...
- **`waybill:also-detected-via`** — when the deduplicator merges entries from multiple readers, lists the alternative reader sources for the surviving component. ([C56](sbom-format-mapping.md))
- **`waybill:assembly-version-informational`** — for .NET assemblies, the raw `AssemblyInformationalVersion` value extracted from the binary. ([catalog](sbom-format-mapping.md))
- **`waybill:assembly-version-informational-stripped`** — for .NET assemblies, the `AssemblyInformationalVersion` stripped of its build-metadata SemVer suffix (everything after `+`) for cross-tool comparison. ([C87](sbom-format-mapping.md))
- **`waybill:assertion-conflict`** — milestone-119 supplement-CDX merge: structured records flagging where the operator's supplement-declared values contradicted scanner observations (closed `winner` enum `scanner`/`supplement`, closed `justification` enum). See [§3.4](#34-transparency--completeness-gaps) for depth coverage. ([C67](sbom-format-mapping.md))
- **`waybill:bazel-archive-name`** — for Bazel-emitted components, the archive filename used in the BUILD/WORKSPACE rule. ([C54](sbom-format-mapping.md))
- **`waybill:bbappend-applied`** — Yocto: signals that a `.bbappend` recipe overlay was applied during build. ([C76](sbom-format-mapping.md))
- **`waybill:binary-class`** — binary-tier classification (`elf-executable` / `elf-shared-library` / `pe-executable` / `mach-o-binary` / etc.). ([C10](sbom-format-mapping.md))
- **`waybill:binary-packed`** — boolean signaling the binary file was detected as packed/compressed (UPX, similar). ([C15](sbom-format-mapping.md))
- **`waybill:binary-stripped`** — boolean signaling the binary has had its debug symbols stripped. ([C11](sbom-format-mapping.md))
- **`waybill:build-inclusion`** — milestone-112 Go build-inclusion classification (`tracked` / `not-tracked` / `unknown`). ([C60](sbom-format-mapping.md))
- **`waybill:build-inclusion-derivation`** — milestone-112 derivation reason for the build-inclusion classification above. ([C61](sbom-format-mapping.md))
- **`waybill:build-reference`** — links a build-tier component to its build-system reference (Bazel target, Maven coord, etc.). ([C57](sbom-format-mapping.md))
- **`waybill:buildinfo-status`** — Debian buildinfo-file presence + verification status (`present` / `absent` / `partial`). ([C13](sbom-format-mapping.md))
- **`waybill:co-owned-by`** — multi-source component co-ownership marker (e.g., Maven coord owned by both `groupA:artifact` and `groupB:artifact`). ([C7](sbom-format-mapping.md))
- **`waybill:component-role`** — milestone-127 role tag for root-selection + downstream filtering. Open-enum string; common values: `build-tool` / `language-runtime` / `main-module` / `workspace-root` / `saas-service`. Emitted to wire output for most values; the `main-module` value is stripped on a per-component basis when the milestone-077 root override fires with the milestone-149 drop or demote path. ([C40](sbom-format-mapping.md))
- **`waybill:component-tier`** — package-tier vs binary-tier vs file-tier classification. See [§3.2](#32-compliance-auditing) for depth coverage. ([C91](sbom-format-mapping.md))
- **`waybill:confidence`** — qualitative confidence label for components identified via heuristic matching (closed enum — currently only `"heuristic"`). For numeric quantitative confidence on fingerprint-matched components see [`waybill:fingerprint-confidence`](#appendix-a--annotation-key-index) (C59) — distinct key. See [§3.3](#33-build-provenance) for depth coverage. ([C16](sbom-format-mapping.md))
- **`waybill:cpe-candidates`** — when a component has multiple unresolved CPE candidates, the full set rides this annotation (resolved candidates ride native `externalRefs[cpe23Type]`). ([C19](sbom-format-mapping.md))
- **`waybill:demoted-from-main-module`** — milestone-149 marker for library entries preserved from the manifest-derived main-module after `--root-name` override + `--preserve-manifest-main-module` opt-in. See [§3.2](#32-compliance-auditing) for depth coverage. ([C102](sbom-format-mapping.md))
- **`waybill:depends-unresolved`** — for components with declared deps waybill couldn't resolve, lists the unresolved dep names (currently emitted only by the Yocto recipe reader; key namespace reserved for cross-ecosystem use). See [§3.4](#34-transparency--completeness-gaps) for depth coverage. ([C77](sbom-format-mapping.md))
- **`waybill:deps-dev-match`** — deps.dev enrichment match record (system + name + version triple). ([C3](sbom-format-mapping.md))
- **`waybill:detected-cargo-auditable`** — boolean signaling the binary embeds cargo-auditable JSON manifest data. ([C36](sbom-format-mapping.md))
- **`waybill:detected-go`** — Go-binary detection metadata (Go module path + version extracted from `runtime/debug.BuildInfo`). ([C14](sbom-format-mapping.md))
- **`waybill:download-url`** — recipe-declared download URL (Yocto / Bitbake `SRC_URI` variant). ([C53](sbom-format-mapping.md))
- **`waybill:duplicate-purl-divergent`** — milestone-134: per-component flag for divergent same-PURL collisions. See [§3.1](#31-vulnerability-scanning) for depth coverage. ([C99](sbom-format-mapping.md))
- **`waybill:elf-build-id`** — ELF binary's GNU build-ID hex string (from `.note.gnu.build-id` section). ([C24](sbom-format-mapping.md))
- **`waybill:elf-compiler-stamps`** — compiler-version strings extracted from ELF `.comment` / `.note.gnu` sections. ([C49](sbom-format-mapping.md))
- **`waybill:elf-debuglink`** — ELF `.gnu_debuglink` reference (path to debug-symbol companion file). ([C26](sbom-format-mapping.md))
- **`waybill:elf-runpath`** — ELF `DT_RPATH` / `DT_RUNPATH` colon-separated runpath entries. ([C25](sbom-format-mapping.md))
- **`waybill:evidence-kind`** — classification of evidence quality (`direct-observation` / `inference` / `enrichment`). See [§3.3](#33-build-provenance) for depth coverage. ([C4](sbom-format-mapping.md))
- **`waybill:exclude-path`** — milestone-113 transparency annotation listing exclude-path patterns that suppressed file emission. ([C63](sbom-format-mapping.md))
- **`waybill:file-inventory-mode`** — milestone-133 marker emitted only when `--file-inventory=full` (override mode). See [§3.4](#34-transparency--completeness-gaps) for depth coverage. ([C97](sbom-format-mapping.md))
- **`waybill:file-inventory-skipped-oversize`** — milestone-133 transparency counter for files skipped during the file-tier walk due to size cap. ([C93](sbom-format-mapping.md))
- **`waybill:file-inventory-skipped-special-files`** — milestone-133 transparency counter for special-file skips during the file-tier walk. ([C94](sbom-format-mapping.md))
- **`waybill:file-inventory-unreadable`** — milestone-133 transparency counter for unreadable-file skips during the file-tier walk. ([C95](sbom-format-mapping.md))
- **`waybill:file-paths`** — milestone-133 file-tier component's path coverage list (every rootfs-relative path where the SHA-256 content was observed). ([C92](sbom-format-mapping.md))
- **`waybill:file-paths-truncated`** — boolean signaling the `waybill:file-paths` array was truncated to the cap; some paths were dropped. ([C96](sbom-format-mapping.md))
- **`waybill:fingerprint-confidence`** — milestone-108 fingerprint-matcher confidence score (0.0–1.0). ([C59](sbom-format-mapping.md))
- **`waybill:fingerprint-corpus-sha`** — milestone-108 corpus identity SHA used for fingerprint matching (provenance). ([C58](sbom-format-mapping.md))
- **`waybill:generation-context`** — document-scope generation-mode metadata. See [§3.3](#33-build-provenance) for depth coverage. ([C21](sbom-format-mapping.md))
- **`waybill:go-vcs-modified`** — Go binary's `vcs.modified` flag from `runtime/debug.BuildInfo`. ([C29](sbom-format-mapping.md))
- **`waybill:go-vcs-revision`** — Go binary's `vcs.revision` (commit SHA) from `runtime/debug.BuildInfo`. ([C27](sbom-format-mapping.md))
- **`waybill:go-vcs-time`** — Go binary's `vcs.time` (commit timestamp) from `runtime/debug.BuildInfo`. ([C28](sbom-format-mapping.md))
- **`waybill:graph-completeness`** — milestone-158 document-scope signal for universal graph reachability (`complete` / `partial` / `unknown`). Rewritten in milestone 170 (was Go-scoped pre-m170; C44's Go-scoped emission retired, moved to `waybill:go-transitive-coverage` — see [§3.4](#34-transparency--completeness-gaps) for depth coverage). ([C104](sbom-format-mapping.md); C44 REMOVED in m170)
- **`waybill:graph-completeness-reason`** — milestone-158 companion enumerating the reason class for non-`complete` graph-completeness (e.g., `orphaned-components-detected: N component(s) not reachable from root`). ([C105](sbom-format-mapping.md))
- **`waybill:go-transitive-coverage`** — milestone-160 document-scope Go-transitive edge-resolution signal (`complete` / `partial` / `unknown`). Modern home for the Go-specific completeness question that pre-m170 lived at C44. See [§3.4](#34-transparency--completeness-gaps) for depth coverage. ([C110](sbom-format-mapping.md))
- **`waybill:go-transitive-coverage-reason`** — milestone-160 companion enumerating the reason class for non-`complete` Go-transitive coverage (e.g., `offline-mode`, `proxy-fetch-degraded`, `goproxy-off-in-chain`, `go-mod-graph-degraded`, `module-cache-empty-and-no-proxy`). ([C111](sbom-format-mapping.md))
- **`waybill:go-transitive-fallback-count`** — milestone-172 document-scope non-negative integer counting Go modules whose FINAL resolution step was the `go.sum` flat fallback (step 5 of the 5-step ladder). Companion to C110: gives the count that C110's verdict aggregates. See [§3.4](#34-transparency--completeness-gaps) for depth coverage. ([C117](sbom-format-mapping.md))
- **`waybill:go-cache-warming-mode`** — milestone-173 document-scope closed-enum string (`"off"` / `"per-workspace"` / `"offline-inhibited"`) reflecting the effective `--warm-go-cache` mode during the scan. See [§3.4](#34-transparency--completeness-gaps) for depth coverage. ([C118](sbom-format-mapping.md))
- **`waybill:go-cache-warming-failed`** — milestone-173 document-scope JSON-encoded array of per-workspace warming failure records (`{reason, workspace}`). Companion to C118. Emitted iff at least one workspace failed. ([C119](sbom-format-mapping.md))
- **`waybill:identifiers`** — milestone-073 user-defined identifier envelope (`<scheme>:<value>` records beyond the built-in `repo:` / `git:` / `image:` / `attestation:` schemes). See [identifiers.md](identifiers.md). ([C47](sbom-format-mapping.md))
- **`waybill:kmp-source-set`** — milestone-122 Kotlin Multiplatform source-set names that declared each dep. ([C68](sbom-format-mapping.md))
- **`waybill:layer-digest`** — milestone-133 OCI layer-digest attribution. See [§3.1](#31-vulnerability-scanning) for depth coverage. ([C88](sbom-format-mapping.md))
- **`waybill:license-concluded-source`** — milestone-132 provenance marker for license conclusions. See [§3.2](#32-compliance-auditing) for depth coverage. ([C98](sbom-format-mapping.md))
- **`waybill:lifecycle-scope`** — milestone-052 finer-grained dev/build/test/runtime scope distinction. See [§3.1](#31-vulnerability-scanning) for depth coverage. ([C42](sbom-format-mapping.md))
- **`waybill:lifecycle-scope-derivation`** — milestone-062 reason-class for why a particular lifecycle-scope was chosen for a component. ([C62](sbom-format-mapping.md))
- **`waybill:linkage-kind`** — binary linkage classification (closed enum: `dynamic` / `static` / `mixed`). See [§3.1](#31-vulnerability-scanning) for depth coverage. ([C12](sbom-format-mapping.md))
- **`waybill:macho-build-tools`** — Mach-O `LC_BUILD_VERSION` build-tools entries (clang/swift version + similar). ([C51](sbom-format-mapping.md))
- **`waybill:macho-build-version`** — Mach-O `LC_BUILD_VERSION` SDK + min-OS values. ([C50](sbom-format-mapping.md))
- **`waybill:macho-codesign-flags`** — Mach-O code-signature flags bitmask (hex). ([C38](sbom-format-mapping.md))
- **`waybill:macho-codesign-identifier`** — Mach-O code-signature identifier string. ([C37](sbom-format-mapping.md))
- **`waybill:macho-codesign-team-id`** — Mach-O code-signature team identifier (Apple developer team). ([C39](sbom-format-mapping.md))
- **`waybill:macho-min-os`** — Mach-O `LC_VERSION_MIN_*` minimum-OS version. ([C32](sbom-format-mapping.md))
- **`waybill:macho-rpath`** — Mach-O `LC_RPATH` runpath entries. ([C31](sbom-format-mapping.md))
- **`waybill:macho-uuid`** — Mach-O `LC_UUID` binary identifier. ([C30](sbom-format-mapping.md))
- **`waybill:not-linked`** — milestone-050 marker for Go components present in source but never linked into the binary output (two-state: present `true` = proven not-linked; absent = either confirmed-linked OR no-binary-present). See [§3.1](#31-vulnerability-scanning) for depth coverage. ([C41](sbom-format-mapping.md))
- **`waybill:npm-role`** — npm-specific component role (e.g., `workspace-root` / `peer-dep`). ([C9](sbom-format-mapping.md))
- **`waybill:orphan-reason`** — when a component appears as a graph orphan (no inbound edges), enumerates the reason class. ([C45](sbom-format-mapping.md))
- **`waybill:os-release-missing-fields`** — document-scope transparency: lists `/etc/os-release` fields missing during distro detection. ([C22](sbom-format-mapping.md))
- **`waybill:pe-linker-version`** — PE binary's linker-version major/minor (from `IMAGE_OPTIONAL_HEADER`). ([C52](sbom-format-mapping.md))
- **`waybill:pe-machine`** — PE binary's machine architecture (x86 / x64 / ARM64). ([C34](sbom-format-mapping.md))
- **`waybill:pe-pdb-id`** — PE binary's PDB GUID + age (for symbol-file correlation). ([C33](sbom-format-mapping.md))
- **`waybill:pe-subsystem`** — PE binary's subsystem value (CLI / GUI / driver). ([C35](sbom-format-mapping.md))
- **`waybill:peer-edge-targets`** — milestone-147 npm peer-edge PURL list for install-vs-functional filtering. See [§3.4](#34-transparency--completeness-gaps) for depth coverage. ([C101](sbom-format-mapping.md))
- **`waybill:produces-binaries`** — milestone-116 marker for source components that produce binary outputs (cross-tier binding helper). ([C64](sbom-format-mapping.md))
- **`waybill:purl-collisions-detected`** — milestone-134 document-scope summary of divergent same-PURL collisions. See [§3.1](#31-vulnerability-scanning) for depth coverage. ([C100](sbom-format-mapping.md))
- **`waybill:raw-version`** — pre-canonicalization raw version string when canonicalization altered the value. ([C17](sbom-format-mapping.md))
- **`waybill:rdepends-unresolved`** — for components with declared runtime-deps waybill couldn't resolve, lists the unresolved names (paired with [`waybill:depends-unresolved`](#appendix-a--annotation-key-index); same Yocto-only emission + reserved-key framing). See [§3.4](#34-transparency--completeness-gaps) for depth coverage. ([C78](sbom-format-mapping.md))
- **`waybill:requirement-range`** — the declared version-range constraint (e.g., `^1.2.0` / `>=2.0`) when known. ([C20](sbom-format-mapping.md))
- **`waybill:resolver-step`** — milestone-091 Go resolver: enumerates which resolution-ladder step produced a transitive component (e.g., `go-mod-graph` / `go-sum-fallback`). ([C48](sbom-format-mapping.md))
- **`waybill:root-selection-heuristic`** — milestone-127 document-scope: which heuristic selected the root component (per the [SBOM root-selection ladder](component-tiers.md)). ([C69](sbom-format-mapping.md))
- **`waybill:sbom-tier`** — per-component tier marker (`source` / `build` / `analyzed` / `deployed` — orthogonal to component-tier). ([C5](sbom-format-mapping.md))
- **`waybill:shade-relocation`** — Maven shade-plugin relocation rule that was applied to the vendored coord. ([C8](sbom-format-mapping.md))
- **`waybill:source-connection-ids`** — comma-separated list of eBPF source-connection identifiers that observed this component. ([C2](sbom-format-mapping.md))
- **`waybill:source-document-binding`** — milestone-072 cross-tier binding envelope. See [§3.3](#33-build-provenance) for depth coverage. ([C46](sbom-format-mapping.md))
- **`waybill:source-files`** — milestone-145 JSON-array of source file paths backing this component (canonicalized via `c.evidence.source_file_paths`). ([C18](sbom-format-mapping.md))
- **`waybill:source-mechanism`** — for cross-tier-attribution components, the mechanism used to bind source ↔ build (e.g., `cmake-fetchcontent-git` / `binary-symbol-fingerprint`). ([C55](sbom-format-mapping.md))
- **`waybill:source-tier`** — companion to `waybill:sbom-tier` identifying the source-tier when relevant. ([C65](sbom-format-mapping.md))
- **`waybill:source-type`** — discovery-provenance tag. See [§3.3](#33-build-provenance) for depth coverage. ([C1](sbom-format-mapping.md))
- **`waybill:src-uri`** — Yocto / Bitbake recipe `SRC_URI` value carrying the upstream source URL(s). ([C71](sbom-format-mapping.md))
- **`waybill:src-uri-local-only`** — boolean signaling all `SRC_URI` entries were `file://` (no external upstream). ([C82](sbom-format-mapping.md))
- **`waybill:srcrev`** — Yocto recipe `SRCREV` value (commit SHA pin for source). ([C70](sbom-format-mapping.md))
- **`waybill:srcrev-by-machine`** — Yocto multi-machine builds: per-machine `SRCREV` override map. ([C72](sbom-format-mapping.md))
- **`waybill:supplement-cdx`** — milestone-119 supplement-CDX merge marker (which CDX entries came from supplement files vs scan output). ([C66](sbom-format-mapping.md))
- **`waybill:trace-integrity-*`** — milestone-002 family of eBPF trace-integrity counters + dropped-event diagnostics. ([C23](sbom-format-mapping.md))
- **`waybill:yocto-class-extend`** — Yocto `BBCLASSEXTEND` flavor list (e.g., `["native", "nativesdk"]`). ([C83](sbom-format-mapping.md))
- **`waybill:yocto-description`** — Yocto recipe `DESCRIPTION` when distinct from `SUMMARY` (added value vs the native `description` field). ([C81](sbom-format-mapping.md))
- **`waybill:yocto-layer`** — Yocto layer name owning the recipe (provenance for multi-layer builds). ([C73](sbom-format-mapping.md))
- **`waybill:yocto-layer-series`** — Yocto layer release series (e.g., `scarthgap` / `kirkstone`). ([C75](sbom-format-mapping.md))
- **`waybill:yocto-layer-version`** — Yocto layer's `LAYERVERSION` declaration. ([C74](sbom-format-mapping.md))
- **`waybill:yocto-layer-version-missing`** — Yocto transparency marker: signals that a layer's `LAYERVERSION` declaration was absent / unparseable. ([catalog](sbom-format-mapping.md))
- **`waybill:yocto-license-closed`** — boolean signaling the Yocto recipe declared `LICENSE_FLAGS = "commercial"` (CLOSED license). ([C80](sbom-format-mapping.md))
- **`waybill:yocto-overrides-merged`** — boolean signaling ≥1 override-syntax merge fired during Yocto recipe parsing (FR-016 union-merge approximation). ([C84](sbom-format-mapping.md))
- **`waybill:yocto-recipe-name`** — Yocto recipe filename-derived name (emitted when host-typed PURL fires per FR-002a). ([C85](sbom-format-mapping.md))
- **`waybill:yocto-recipe-version`** — Yocto recipe filename-derived version (companion to recipe-name). ([C86](sbom-format-mapping.md))
- **`waybill:yocto-unexpanded-vars`** — Yocto: transparency annotation listing variables that couldn't be expanded during recipe parsing. ([C79](sbom-format-mapping.md))
- **`waybill:vendored`** — marker for components detected as vendored into the source tree (e.g., `vendor/` for Go, Cargo workspace `vendor/`). ([catalog](sbom-format-mapping.md))

---

## Appendix B — Milestone-citation map

Each depth-covered signal cites the milestone that introduced or stabilized it. Consumers comparing a waybill binary version against signal availability can use this table to determine whether a specific signal is available in their pinned binary.

| Signal | Milestone | Verb |
|---|---|---|
| `waybill:lifecycle-scope` | 052 | added (Constitution V redesign: replaces an alpha-era custom annotation with native CDX `scope` + finer-grained annotation) |
| `waybill:lifecycle-scope` | 228 (issue) | extended to SPDX 2.3 as parity-bridge so `DEPENDS_ON`-walkers can recover the dev/build/test distinction |
| `waybill:layer-digest` | 133 (US2.2) | added |
| `waybill:duplicate-purl-divergent` | 134 | added (Cargo only this milestone; ecosystem-agnostic detection logic) |
| `waybill:purl-collisions-detected` | 134 | added (document-scope companion of `waybill:duplicate-purl-divergent`) |
| `waybill:license-concluded-source` | 132 | added (issue #363) |
| `waybill:component-tier` (`file` value) | 133 (US1.B) | added; default flip to `orphan` mode in 133 (US1.C) |
| `waybill:demoted-from-main-module` | 149 | added (closes issue #151) |
| `waybill:source-type` | 002 | added |
| `waybill:source-type` | 049, 050, 052, 055 | refined / extended across multiple milestones |
| `waybill:evidence-kind` | 002-era | added (foundational discovery / enrichment infrastructure); depth coverage added in 151 |
| `waybill:confidence` | 002-era | added (foundational; carries qualitative `"heuristic"` value — distinct from milestone-110's numeric `waybill:fingerprint-confidence`); depth coverage added in 151 |
| `waybill:linkage-kind` | 005-era | added (binary tier readers landed); enum stabilized in milestone 104 (binary-role classification); depth coverage added in 151 |
| `waybill:not-linked` | 050 | added (Go binary-vs-source comparison G3 redesign); depth coverage added in 151 |
| `waybill:assertion-conflict` | 119 | added (supplement-CDX merge FR-008 / FR-009); depth coverage added in 151 |
| `waybill:depends-unresolved` + `waybill:rdepends-unresolved` | 128 | added (Yocto recipe enrichment FR-009 — currently Yocto-only emission, key namespace reserved); depth coverage added in 151 |
| `waybill:generation-context` | 002 | added |
| `waybill:generation-context` | 005, 047 | refined |
| `waybill:source-document-binding` | 072 | added |
| `waybill:file-inventory-mode` | 133 (US4) | added; codified in Constitution Strict Boundary §5 |
| `waybill:graph-completeness` | 061 | added as Go-scoped signal (closes #119); 170 rewrote as universal reachability (C104), retired C44 Go-scoped emission |
| `waybill:graph-completeness-reason` | 061 | added as Go-scoped reason (closes #119); 170 rewrote as universal reachability reason (C105) |
| `waybill:go-transitive-coverage` | 160 | added (closes #494) — modern home for Go-scoped transitive-edge coverage; replaces the pre-m170 C44 emission |
| `waybill:go-transitive-coverage-reason` | 160 | added (closes #494) — companion for C110 |
| `waybill:go-transitive-fallback-count` | 172 | added — doc-scope count of Go modules that landed on step-5 go.sum flat fallback; companion to C110 giving the numeric count under the aggregate verdict |
| `waybill:go-cache-warming-mode` | 173 | added — doc-scope closed-enum ({off, per-workspace, offline-inhibited}) surfacing the operator's chosen cache-warming mode; companion to C117 tuple `(C118, C117)` is fully self-describing |
| `waybill:go-cache-warming-failed` | 173 | added — doc-scope JSON-encoded array of per-workspace warming failure records; emitted iff at least one workspace failed warming |
| `waybill:peer-edge-targets` | 147 | added (closes Trivy-comparison orphan gap on the looker-frontend npm lockfile) |

waybill releases follow the `v*-alpha.*` tag sequence. As of milestone 149, the released version is `v0.1.0-alpha.52`. To determine signal availability for a specific binary version, cross-reference the [CHANGELOG](../../CHANGELOG.md) for the release-to-milestone mapping.
