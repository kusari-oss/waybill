...
- Q: How should `split-manifest.json` evolve to accommodate the new `members[]` field? → A: Additive optional field. `members` omitted when a group covers exactly one main-module (preserves m215 wire shape verbatim); present as `[{purl, source_dir}]` when the group covers ≥2 members. Schema URL unchanged. SC-005 byte-identity contract holds for every single-member path (m215's default) without regen. Rejected schema URL bump (v2) because the churn cost on existing consumers outweighs the code-branching-avoidance win; matches waybill's consistent additive-over-versioned annotation-shape evolution precedent (m111, m134, m176).
- Q: Which filename convention should multi-member sub-SBOMs use? → A: `<dir-slug>.multi.<format-ext>` (e.g., `services-api.multi.cdx.json`). Human-readable at `ls` time; the `.multi` marker unambiguously signals "this SBOM merges multiple main-modules — check the manifest's `members[]` for details". `<dir-slug>` derives from the canonicalized `source_dir` with `/` → `-` (matching m215's slugification of PURL names). Rejected `<combined-hash>` (opaque at `ls`) and `<dir-slug>.<hash>` (hybrid loses the "multi" signal). Single-member groups still use m215's `<slug>.<ecosystem>.<format-ext>` shape verbatim (SC-005 byte-identity).
...
- **FR-005**: The `split-manifest.json` schema MUST be extended additively: a new optional `members: [{purl, source_dir}]` field on each `SplitEntry`. When a group covers exactly ONE main-module, the field MUST be OMITTED (byte-identity with m215 wire shape). When a group covers ≥2 main-modules, the field MUST be PRESENT with one array element per contributing main-module, sorted lex by `purl` for byte-identity across scan runs. The schema URL is unchanged (still `.../split-manifest/v1.json` or equivalent). Per Q1 clarification (2026-07-23): additive-optional wins over a v2 URL bump because SC-005 byte-identity for the m215 single-member paths would otherwise require every consumer to update their parser on upgrade.
- **FR-006**: Filename convention for grouped sub-SBOMs MUST be deterministic and MUST NOT collide between neighbor groups in the same output dir. When a group covers exactly one main-module, the filename MUST match m215's `<slug>.<ecosystem>.<format-ext>` convention verbatim (SC-005 byte-identity contract). When a group covers ≥2 main-modules, the filename MUST be `<dir-slug>.multi.<format-ext>` (e.g., `services-api.multi.cdx.json`). `<dir-slug>` derives from the canonicalized `source_dir` with `/` → `-` — matching m215's slugification of PURL names. The `.multi` literal is a fixed marker signaling "merged group; consult `split-manifest.json` `members[]` for composition". Per Q2 clarification (2026-07-23).
...
- **SplitManifest (v1 additive)**: extends the m215 `SplitEntry` shape with an optional `members: [{purl, source_dir}]` field. OMITTED when a group covers exactly one main-module (m215 wire-shape compat); PRESENT (sorted lex by `purl`) when a group covers ≥2 members. Schema URL unchanged. Locked per Q1 clarification.
- **Filename slug for grouped sub-SBOMs**: deterministic function of the group's `(key, members)`. Single-member groups reuse m215's `<slug>.<ecosystem>.<format-ext>` shape verbatim. Multi-member groups use `<dir-slug>.multi.<format-ext>` where `<dir-slug>` is the canonicalized `source_dir` with `/` → `-` (per Q2 clarification).
...
- Filename convention for multi-member groups locked at Q2 clarification (see Clarifications §Session 2026-07-23): `<dir-slug>.multi.<format-ext>` — human-readable + `.multi` marker signals the merged shape. Single-member groups keep m215's `<slug>.<ecosystem>.<format-ext>` verbatim.
- Split-manifest schema evolution locked at Q1 clarification (see Clarifications §Session 2026-07-23): additive optional `members` field, omitted for single-member groups, present sorted-lex for multi-member. Schema URL unchanged. m215 wire-shape byte-identity preserved for every existing consumer.
