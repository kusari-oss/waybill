...
- [X] T034 [P] Comment on #886 and #870 with the outcome, and state in the PR body that `gradle-bitwarden-android`'s bound moved because the old number counted fabricated edges — a reviewer comparing against the old figure will otherwise read the drop as a regression
- [X] T035 Open the PR with `./scripts/pre-pr.sh` green and every per-target `N passed; 0 failed` line enumerated
