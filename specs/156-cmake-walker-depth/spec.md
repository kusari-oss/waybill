...
- **SC-001 (Kamailio testbed roster — empirically revised at implementation time)**: After milestone 156 ships, `mikebom sbom scan --path /path/to/kamailio --format cyclonedx-json` MUST produce ≥4 components carrying `mikebom:source-mechanism = "cmake-find-package"` OR `"cmake-pkg-check-modules"` (up from 1 post-milestone-155). The empirically-observed names (2026-07-02, Kamailio HEAD): `openssl` (depth-1 in `cmake/defs.cmake`), `pkgconfig` (depth-2 in one of the Find*.cmake files), `libfreeradiusclient` (depth-2 find_package inside a Find*.cmake), `radcli` (depth-2 pkg_check_modules). **Design correction at impl-time**: milestone-155's F1 remediation assumed the 10 `cmake/modules/Find<Name>.cmake` files were each a top-level `find_package(<Name>)` call site. They are NOT — they are CMake module DEFINITIONS using `find_path` / `find_library` / `find_package_handle_standard_args`. Only 2 of the 10 contain nested `find_package` calls. Milestone 156's walker-depth capability is verified independently via synthetic SC-004 fixture (depth-3 find_package emission). Verified via manual operator-cadence per quickstart.md Scenario 1.
