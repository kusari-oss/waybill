...
- The single-lockfile early-adopter orphan (`python-default.pants.lock` with empty `locked_resolves` at repo root, `//`-shape) is documented as US1 Acceptance Scenario 3. It's a no-op for component emission (0 components) but a real test case for the `//` stripper.
