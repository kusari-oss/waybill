...
Extend the milestone-223 Pants pex-lockfile reader with two additive capabilities discovered during an early-adopter (24-resolve Pants 2.33) sanity check:
...
**Scale/Scope**: Real-world Pants monorepos observed with up to 30 resolves × up to 400 pinned entries per resolve (~ 12 K components). an early adopter: 24 resolves × ~400 entries avg = ~9,838 components (SC-004 baseline).
