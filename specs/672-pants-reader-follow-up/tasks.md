...
Deliver just US1 (front-matter tolerance) as a shippable minimum. Value: recovers the Altana adopter's `python-default.pants.lock` legacy file + generic pre-2.30 Pants users. The `[python.resolves]` map support in US2 is a distinct high-value addition — bundling both in one PR is cheaper than two shippable-slice PRs because they share the fixture helpers (T008) and the mod.rs discovery loop shape (T012–T013).
