...
- **SC-001**: With the dependency-graph path disabled (`--no-deps-dev-graph`), a scan completes enrichment at least **20× faster** with the batch path than the same scan sequential. Measured baseline: **41.68s** (T001, warm, 769 enrichable components), so the target is **≤2.08s**. The graph path is excluded because this feature does not change it; including it would measure a subsystem the work never touches and cap the achievable result at ~6× regardless of how good batching is.
...
- **SC-008**: Under the same conditions as SC-001, a scan with the batch path NOT selected — the default, and the fallback — completes enrichment at least **5× faster** than the 41.68s baseline, so **≤8.34s**. Stated separately because an operator who never passes the flag must still stop experiencing the scan as a hang.
