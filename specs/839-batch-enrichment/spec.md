...
- **SC-001**: A scan of a repository with roughly 7,500 enrichable components completes enrichment at least **20× faster** than the same scan on the pre-change sequential path, where that baseline is the figure recorded by T001 rather than any number quoted in advance. Stated as a ratio because the absolute baseline is unsettled: issue #766 reports ~17 minutes, while a clean-room measurement of the request path alone extrapolates to ~7 minutes (`measurements/`), and the gap is unexplained. A ratio is verifiable against whichever baseline T001 establishes; an absolute target quoted against an unreproduced figure is not. The 20× floor is deliberately well under the ~102× measured for batch=100, so the criterion has headroom rather than encoding a best case.
...
- **SC-008**: A scan of the same repository with the batch path NOT selected — the default, and the fallback — completes enrichment at least **5× faster** than the T001 baseline. Measured concurrency alone reaches ~7.7× at the FR-003b ceiling of 8 (`measurements/`), so 5× is a floor with headroom rather than a stretch. Stated separately from SC-001 because an operator who never passes the flag must still stop experiencing the scan as a hang.
