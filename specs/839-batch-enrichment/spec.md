...
2. **Given** the same repository, **When** the operator compares wall time between the bulk path and the *sequential* per-component path as it behaved before this change, **Then** the bulk path is at least ten times faster. The baseline is named explicitly because FR-003a also speeds up the per-component path, and comparing against the improved one would understate the bulk gain while appearing to fail the criterion.
