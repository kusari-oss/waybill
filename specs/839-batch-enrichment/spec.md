...
- **SC-001**: With the dependency-graph path disabled (`--no-deps-dev-graph`), a scan completes at least **20× faster** with the batch path than the same scan sequential. Measured sequential baseline: **38.33s** (T017, warm, 709 enrichable components), so the target is **≤1.92s**. Projected ~1.0s, which is ~37× — the bar is set well below the projection so it records a real floor rather than a best case. The graph path is excluded because this feature does not change it.
...
- **SC-008**: Under the same conditions as SC-001, a scan with the batch path NOT selected — the default, and the fallback — completes at least **3.5× faster** than the sequential baseline. **Measured: 3.63×** (T017; 34.83s → 9.60s with the licence path isolated, coverage identical at 584). This criterion originally asked ≥5×, taken from a Python harness using eight independent connections. waybill uses one pooled client and reaches ~3.8×; instrumentation confirms the bound is saturated (`max_in_flight=8`), so the ceiling is upstream and not ours to raise — and FR-003b forbids raising it anyway, since tripling the bound bought only 1.22×. The bar now records what the path actually delivers.
