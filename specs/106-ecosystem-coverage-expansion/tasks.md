...
- [ ] T011 [P] [US1] Create fixture tree `mikebom-cli/tests/fixtures/golden_inputs/uv_lock/` with 4 sub-fixtures: `basic/` (3 PyPI packages, no deps), `with_dependencies/` (`[[package.dependencies]]` graph), `workspace/` (root + 2 members + 1 intra-workspace edge), `source_only/` (degenerate case for warn path).
- [ ] T012 [P] [US1] Add contract tests in `mikebom-cli/src/scan_fs/package_db/pip/uv_lock.rs` (or `tests/scan_uv_lock.rs` integration test if module-private tests get unwieldy): `emits_basic_pypi_components`, `emits_dependency_edges`, `emits_workspace_root_and_members`, `emits_intra_workspace_edge`, `warns_on_source_only_entry`.

### Implementation

- [ ] T013 [P] [US1] Create `mikebom-cli/src/scan_fs/package_db/pip/uv_lock.rs` skeleton: define `UvLockfile`, `UvPackage`, `UvSource`, `UvDepRef` structs per `data-model.md` with `#[derive(Deserialize)]` for serde-toml.
- [ ] T014 [US1] Implement `pub fn parse_lockfile(path: &Path) -> Result<UvLockfile>` using `toml::from_str` (existing workspace dep). Validate: every `[[package]]` MUST have non-empty `name`; warn-and-skip otherwise (FR-010).
- [ ] T015 [US1] Implement PURL derivation per `contracts/uv-lock.md` table: `Registry` → `pkg:pypi/<n>@<v>`; `Workspace` → same form + `component-role: "main-module"`; `Git` → `pkg:git+https://<url>@<rev>`; `Path` → `pkg:generic/<n>` + `source-type: "local"`.
- [ ] T016 [US1] Implement workspace detection: read root `pyproject.toml`'s `[tool.uv.workspace]` block; if present, gather `members` array. Workspace members in `uv.lock` are identified by `source = { workspace = true }`.
- [ ] T017 [US1] Implement workspace emission per `contracts/workspace-emission.md`: call `workspace::synthesize_workspace_root(...)` to produce the synthetic root, emit each member with `component-role: "main-module"`, build `dependsOn` edges from root to members, plus intra-workspace edges between members (via the `[[package.dependencies]]` arrays that reference workspace-source siblings).
- [ ] T018 [US1] Wire `uv_lock::read` into the pip dispatcher at `mikebom-cli/src/scan_fs/package_db/pip/mod.rs`. File-pattern trigger: `uv.lock` anywhere under the scan root.

### Polyglot + goldens

- [ ] T019 [P] [US1] Generate CDX 1.6 / SPDX 2.3 / SPDX 3.0.1 byte-identity goldens for the uv_lock fixtures: `mikebom-cli/tests/fixtures/golden/{cyclonedx,spdx-2.3,spdx-3}/uv-lock.{cdx,spdx,spdx3}.json`. Use `MIKEBOM_UPDATE_*_GOLDENS=1` to regen.
- [ ] T020 [US1] Run `./scripts/pre-pr.sh` clean. Open PR titled `feat(uv): add uv.lock reader with workspace emission (closes #276)`.
