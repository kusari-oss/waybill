...
**⚠️ Cross-phase sequencing (M2)**: Phase 4 (US2, env-var rename) MUST complete before Phase 3's T014-T016 (golden regeneration) execute. The env-var names themselves rename from `MIKEBOM_UPDATE_*_GOLDENS` → `WAYBILL_UPDATE_*_GOLDENS`; regen invocation uses the RENAMED env var, so the rename must precede the invocation. The Dependencies section at the end of this file specifies the recommended solo-dev execution order (Setup → Foundational → US2 → US3 → US1 → US4 → Polish); the phase numbering here reflects USER-STORY PRIORITY, not execution order.
...
- [ ] T030 [P] [US3] Rewrite prose in exactly this INCLUSIVE list of doc files (case-preserving `Mikebom` → `Waybill` + `mikebom` → `waybill`): `docs/architecture/**/*.md`, `docs/user-guide/**/*.md`, `docs/ecosystems.md`, `docs/design-notes.md`, `docs/index.md`, `docs/DEPENDENCIES.md`, `docs/releases.md`, `docs/reference/**/*.md`, `docs/research/**/*.md`, `docs/examples/**/*.md`. **EXPLICIT EXCLUSIONS (per M1 analyze finding)**: `docs/audits/**` (historical audit reports preserved by T031) AND `docs/migration/**` (migration guide's title + prose literally names the pre-rename form; created in US4 T033). The `rename_pass.py --pass 5` scope config (T002) must match this INCLUSIVE list — do NOT use a broad `docs/**/*.md` glob that would sweep the exclusions.
- [ ] T031 [US3] EXPLICITLY PRESERVE `docs/audits/*.md` AND `docs/migration/**` — audit reports are historical fixtures (dates like `2026-07-06-tauri-airflow.md`); the versions + tool names they cite are historical facts preserved as-authored. Migration guide's title literally names the pre-rename form (`mikebom-to-waybill.md`). NO mikebom → waybill substitution in either path. Verify: `grep -rn '\bmikebom\b' docs/audits/` still returns pre-rename hits (positive-check that preservation held).
...
- [ ] T045 CI-side verification: all 4 Lint+test lanes (linux-x86_64 default + ebpf-tracing, macOS, Windows) + Kusari Inspector + 15 rootfs/language scanners MUST pass. Any failure → diagnose per feedback_ebpf_container_test_gap memory (macOS default-features can miss Linux+ebpf-tracing bugs). Merge blocked until all 20 checks green. **FR-018 scope guard (M3 analyze finding)**: any pre-existing test that CHANGES ASSERTION BEHAVIOR post-rename (not just gets renamed identifiers) is a rename BUG — file a follow-up issue and either revert the change or narrow the rename scope. Do NOT bundle a behavioral fix inline. Similarly, any diff in emitted attestation semantics beyond the pure `mikebom → waybill` prefix-swap (per T015 golden diff-purity check) is a rename bug, not a feature bundling opportunity.
