...
- **FR-022b**: FR-022's offline guarantee MUST hold **structurally**, because the report path never reaches an enriching stage, rather than by an operator passing a flag. A guarantee that depends on a flag is one a future change can silently remove.
...
- **SC-001**: Run against this repository, `waybill-cli/tests/` is reported with a claim status reflecting that readers did match its lockfiles **and** an ambiguity record naming the competing interpretations — rather than being silently treated as dependencies, or omitted, or having the ambiguity suppressed by the claim.
...
- **SC-007**: The report is produced with no outbound network requests, verified with network access denied and with no offline flag passed — the guarantee is structural (FR-022b), so removing it would require deleting a requirement rather than changing a default.
