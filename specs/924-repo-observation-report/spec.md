...
- **SC-008**: ~~Report size stays bounded: the report for a repository an order of magnitude larger than another is not an order of magnitude larger.~~ **Amended after T039/T040 measurement.** Record count tracks *significant structure*, not directory count, so the original wording is contradicted or satisfied depending purely on which pair of repositories is chosen — this repo has 4.5x the directories of the polyglot reference and 17.8x the records, because it has 113x the marker files. A criterion that reads structural richness as a size failure fails on exactly the repositories this feature serves best. The bound that is meaningful, and measurable, is on the component that actually scales with size: **size-driven records — those earned only by exceeding the significance threshold — stay under 5% of directories walked.** Measured 1.4% (this repo, 76 of 5,400) and 2.2% (polyglot, 26 of 1,187). See `measurements/after.md` for the arithmetic behind both readings.
