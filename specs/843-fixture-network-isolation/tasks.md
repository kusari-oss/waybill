...
- [ ] T008 [P] [US1] Verify each modified manifest under `waybill-cli/tests/fixtures/` in isolation: `cd <fixture> && time go mod graph` completes in ~10ms, whether it prints a graph or a local `no such file or directory`. A result of a second or more means it is still reaching the proxy.
- [ ] T009 [US1] **Measurement checkpoint.** Re-measure the three arms from T001 and record them in `specs/843-fixture-network-isolation/baseline.md` beside the originals. SC-001 (two consecutive scans within 20%), SC-002 (networked within 20% of no-network), SC-003 (zero fixture-attributable network requests, from a fresh shim run against T002's inventory).
...
- [ ] T011 [US2] Run the three consuming tests identified in T004 — `waybill-cli/tests/mod_why_scaling.rs`, `waybill-cli/tests/goroot_skip.rs`, `waybill-cli/tests/pants_go_reader.rs` — and confirm they pass unchanged: `cargo +stable test --test mod_why_scaling --test goroot_skip --test pants_go_reader`.
- [ ] T012 [US2] For each fixture `docs/development/go-fixture-inventory.md` classifies as `deliberate`, verify it still fails to resolve — locally, not over the network. Research found this set empty; if T003 finds it non-empty, this task is where the missing-target shape is applied and verified.
- [ ] T013 [US2] Verify the Go resolution ladder still reports degraded coverage where it did before: compare `waybill:go-transitive-coverage`, `-reason` and `-fallback-count` in a fresh scan against the values committed in `waybill-cli/tests/fixtures/golden/cyclonedx/golang.cdx.json` (`unknown`, `offline-mode: …`, `11`). Any change means a fixture that was meant to stay unresolvable now resolves.
...
- [ ] T016 Prove the guard at `waybill-cli/tests/fixture_network_guard.rs` works (G-4.1): add a throwaway manifest with an unreplaced `require` under `waybill-cli/tests/fixtures/`, confirm the guard fails and names it, remove it, confirm the guard passes. Assert by doing, not by inspection.
...
- [ ] T018 [P] **No repository file** — file the `go version` finding as a GitHub issue against `kusari-oss/waybill`: the scan invokes it **27 times** per run as a per-workspace capability probe (T002). Independent of this feature, and cheap to cache.
...
- [ ] T020 Run the full pre-PR gate via `./scripts/pre-pr.sh` — `cargo +stable clippy --workspace --all-targets` and `cargo +stable test --workspace`, both clean — and enumerate the per-suite results rather than grepping for failures.
