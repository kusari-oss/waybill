...
- [X] T022 [US1] Implement `CompilerPipelineAggregator` struct in `mikebom-cli/src/trace/compiler_pipeline.rs` — state: `invocations: BTreeMap<u64, CompilerInvocation>`, `pid_to_invocation_id: HashMap<u32, u64>`, `next_invocation_id: u64`, `filter_config: FilterConfig`, `dropped_event_count: u64`. Methods: `handle_exec_event(&mut self, event: CompilerExecEvent)`, `handle_file_open(&mut self, event: FileOpEvent)`, `handle_file_close(&mut self, event: FileOpEvent)`, `finalize(self) -> CompilerPipelineData` (assembles DAG + sorts everything deterministically per R8)
- [X] T023 [US1] Implement userspace secrets-denylist + heuristic filter in `mikebom-cli/src/trace/compiler_pipeline.rs` per research R5 — glob-match paths against R5's userspace patterns (`/var/run/secrets/*`, `/run/secrets/*`, `/run/keys/*`, `~/.ssh/*`, `~/.aws/*`, `~/.gnupg/*`, `~/.docker/config.json`, `~/.netrc`, `~/.kube/config`) + basename heuristic (`.pem`, `.key`, `.crt`, `_rsa`, `_ed25519`). Expand `~` via the `dirs::home_dir()` (already in workspace via `mikebom-cli/src/scan_fs/oci_pull.rs`). On match, increment `secrets_read_filtered` counter + DROP the entry (unless `include_system_reads == true`)
- [X] T024 [US1] Implement DAG assembly in `CompilerPipelineAggregator::finalize` — walk `invocations` to produce `dag_edges` (parent→child linkages); sort invocations by (start_timestamp_ns, pid); for each invocation, sort read_set + write_set by path lex; sort dag_edges by (parent_invocation_id, child_invocation_id) per R8
- [X] T025 [US1] Extend `mikebom-cli/src/trace/loader.rs` — load + attach the new `sched_process_exec` + `sched_process_fork` tracepoints alongside existing programs; attach the extended file_ops kprobes (which already exist but now have the compiler-descendant prelude); wire the `COMPILER_EXEC_EVENTS` ring buffer into the processor
- [X] T026 [US1] Extend `mikebom-cli/src/trace/processor.rs` — dispatch new `COMPILER_EXEC` event type to `CompilerPipelineAggregator::handle_exec_event`; dispatch the file-op events with a `compiler_invocation_id` field (present when the descendant filter fired) to `handle_file_open` / `handle_file_close` on the aggregator

### CLI flag + integration

- [X] T027 [US1] Add `--include-system-reads` flag to `ScanArgs` in `mikebom-cli/src/cli/scan_cmd.rs` — boolean; default false; doc-comment cites FR-016 + FR-016a; wired to `FilterConfig::include_system_reads`. NO flag is added to `TraceRunArgs` beyond this scan-cmd flag reuse (matches the existing pattern where scan_cmd + trace share the same arg struct)
- [X] T028 [US1] Wire the finalized `CompilerPipelineData` into the emitted `BuildTracePredicate` — locate the trace-end code path (`mikebom-cli/src/cli/scan.rs` or `run.rs`); on trace completion, call `aggregator.finalize()` + set `predicate.compiler_pipeline = Some(data)`. When `--features ebpf-tracing` is off OR the host is non-Linux, keep `compiler_pipeline = None` per R10

### Emitter augmentation (mikebom-cli/src/generate/)

- [X] T029 [US1] Implement write-set-to-SBOM-component mapping helper `map_component_to_source_read_set(component, compiler_pipeline_data) -> Option<SourceReadSetPayload>` in `mikebom-cli/src/generate/mod.rs` per Clarifications Q1 — for each component with a file path (via m133 evidence OR file-anchoring `hashes[]`), find every invocation whose `write_set` contains the component's file path; take the union of those invocations' `read_set` PLUS the read_sets of ALL ancestor invocations (traverse `parent_invocation_id` chain). Sort the resulting read_set by path lex per R8
- [X] T030 [US1] Extend `mikebom-cli/src/generate/cyclonedx/builder.rs` — added `compiler_pipeline` field + `with_compiler_pipeline` setter on `CycloneDxBuilder`; `build_components` now calls the T029 mapping helper per component and appends `mikebom:source-read-set` (traced only) + `mikebom:read-set-source` (`traced|unknown`) as CDX `properties[]` entries. Wired at `cyclonedx/mod.rs` via `.with_compiler_pipeline(scan.compiler_pipeline.cloned())`.
- [X] T031 [US1] Extend `mikebom-cli/src/generate/spdx/annotations.rs` — `annotate_component` gains `compiler_pipeline: Option<&CompilerPipelineData>`; C130 + C131 emit at the tail via the `MikebomAnnotationCommentV1` envelope. Threaded through `packages::component_to_package` + `packages::build_packages` off `ScanArtifacts.compiler_pipeline`.
- [X] T032 [US1] Extend `mikebom-cli/src/generate/spdx/v3_annotations.rs` — `build_component_annotations` + `push_component_fields` gain `compiler_pipeline: Option<&CompilerPipelineData>`; C130 + C131 emit at the tail as SPDX 3 `Annotation` elements via the shared envelope. `v3_document.rs` passes `scan.compiler_pipeline` through.
- [X] T033 [US1] Register catalog rows C130 + C131 in `mikebom-cli/src/parity/extractors/mod.rs` EXTRACTORS array — added `c130_cdx`/`c130_spdx23`/`c130_spdx3` + `c131_*` extractor helpers in the per-format submodules, wired both rows into the EXTRACTORS table as `Directionality::SymmetricEqual`. Verified via the 3 parity structural tests (sort order + unique labels + catalog-row correspondence).
- [X] T034 [US1] Update `docs/reference/sbom-format-mapping.md` — appended C130 + C131 rows in Section C with wire-shape examples + `MikebomAnnotationCommentV1` envelope samples + Principle V audit ("KEEP-NO-NATIVE") citing standards-native alternatives that were rejected.
...
- [X] T042 [P] Emit `mikebom:compiler-pipeline-completeness` doc-scope annotation (C132) — extended `cyclonedx/metadata.rs::build_metadata` (new `compiler_pipeline` param + `metadata.properties[]` push), `spdx/annotations.rs::annotate_document` (envelope push), `spdx/v3_annotations.rs::push_document_fields` (envelope push). Wire value = `serde_json::to_value(&pipeline.completeness)` producing `{"state":"complete"}` / `{"state":"degraded",...}` / `{"state":"partial",...}` per data-model E11.
- [X] T043 [P] Emit `mikebom:secrets-read-filtered` doc-scope annotation (C133) — same 3 emitters, gated on `pipeline.secrets_read_filtered > 0`. Wire value = decimal u64 as string.
- [X] T044 [P] Emit `mikebom:trace-attach-late` per-component annotation (C134) — extended `cyclonedx/builder.rs::build_components` (per-component `properties[]` push), `spdx/annotations.rs::annotate_component` (envelope push), `spdx/v3_annotations.rs::push_component_fields` (envelope push). Wire value = literal `"true"`. Emission gate: `pipeline.completeness == Partial(AttachLate)` — best-effort per-component signal derived from doc-scope truth (per-invocation `attach_late: bool` is a future model extension per contracts/annotations.md A-5 "conservative interpretation" clause).
- [X] T045 [P] Register catalog rows C132..C134 in `mikebom-cli/src/parity/extractors/mod.rs` EXTRACTORS array — added `c132_cdx/c132_spdx23/c132_spdx3`, `c133_*`, `c134_*` extractor helpers; wired 3 rows into the table as `Directionality::SymmetricEqual`. Verified via the 3 parity structural tests (sort + unique labels + catalog correspondence).
- [X] T046 [P] Update `docs/reference/sbom-format-mapping.md` — appended C132/C133/C134 rows with wire-shape examples + `MikebomAnnotationCommentV1` envelope samples + Principle-V KEEP-NO-NATIVE rationale citing standards-native alternatives that were rejected.

### FR-018 stdin-input handling

- [X] T047 Implement stdin-input marker per FR-018 — extended `CompilerPipelineAggregator::handle_file_event` (the aggregator uses one unified `handle_file_event` rather than a separate `handle_file_open`) to detect `"-"` (compiler CLI convention) + `"/dev/stdin"` (POSIX fd-0 symlink) on Read/Open events and delegate to `record_stdin_input(invocation_id, bytes_read: 0)`. Detection runs BEFORE the FR-016 `/dev/` denylist so `/dev/stdin` isn't swallowed by the system-prefix filter. Write events on `/dev/stdin` are ignored (stdin is input-only). `bytes_read: 0` sentinel documented as pending future eBPF read-syscall byte counters. 3 new unit tests (`stdin_dash_read_event_lands_as_stdin_input_not_regular_file`, `stdin_dev_stdin_path_lands_as_stdin_input`, `stdin_write_event_is_ignored_not_treated_as_stdin_input`) pass locally. Integration test file scaffolded at `mikebom-cli/tests/compiler_pipeline_stdin.rs` behind `#[cfg(all(target_os = "linux", feature = "ebpf-tracing"))]` — body deferred alongside the T017 fixture (needs Linux + `sudo gcc -x c -` invocation).
...
- [X] T050 Verify default-features (no ebpf-tracing) build still passes clean — verified locally on macOS: `cargo check -p mikebom --all-targets` + `cargo clippy -p mikebom --all-targets -- -D warnings` both green; `cargo test -p mikebom --bin mikebom` returns `ok. 3098 passed; 0 failed`. `BuildTracePredicate.compiler_pipeline` deserializes as `None` on default-features hosts per R10.
...
- [X] T052 [P] Update `docs/architecture/attestations.md` — added subsection "`compiler_pipeline` (`CompilerPipelineData`, milestone 210)" under `## BuildTracePredicate fields`. Documents the eBPF capture path, per-invocation shape (invocation identity + lifetime + command context + I/O sets + diagnostic counters), per-scan aggregate signals (completeness / secrets_read_filtered / filter_categories_applied), and the C130/C131/C132/C133/C134 emission pipeline. Locks the `mikebom.dev/attestation/compiler-invocation/v0.1` URI per contracts/attestor-predicate.md.
- [X] T053 [P] Add architectural note to `docs/architecture/scanning.md` — added "Trace-mode compiler-pipeline enrichment (milestone 210)" section covering the end-to-end data flow (eBPF tracepoints → `COMPILER_INVOCATIONS` HashMap → ring buffer → user-space aggregator → `BuildTracePredicate.compiler_pipeline` → per-component + doc-scope annotations on all three SBOM formats).
- [X] T054 [P] Add contributor guide entry in `docs/architecture/attestations.md` — added "Adding a new compiler to the whitelist" subsection with 5-step recipe (COMPILER_WHITELIST edit, CompilerFamily enum extension, comm→family mapping helper, fixture, doc update).

### Final gates

- [X] T055 Verify byte-identity of pre-m210 attestation goldens — verified locally: `cargo test -p mikebom --bin mikebom --no-fail-fast` (3098 passed / 0 failed) + `cargo test --workspace --no-fail-fast` (0 failed across every target incl. mikebom_common's 11 fuzz tests + xtask + doctests) with no `--features ebpf-tracing`; `git status --short | grep -iE 'golden|fixture.*\.json|expected.*\.json'` returns empty afterward, confirming zero regeneration. The additive `Option<CompilerPipelineData>` field + emitter-side `if let Some(pipeline) = ...` gates preserve byte-identity for scan-mode.
