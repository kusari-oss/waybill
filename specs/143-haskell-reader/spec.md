...
- **FR-007a**: When the same dependency name is declared in **more than one manifest** in the scan, the emitted component MUST carry the constraint from **every** declaring manifest, and MUST cite every declaring manifest in `mikebom:source-files`. **Added after #936.** FR-007 was written for a single manifest and said nothing about the multi-manifest case, so the reader kept whichever manifest sorted first and discarded the rest — constraint and manifest path together. A library requiring `base >=4.11 && <4.22` alongside an example requiring `base >=4.14 && <4.15` emitted only the example's, which a consumer reads as the library's requirement. Dropping a constraint is not a smaller error than failing to correlate it with its manifest: the surviving value is not merely incomplete, it is wrong about the package it is attributed to.
- **FR-007b**: A `build-depends:` entry naming a package **defined elsewhere in the same scan** MUST NOT emit a separate design-tier component. **Added after #936.** The self-reference filter in FR-007 covered only the declaring manifest's own name, so a reference to a sibling package minted a second, versionless component for a package that already emits as a main module with a real version. The duplicate is unreachable from the root and degrades `mikebom:graph-completeness` to `partial` for a reason that is an artifact of the scan rather than a property of the project.
- **FR-007c**: A lockfile governs the `*.cabal` files in **its own directory**, and suppresses a dependency's design-tier entry only when it supplies an **exact pin for that dependency**. **Added after #938.** FR-007 says "when neither `cabal.project.freeze` nor `stack.yaml.lock` is present", leaving "present" unscoped; the implementation read it as repository-global and all-or-nothing, which failed twice over. A lockfile in one directory silenced every `*.cabal` in the tree, so a package elsewhere with no lockfile of its own lost its dependencies outright — the suppression fired with nothing to replace what it silenced. And within a governed directory a dependency the lockfile did not mention was dropped as well: no pin, because the lockfile is silent on it, and no fallback, because the lockfile suppressed it. Measured on a public library with a three-constraint freeze at the root: 5 components emitted where 21 were correct. A lockfile is authoritative about what it describes and says nothing about anything else; cabal itself scopes `cabal.project.freeze` to the project that owns it.
- **FR-007d**: A `RangeConstraint` entry in a freeze file MUST NOT suppress the declaration it restates. **Added after #938.** A range names an acceptable set, not a chosen member — the same class of information `build-depends:` already carries — so it is not grounds for silencing the manifest. Only an exact `==` pin is.
...
- **FR-009a**: A lockfile that parses cleanly but yields **zero usable constraints** MUST take the same fallback as a malformed one. **Added after #937.** The original wording distinguished only "present" from "malformed", leaving a third state — present, parsed, empty — to be treated as authoritative. Combined with the FR-002 defect below it deleted every dependency from the document: the suppression fired with nothing to replace what it silenced. A lockfile that describes nothing has not described the dependencies.

  *Note on FR-002, not a change to it.* Real `cabal v2-freeze` output qualifies every constraint by scope (`any.aeson ==2.2.3.0`, `setup.Cabal ==3.10.3.0`). FR-002's requirement to "extract each `==<version>` exact-pin constraint from the `constraints:` block" already covers those; the implementation did not, because every constraint regex anchored on a name that cannot contain a dot. Both the unit fixtures and the US1 integration fixture were hand-written with unqualified names — a format `cabal` never emits — so neither layer exercised real output. Fixture text for a parser should come from the tool it parses.
