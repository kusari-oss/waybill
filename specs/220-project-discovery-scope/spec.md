...
- **FR-004**: `--project-discovery=root-only` MUST honor ecosystem-native workspace-member declarations at the root level. When a root-level `Cargo.toml` has `[workspace] members = [...]`, or a root-level `package.json` has `"workspaces": [...]`, or a root-level `go.work` has `use (...)`, or a root-level `pom.xml` has `<modules>...`, waybill MUST walk each declared member's directory as if it were part of the root project. Members' `pkg:*` components + transitive deps land in the SBOM tagged with `waybill:workspace-member` (existing m127-era annotation).
