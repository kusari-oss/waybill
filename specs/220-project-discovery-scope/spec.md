...
- **FR-004**: `--project-discovery=root-only` MUST preserve ecosystem-native workspace-declared members in the emitted SBOM. Members are identified via the existing `waybill:workspace-member` annotation stamped by m176 (see FR-006). That annotation carries a **JSON-encoded array of scan-root-relative workspace DIRECTORIES** (`["."]`, `["services/api"]`) derived from each component's own `evidence.source_file_paths` — it is self-descriptive directory provenance, NOT a back-reference to a root's PURL, and it is stamped on plain transitive dependencies too. **No per-reader walker changes are required**: readers walk everything at discovery time under all modes; m220's post-discovery filter RETAINS components whose annotated directories are in scope and DROPS components living under an out-of-scope nested directory. Because m064 gives every cargo main-module in a workspace the shared root `Cargo.lock` as its only evidence path, root and members alike carry `["."]` and are retained together, while an independent nested project carries its own subdirectory and is not.
...
- **FR-007**: `--project-discovery=strict` MUST discover ONLY the root-level manifest file(s). Workspace-member declarations are IGNORED — the SBOM contains the root manifest's own PURL + its own dependency closure ONLY, with no workspace-member main-modules. "Root manifest only" scopes which PROJECTS are discovered; it does not strip the discovered root's own dependencies, including those that are graph orphans because their reader emits no `dependsOn` edge (e.g. Gemfile.lock gems). Root-vs-member disambiguation reuses m201's `waybill:is-workspace-root` flag; when no in-scope root carries it, Strict degrades to root-only behaviour rather than inventing a new heuristic (FR-006).
