...
- [X] T001 Verify branch `220-project-discovery-scope` is checked out and up-to-date with `main` post-alpha.68 release. Confirm HEAD is the plan-phase commit: `git log -1 --oneline` should show `7edd4cc plan(220): project-discovery scope — plan + research + data-model + contracts + quickstart`.
- [X] T002 Capture the alpha.68 SC-005 baseline: build release binary at HEAD (`cargo +stable build -p waybill --release`), then run against an existing m215 split fixture: `./target/release/waybill sbom scan --path waybill-cli/tests/fixtures/split_heterogeneous --format cyclonedx-json --output /tmp/m220_baseline.cdx.json`. Snapshot component count via `jq '.components | length' /tmp/m220_baseline.cdx.json`. This is the reference set that default-mode m220 output MUST reproduce byte-identically.
- [X] T003 [P] Read `waybill-cli/src/generate/split.rs:96-160` (`enumerate_workspace_roots` + `SubprojectRoot` struct + `is_main_module` helper). This is the discovered-main-module surface m220 filters.
- [X] T004 [P] Read `waybill-cli/src/generate/split.rs:220-320` (`project_for_root` BFS). This is the BFS routine m220's filter reuses verbatim per contracts/scope-filter-algorithm.md Step 2.
- [X] T005 [P] Read `waybill-cli/src/cli/scan_cmd.rs:3234-3300` (the existing `waybill:workspace-member` reader-aggregate that already inspects the annotation). Confirm the annotation values are populated by cargo/npm/go/maven readers per contracts/workspace-member-preservation.md ecosystem-detection matrix.
...
- [X] T006 Create `waybill-cli/src/generate/project_discovery/` module dir with three files: `mod.rs` (`ProjectDiscoveryMode` enum per data-model E1 + `is_root_in_scope` per E2 + `follows_workspace_members` per E3 + `Display` per E4 + `ProjectDiscoveryReport` struct per E5), `filter.rs` (empty stub — populated by T009), `report.rs` (helper for the report's INFO-log formatting, optional). Register the new module in `waybill-cli/src/generate/mod.rs` via `pub mod project_discovery;`.
- [X] T007 [P] Add `pub project_discovery_mode: Option<crate::generate::project_discovery::ProjectDiscoveryMode>` field to `ScanArtifacts` in `waybill-cli/src/generate/mod.rs` per data-model E8. Default at every construction site: `None`. Grep for `ScanArtifacts {` construction sites (7+ per m218/m219 precedent) and add the new field to each. Update `ScanArtifacts::narrow` to copy the field through.
- [X] T008 In `waybill-cli/src/cli/scan_cmd.rs`, add the `--project-discovery` clap arg per data-model E7 + contracts/project-discovery-flag.md. Type: `pub project_discovery: crate::generate::project_discovery::ProjectDiscoveryMode`. Clap attributes: `long = "project-discovery"`, `value_enum`, `default_value = "all"`, `require_equals = true`. Update `Default for ScanArgs` impl to set `project_discovery: ProjectDiscoveryMode::All`. Also add the env-var bridge at the same site scan_cmd bridges other flags (mirror the m218 `WAYBILL_EXPERIMENTAL_CROSS_ECOSYSTEM_EDGES` pattern): when `args.project_discovery != ProjectDiscoveryMode::All`, set `std::env::set_var("WAYBILL_PROJECT_DISCOVERY", args.project_discovery.to_string())`.
- [X] T009 In `waybill-cli/src/generate/project_discovery/filter.rs`, implement `pub fn apply_scope_filter(...)` per contracts/scope-filter-algorithm.md (5 steps: fast-path All, enumerate + filter main-modules, BFS-project each root via m215 `project_for_root`, workspace-member annotation follow-up under RootOnly, filter component + relationship slices, build report). **Step 3 MUST include the FR-005 fixpoint-recursion pass** per contracts/workspace-member-preservation.md "Recursion (FR-005 nested workspaces)" section — after the initial annotation follow-up, iterate: any pulled-in component tagged `waybill:component-role = main-module` joins `root_purls`; re-run the annotation follow-up against the expanded set; repeat until no new roots are added (fixpoint terminates because component set is bounded). Copy the pseudocode from contracts/workspace-member-preservation.md lines 74-104 verbatim as the implementation guide. Add 5 unit tests: (1) All mode is zero-op; (2) RootOnly on a synthetic 2-root fixture drops nested; (3) RootOnly preserves workspace-member components; (4) Strict drops workspace members; (5) report counters correct. Note: T021 + T022 add 2 more unit tests (belt-and-suspenders + nested-workspace fixpoint) — filter.rs ends up with 7 unit tests total.
- [X] T010 [P] Add C140 parity extractors per contracts/project-discovery-annotation.md. Three files touched: `waybill-cli/src/parity/extractors/cdx.rs` (add `cdx_anno!(c140_cdx, "waybill:project-discovery-mode", document);` at end); `spdx2.rs` (add `spdx23_anno!(c140_spdx23, ..., document);`); `spdx3.rs` (add `spdx3_anno!(c140_spdx3, ..., document);`). Then `parity/extractors/mod.rs`: add `c140_cdx`, `c140_spdx23`, `c140_spdx3` to the three use-list blocks + add a new `ParityExtractor` row for C140 after C139 (m218's last row).
- [X] T010b [P] Add C140 wire-emission code paths in each of the three format emitters (parity-catalog registration in T010 is separate — this task wires up the actual emission into the emitted SBOM). Three files touched — copy the m217 C136 `waybill:go-toolchain-detected` emission pattern verbatim (grep each file for `waybill:go-toolchain-detected` to locate the exact insertion sites):
  - `waybill-cli/src/generate/cyclonedx/metadata.rs` — when `artifacts.project_discovery_mode.is_some()`, append `{"name": "waybill:project-discovery-mode", "value": mode_string}` to `metadata.properties[]`. Use `mode.to_string()` (invokes the Display impl from E4).
  - `waybill-cli/src/generate/spdx/annotations.rs` — when `artifacts.project_discovery_mode.is_some()`, emit a document-scope `Annotation` on `SPDXRef-DOCUMENT` with `MikebomAnnotationCommentV1` envelope shape per contracts/project-discovery-annotation.md wire example.
  - `waybill-cli/src/generate/spdx/v3_annotations.rs` — when `artifacts.project_discovery_mode.is_some()`, emit an `Annotation` element on the SpdxDocument root IRI with the same envelope shape.
  Silence gate: when `artifacts.project_discovery_mode.is_none()` (default `All` mode), emit NOTHING — SC-005 byte-identity requires zero delta. Coupled with T010 + T011 (parity extractor + docs row) via the `every_catalog_row_has_an_extractor` bidirectional test — all three tasks belong to the same commit.
- [X] T011 [P] Add a docs C140 row to `docs/reference/sbom-format-mapping.md` following the m217 C136 + m218 C137/C138/C139 doc-scope KEEP-NO-NATIVE template. Content per contracts/project-discovery-annotation.md standards-native audit (4 rejected alternatives). **⚠️ COUPLED WITH T010**: T011 (docs row) MUST commit together with (or before) T010 (extractor registration) to keep the `every_catalog_row_has_an_extractor` bidirectional test green at every commit. Same trip that caught m216/m217/m218/m219 pre-PR gates.
- [X] T012 In `waybill-cli/src/scan_fs/mod.rs`, wire up the filter integration. Read the mode from env var `WAYBILL_PROJECT_DISCOVERY` (**LOAD-BEARING**: scan_fs consumes the CLI value via this env-var bridge per contracts/project-discovery-flag.md; T008's `set_var` at the scan_cmd site is the sole write-path — without the bridge the CLI arg is invisible to scan_fs). Falls back to `ProjectDiscoveryMode::All` on parse failure. After `enumerate_workspace_roots` populates the resolved-component + relationship set, call `apply_scope_filter(components, relationships, mode, &scan_root)` per contracts/scope-filter-algorithm.md.

  **FR-008 empty-root fallback** (spec Edge Cases §1): after `apply_scope_filter` returns, check `report.root_main_modules == 0 && mode != ProjectDiscoveryMode::All`. If true, emit `tracing::warn!(mode = %mode, "scan: project-discovery=<mode> found zero root-level manifests; falling back to synthetic pkg:generic root")` and use the pre-filter component + relationship slices (i.e., treat as if mode were `All` for that scan) so the existing synthetic-`pkg:generic/<repo>@0.0.0-unknown` root path fires cleanly per FR-008. Mark `ScanArtifacts.project_discovery_mode = None` for the fallback branch so C140 is NOT emitted (fallback = no scope was actually applied). Add a unit or integration test on an all-nested-manifests fixture (root dir has no manifest; every project is in `services/*/{package.json, go.mod, Cargo.toml}`) asserting: WARN log fires, SBOM is emitted (non-empty), C140 annotation absent, `nested_projects_ignored == 0`.

  Thread the returned `ProjectDiscoveryReport.mode` (Some(mode) when non-default AND fallback did not fire) into `ScanArtifacts.project_discovery_mode`. Emit FR-012 INFO log per contracts (`tracing::info!(mode = %report.mode, root_main_modules = report.root_main_modules, workspace_members_followed = report.workspace_members_followed, nested_projects_ignored = report.nested_projects_ignored, "scan: project-discovery mode complete")`) when mode is non-default.
...
- [X] T013 [US1] Author the polyglot-nested-independent fixture at `waybill-cli/tests/fixtures/project_discovery/polyglot_nested_independent/`. Structure per research R10:
...
- [X] T018 [US2] Author the cargo-workspace-with-independent-neighbor fixture at `waybill-cli/tests/fixtures/project_discovery/cargo_workspace_with_independent_neighbor/`. Structure per research R10:
...
- **Phase 2** (T006-T012 + T010b) — T006 first (creates the module); T007 || T008 (different files); T009 depends on T006; T010 || T010b || T011 (parity extractors + wire emitters + docs row; **⚠️ ALL THREE MUST commit together per bidirectional catalog test invariant + C140 emission gate — parity registration without emission = registered-but-silent annotation, breaks SC-008**); T012 depends on T009 (needs apply_scope_filter available) and T010b (needs C140 emitter present to satisfy the FR-011 emission gate).
...
- **T010+T010b+T011 must commit together** per bidirectional catalog test invariant + C140 emission gate (analyze-phase lesson from m216/m217/m218/m219). All three parallel-authorable, then bundled into one commit — parity row + wire emitters + docs row are one atomic change.
...
**MVP scope (US1 + US2 both P1)**: Ship Phases 1+2+3+4+part-of-6 (T001-T022 + T026-T035, skip US3/T023-T025) — plus T010b from Phase 2. Delivers the shallow-scan correctness fix + workspace-member preservation. **33 tasks.**

**Recommended scope (US1 + US2 + US3)**: Ship all 36 tasks. Adds Strict-mode coverage — the third variant costs 3 tests and gives operators the literal-shallow semantic + a natural third enum variant. **36 tasks total.**

## Format Validation

All 36 tasks follow the checklist format (`- [ ] TID [P?] [Story?] Description with file path`). Story labels present on all Phase 3-5 tasks (US1/US2/US3); absent on Phase 1/2/6 tasks per convention. File paths absolute-repository-relative throughout. Parallel markers `[P]` applied where independence is genuine.
