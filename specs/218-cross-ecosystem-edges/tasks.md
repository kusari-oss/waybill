...
- [ ] T020 [US1] Thread from `ScanDiagnostics` to `ScanResult` in `waybill-cli/src/scan_fs/mod.rs`, then from `ScanResult` to `ScanArtifacts` construction in `waybill-cli/src/cli/scan_cmd.rs`. Grep for existing `go_workspace_mode` field propagation as the template — m161/m217 established this thread pattern.
...
- [ ] T031 [US2] In `waybill-cli/src/generate/spdx/annotations.rs`, add per-Package + doc-scope emission analogous to T029/T030. Per-Package annotations MUST use the standard `MikebomAnnotationCommentV1` envelope. Grep for `waybill:workspaces-detected` in that file — the m176 doc-scope precedent is the exact template to mirror for C139; grep for `waybill:package-shape` — the m216 per-Package precedent is the template for C137/C138.
- [ ] T032 [US2] In `waybill-cli/src/generate/spdx/v3_annotations.rs`, add per-Relationship + doc-scope emission for the SPDX 3 format. C137/C138 emit as `Annotation` elements whose `subject` IRI is the specific `Relationship` element joining source + target. C139 emits doc-scope on the SpdxDocument root IRI — same template as m217 C136 (grep for `waybill:go-toolchain-detected` in v3_annotations.rs). Requires threading the report through the SPDX 3 emission pipeline in `waybill-cli/src/generate/spdx/v3_document.rs` (same shape as m217's `go_toolchains_detected` field addition to v3_document's ScanArtifacts constructor).
...
- [ ] T036 [US2] Author a purpose-built ambiguous-multi-eco fixture at `waybill-cli/tests/fixtures/cross_ecosystem/ambiguous_multi_eco/` — a scan directory containing (a) a mini Gemfile.lock declaring `json` as a dep, AND (b) a mini `Cargo.toml` declaring another workspace member so the scan surfaces `pkg:cargo/` components alongside the pkg:gem/, plus (c) a synthetic `pkg:pypi/json@0.1.1` component fabricated via a small helper test-fixture setup. Add `#[test] fn us2_ambiguous_match_emits_all_candidates_with_c138()` asserting: multiple edges emitted from the `pkg:generic/` main-module for the same dep name (`json`), each carrying BOTH C137 AND C138 annotations, with `alternates[]` correctly excluding self.
...
- **Phase 3 US1** (T013-T023) — depends on Phase 2 complete. T013 || T014 (different files); T015 depends on T014; T016 depends on T013+T014; T017 || T016 (same file, sequential); T018 depends on T017; T019 depends on T018; T020 depends on T019; T021 depends on T020; T022 depends on T021 (flag-on scan needs full pipeline); T023 depends on T021 (same reason).
- **Phase 4 US2** (T024-T036) — depends on Phase 3 US1 complete (needs report data flowing). **⚠️ T024 (docs C137/C138/C139 rows) MUST commit together with (or before) T028 (EXTRACTORS registration)** — bidirectional catalog test invariant per m216/m217 C1 pattern. T024 → T025 || T026 || T027 (all three format-side extractor registrations parallel) → T028 (mod.rs registration composes them). T029 || T030 (different files); T031 depends on T029+T030 (SPDX 2.3 mirrors CDX); T032 depends on T031 (SPDX 3 threading pattern). T033-T036 test tasks depend on emission code (T029-T032) complete.
