...
- **FR-002**: The synthetic main-module component MUST use PURL type [NEEDS CLARIFICATION: `pkg:gem/<name>@<version>` (published-gem shape; consistent with the gem reader's other components but implies the app IS a published gem, which it isn't) VS `pkg:generic/<name>@<version>` (honest but less searchable — CVE scanners keying on `pkg:gem/` won't match) VS `pkg:gem/<name>@<version>` with a `waybill:package-shape = "application"` disambiguating annotation (best of both — real ecosystem-aware PURL, plus a marker that identifies it as an app not a gem)]. The choice materially affects downstream consumer behavior (vuln scanners, SBOM merge tools) and cannot be defaulted without operator/consumer input.
...
- **FR-008**: The new component MUST carry an annotation that distinguishes it from a published-gem main-module so downstream consumers can filter (annotation name TBD in FR-002 resolution).
...
- **FR-012**: The emitted sub-SBOM filename for a Ruby application MUST follow the m215 `<slug>.<ecosystem>.<format-ext>.json` convention with `<ecosystem>` matching whatever PURL type FR-002 resolves to.
...
- **SC-001**: Running `waybill sbom scan --split` against the `~/Projects/iac` reproducer monorepo emits **37 sub-SBOMs** (was 34 pre-feature) — the 3 new sub-SBOMs correspond to `common-infra/`, `app-infra/`, `archives/gcp/`.
- **SC-002**: On a fixture directory containing `Gemfile` + `Gemfile.lock` with 2 declared deps and no `.gemspec`, a single-SBOM scan emits a root component whose `metadata.component` identifies the Ruby application (NOT the `synthetic-placeholder` heuristic).
...
- **Companion annotation**: A new annotation (e.g., `waybill:package-shape = "application"` or similar) will be added to distinguish Gemfile-derived main-modules from published-gem main-modules. Name subject to FR-002's clarification resolution. Precedent: the milestone-004 US2 binary-role annotations use a similar pattern.
