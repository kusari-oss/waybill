...
**The probe has a known bug, and fixing it is a task.** Its nulled-set
extraction uses a line-anchored regex and therefore reports
`editedCabalFile` — an attribute inside a derivation override, not a package
(research R3). Any count it prints for boot libraries is high by at least one.
...
- **Do not use indentation** to decide whether `x = null;` is a package
  override. Track attrset depth (R3).
